...
\tikzset{
  ...
  gvcut/.style ={gvertex, draw=roleDiscover, fill=roleDiscover!20},
  ...
  gdir/.style     ={-{Stealth[length=2.2mm]}, line width=1.1pt, KULblauw1, line cap=round},
...
\makeatletter
\def\thm@space@setup{%
    \thm@preskip=1em plus 0.3em minus 0.2em
    \thm@postskip=1em plus 0.3em minus 0.2em
}
\makeatother
...
% --- epistemic-role palette: the MODEL -> MEASURE -> PROVE/DISCOVER -> OBSERVE
...
\definecolor{roleObserve}{RGB}{140,80,160}    % violet     : sampled estimates
...
\expandafter\newcommand\csname rolebadge@OBSERVE\endcsname{\rb{roleObserve}{OBSERVE}}
...
% DISCOVER -> OBSERVE spine scannable down the page at a glance.
...
\begin{tikzpicture}[>=Stealth,
  ...
  \node[rolenode, fill=roleObserve!18,  draw=roleObserve]  (obs) at (10.5,0)  {\textbf{OBSERVE}\\[1pt]\scriptsize sampling};
  \draw[flow] (mod) -- (mea);
  \draw[flow] (mea) -- (pro);
  \draw[flow] (mea) -- (dis);
  \draw[flow] (pro) -- (obs);
