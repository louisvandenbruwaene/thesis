...
# mostly finish through n=5-8, hypergraph rows through n=3-5, so both
# ranges run a couple of sizes past that into formula/lower-bound territory
# rather than stopping exactly where the machine-checked numbers do.
_GRAPH_TABLE_NS = list(range(2, 9))    # n = 2..8
_HYPER_TABLE_NS = list(range(3, 9))    # n = 3..8
...
def variant_value_tables() -> None:
    """Exact-number companions to the four variant-bounds grids.

    Same sixteen panels, same colour code (vtProved/vtConjectured/vtOpen for
    a row's status, vtExact overriding it wherever the machine independently
    confirms the number), but as numbers a reader can read off instead of a
    curve position they have to estimate by eye. Written as four plain
    ``tabular`` fragments, one per grid, meant to be \\input inside a
    ``table`` environment the way figures/rediscovery_table.tex already is.

    Split out of :func:`main` so it can be rerun on its own
    (``python make_figures.py --tables-only``).
    """
    groups = [
        (0, 8, _GRAPH_TABLE_NS, "graphs"),
        (8, 16, _HYPER_TABLE_NS, "hypergraphs"),
    ]
    for m_val in (3, 6):
        print(f"gathering all-variant grid m={m_val} for the value tables...")
        panels = gather_variant_grid(m=m_val)
        for lo, hi, ns, stem in groups:
            lines = [
                f"% Generated by make_figures.py:variant_value_tables(), m={m_val}.",
                f"% Colours: vtProved/vtConjectured/vtOpen match the curve in the",
                f"% matching figure; vtExact (bold) is a machine-checked value.",
            ...
            out = FIGURES / f"variant_table_m{m_val}_{stem}.tex"
            out.write_text("\n".join(lines) + "\n")
            print(f"wrote {out.name}")
