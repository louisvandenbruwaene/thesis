...
## 2026-09-02 (Opus, second session) — input validation, and render vs rebuild

VALIDATION. `_require_integer(name, value, minimum, maximum, range_error)` is
now the only place an integer is checked, because the forgotten line is the
first one: `bool` subclasses `int`, so `True` passes every naive test and means
vertex 1. Wired into both `__init__`s, `_require_vertex`, `_assign`,
`add_edge`/`remove_edge`, `Hypergraph._vertex_set`, both connectivity endpoint
guards, and `solve`'s domain (n >= 1, m >= 2, 2 <= r <= n; n = 1 is admitted
because 0 is its true answer).

**TWO LAUNDERING PATHS, both found by writing the fix rather than reading the
audit that prompted it.** A guard on `_assign` alone does NOT catch
`add_edge(0, 1, 1.9)` on a simple graph: `min(mu + 1.9, 1)` returns a clean
integer 1 first. And `frozenset([0, 1, True])` is `{0, 1}`, so a bad member
disappears instead of being rejected, leaving a hyperedge one vertex short,
which a declared uniformity catches by accident and `r=None` does not catch at
all. Check before the normalising step, never after.

CACHE. `--refresh` reconciled a rerun against the record and kept the better
value, so a "successful" refresh could stamp the new program's hash onto old
values. Fixing that ran into the deeper problem: a fingerprint answers "did any
byte change", never "did any answer change". Rendering now never computes (a
missing key is an error), `--rebuild` recomputes from scratch into a candidate,
`--compare` lists departures, `--promote` is the only act that writes the
record. Provenance recorded, never enforced. `_reconcile` and `--accept-stale`
deleted.

MEASURED. A full rebuild is 65 minutes for 529 values. All 129 exact values came
back identical, which is the evidence the validation changed no answer. 132
search entries moved, 89 down, and exactly one plotted point changed, upward.
Larger m is CHEAPER: a weaker constraint lets the incumbent climb and prune.
The 1800s budgets are headroom that is never spent.

CROSS-MODEL FLOOR. `_lift_multi_above_simple`: a simple witness is a multi
witness, so no multi lower bound may sit below its simple counterpart. The m=6
hypergraph vertex panel broke this at six of ten sizes (13 against 15 at n=7),
a search artefact of the (m-1)x larger space, present in the record long before
this session.

RECORD_REVISION. `git diff` ignored untracked files; now `git status
--porcelain`. `git tag -f` could move a tag already printed in a handed-in PDF;
now it refuses. And the documented rebuild needs `-g`, because a file read with
`\InputIfFileExists` while absent is recorded as a lookup that failed, not a
dependency to watch.

VERIFY: 157 tests OK, 1 skip. Self-check ALL CHECKS PASSED. 111pp, all gates 0.
