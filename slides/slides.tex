...
\begin{columns}[c,onlytextwidth]
  \begin{column}{0.40\textwidth}\centering
    \begin{tikzpicture}[scale=1.0,every node/.style={transform shape}]
      ...
      \node[font=\scriptsize\color{black!60}] at (0,-2.0) {two independent $s$-$t$ routes};
    \end{tikzpicture}
  \end{column}
...
\begin{columns}[t,onlytextwidth]
  \begin{column}{0.31\textwidth}\centering
    ...
    {\color{black!75}Quantify a network's\\spare, independent routes.}
  \end{column}
  \begin{column}{0.31\textwidth}\centering
    {\bfseries\color{KULblauw1}The tools}\\[8pt]
    {\color{black!75}maximum flow,\\search, certificates}
  \end{column}
  \begin{column}{0.31\textwidth}\centering
    {\bfseries\color{KULblauw1}Where it lives}\\[8pt]
    {\color{black!75}grids, pipelines,\\road networks}
  \end{column}
\end{columns}
\vfill
\end{frame}
...
\begin{tikzpicture}[
  ...
  \node[opt] at (-3.0,2) {simple};
  \node[opt] at (-1.0,2) {multigraph};
  \node[opt] at (1.3,2)  {hypergraph};
  \node[font=\large\bfseries,text=black!55] at (3.2,2) {$\times 3$};

  \node[ax] at (-5.6,0.7) {Direction};
  \node[opt] at (-3.0,0.7) {undirected};
  \node[opt] at (-1.0,0.7) {directed};
  \node[font=\large\bfseries,text=black!55] at (3.2,0.7) {$\times 2$};

  \node[ax] at (-5.6,-0.6) {Separation};
  \node[opt] at (-3.0,-0.6) {edge-disjoint};
  \node[opt] at (-1.0,-0.6) {vertex-disjoint};
...
\begin{tikzpicture}[
  ...
  \node[bin,fill=stProved!12,draw=stProved] at (-4.7,0) {};
  \node[bin,fill=stMachine!12,draw=stMachine] at (-1.55,0) {};
  \node[bin,fill=stConj!12,draw=stConj] at (1.6,0) {};
  \node[bin,fill=stOpen!12,draw=stOpen] at (4.75,0) {};
  ...
  \node[it] at (-4.7,0.1) {undirected edge\\[4pt] multigraph vertex\\[4pt] directed $m=2$};
  \node[it] at (-1.55,0.1){directed multigraph,\\$n\le 6$\\[4pt] hypergraph vertex,\\$m\le 3$};
  \node[it] at (1.6,0.1)  {directed arc, $m\ge 3$\\[4pt] directed vertex, $m\ge 3$};
  \node[it] at (4.75,0.1) {vertex-disjoint, $m\ge 6$\\[4pt] hypergraph vertex,\\$m\ge 4$\\[4pt] directed hypergraph};
\end{tikzpicture}
\vfill
\end{frame}
...
\begin{columns}[t,onlytextwidth]
  \begin{column}{0.52\textwidth}
    {\bfseries\color{KULblauw1}Search: annealing vs tabu}\\[8pt]
    ...
    \begin{tabular}{lrrr}
      ...
      \bottomrule
    \end{tabular}\\[8pt]
    {\footnotesize\color{black!60}Tabu reaches the directed optima\\where annealing plateaus.}
  \end{column}
  \begin{column}{0.46\textwidth}
    {\bfseries\color{KULblauw1}Code: Python with a C hot path}\\[10pt]
    \centering
    \begin{tikzpicture}[box/.style={rounded corners=3pt,minimum width=5.0cm,
      ...
      \node[box,fill=KULblauw1!12,draw=KULblauw1] (p)
        {Python core:\; \texttt{numpy} + \texttt{scipy} max-flow};
      \node[box,fill=vertexorange!12,draw=vertexorange!85!black,below=5pt of p] (c)
        {optional C hot path\; (\texttt{\_erdos\_fast.so})};
    \end{tikzpicture}\\[12pt]
    \footnotesize\raggedright Speedups:\;\;
    max-flow $\approx 2\times$,\;\; canonical form $\mathbf{147\times}$.
  \end{column}
\end{columns}
\end{frame}
...
\begin{columns}[c,onlytextwidth]
  \begin{column}{0.52\textwidth}\centering
    ...
    {\scriptsize\color{black!60}At $m=3$: linear hub leads early,\\quadratic family overtakes near $n=9$.}
  \end{column}
  \begin{column}{0.44\textwidth}
    {\bfseries Two genuine advances}\\[12pt]
    ...
    the whole family collapses to {\bfseries two checks} on $7$ vertices.
  \end{column}
\end{columns}
\end{frame}
...
\begin{columns}[t,onlytextwidth]
  \begin{column}{0.54\textwidth}
    ...
    \begin{itemize}\itemsep6pt
      \item the backward-arc lemma (closes the directed case)
      \item two finite $n=7$ checks ($m=3$ multigraph)
      \item vertex-disjoint $m\ge6$, open since 1974
      \item hypergraph vertex $m\ge4$
    \end{itemize}
  \end{column}
  \begin{column}{0.43\textwidth}
    {\bfseries\color{KULblauw1}One pipeline, every variant}
    \begin{itemize}\itemsep6pt
...
\begin{tikzpicture}\node[draw=KULblauw1,line width=1.2pt,rounded corners=5pt,
    fill=KULblauw1!8,inner sep=9pt,text width=0.82\textwidth,align=center]
  {\itshape The microscope does not answer every question. It shows exactly
   which questions are left.};\end{tikzpicture}
\end{center}
\end{frame}
