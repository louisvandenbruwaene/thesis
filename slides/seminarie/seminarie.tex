...
{$c_m>m/2$ \ \sProposed\ \ {\scriptsize\color{black!50}under the block formulation}}
{\begin{vlist}
  \item at $m=5$ the two agree to $n=13$, first gap $33$ against $32$ at $n=14$
  \item $c_m=\lim_n k_m(n)/(n-1)$, and $m/2<c_m\le2(m-1)$
\end{vlist}}
...
{\begin{vlist}
  \item so $\ell_m^{\dir}(n)=n^2/4+\Theta_m(n)$, but the linear coefficient is open
...
{$M(n)$ at $m=2$ \ \sProved\newline $\tfrac{n^2}{4}+\Theta_m(n)$ \ \sProved\ {\scriptsize\color{black!50}[conditional]}}
...
{$(m-1)\tfrac{n^2}{4}+O_m(n)$ \ \sProposed}
{thickened blocks small $n$, bipartite large $n$ \ \sProposed}
{\begin{vlist}
...
{\begin{vlist}
  ...
  \item off that range the supply of distinct hyperedges runs out: at $n=r=m=3$
        the bound reads $2$ and only one hyperedge exists
\end{vlist}}
{\cStarHyper}{degree-capped star hypertree \ \tExtremal}
...
{a star hypertree attains both \ \sProved\ \ {\scriptsize\color{black!50}$m=3$ for $2<r<n$}}
{\begin{vlist}
...
\end{vlist}}
{\cStarHyper}{star hypertree \ \tExtremal}
