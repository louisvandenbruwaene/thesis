...
\column{0.585\textwidth}
  {\footnotesize
   \noindent{\color{black!40}\bfseries\scriptsize UPPER}\ \ #3\par
   \vspace{7pt}
   \noindent{\color{black!40}\bfseries\scriptsize LOWER}\ \ #4\par}
  \vspace{10pt}
  {\footnotesize #5}
...
\newcommand{\cTheta}{\begin{tikzpicture}
  \node[gvertex, minimum size=4.8mm] (x) at (-1.6,0) {};
  \node[gvertex, minimum size=4.8mm] (y) at ( 1.6,0) {};
  \foreach \i/\yy in {1/1.0, 2/0, 3/-1.0}{
...
\newcommand{\cRepStar}{\begin{tikzpicture}
  ...
  \foreach \i/\a in {1/90, 2/210, 3/330}{
    ...
    \coordinate (w\i) at ({\a+38}:1.6);}
  \begin{scope}[on background layer]
    \foreach \d in {1.6pt,-1.6pt}{
      \draw[metro, metroA, line width=2.4pt, transform canvas={xshift=\d}] (u1) -- (c) -- (w1);
      \draw[metro, metroB, line width=2.4pt, transform canvas={xshift=\d}] (u2) -- (c) -- (w2);
      \draw[metro, metroC, line width=2.4pt, transform canvas={xshift=\d}] (u3) -- (c) -- (w3);}
  \end{scope}
  \node[metrostop, minimum size=5.4mm] at (c) {};
  \foreach \i in {1,2,3}{\node[metrostop, minimum size=4.6mm] at (u\i) {};
                         \node[metrostop, minimum size=4.6mm] at (w\i) {};}
...
{$k_m(n)=\ell_m(n)$ for $m\le4$, \ $k_5(n)=\floorfrac{8n}{3}-4$ for $n\ge14$ \ \sProved\par
 \hspace*{1.15cm}{\scriptsize\color{black!50}$m\ge6$:} \sOpen}
...
{$M(n)=\max\bigl(2(n-1),\floorfrac{n^2}{4}\bigr)$ at $m=2$ \ \sProved\par
 \hspace*{1.15cm}$\floorfrac{n^2}{4}+4(m-1)(n-1)$ at every $n$ \ \sProved}
...
{$M(n)$ at $m=2$ \ \sProved\ \ {\scriptsize\color{black!50}the $m=2$ argument ignores separation}\par
 \hspace*{1.15cm}$\tfrac{n^2}{4}+\Theta_m(n)$ \ \sProved\ {\scriptsize\color{black!50}[conditional]}}
...
{$K_2(n)=n-1$ \ \sProved\par
 \hspace*{1.15cm}order $m^2n$, coefficient $\le2$ \ \sProposed}
{$K_3(n)\ge2(n-1)$ \ \sProved\par
 \hspace*{1.15cm}coefficient $3-2\sqrt2$ \ \sProposed}
{\begin{vlist}
  \item parallel copies are separated routes, so this is not the simple problem:
        $K_5(4)=14$ against $L_5(4)=12$
  \item neither a common limit nor the best blocks are established
\end{vlist}}
{\cTheta}{theta block \ \tNear}
...
\end{vlist}}
{\cAugBip}{thickened bipartite digraph \ \tNear}
...
{$\floorfrac{n-1}{r-1}$ at $m=2$, \ $\floorfrac{2(n-1)}{r-1}$ at $m=3$ \ \sProved\par
 \hspace*{1.15cm}{\scriptsize\color{black!50}$m\ge4$:} \sOpen}
...
{$\floorfrac{n-1}{r-1}$ at $m=2$, \ $\floorfrac{2(n-1)}{r-1}$ at $m=3$ \ \sProved\par
 \hspace*{1.15cm}{\scriptsize\color{black!50}larger $m$:} \sOpen}
...
\end{vlist}}
{\cTwoLayer}{two-layer construction, hyperedges repeated \ \tAsym}
