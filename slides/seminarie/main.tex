...
\vspace{2pt}
\noindent{\tiny\color{black!60} The eight models side by side, undirected on top and directed below. The simple graph is a triangle, the multigraph has parallel edges, the hypergraph has three hyperedges, and the multi-hypergraph has one repeated hyperedge. The directed versions have arcs instead of edges, with the same pattern of repetition and hyperedges.}\par
\end{frame}
...
%  thm:multigraph-edge
% ============================================================================
\begin{frame}{Undirected multigraphs}
\vfill
\begin{theorem}
...
%  thm:leonard
% ============================================================================
\begin{frame}{B\'artfai, Bollob\'as, Leonard}
\vfill
...
% ============================================================================
%  thm:dir-arc-m2-exact
% ============================================================================
\begin{frame}{Directed arcs at $m=2$}
\vfill
\begin{theorem}[Directed arc value at $m = 2$]
For simple digraphs,
\[
...
% ============================================================================
%  thm:dir-vertex-m2-exact
% ============================================================================
\begin{frame}{Directed vertices at $m=2$}
\vfill
\begin{theorem}[Directed vertex value at $m = 2$]
...
\vfill
\begin{proposition}[Hypergraph edge bound]
...
\vfill
\begin{theorem}[Hypergraph vertex value at $m = 2$]
...
\vfill
\begin{theorem}[Hypergraph vertex value at $m = 3$]
...
% ============================================================================
%  thm:dir-arc-linear-error
% ============================================================================
\begin{frame}{The quadratic bound: directed arcs}
\vfill
\begin{theorem}[The quadratic bound with a linear error]
...
% ============================================================================
%  thm:dir-vertex-linear-error
% ============================================================================
\begin{frame}{The quadratic bound: directed vertices}
\vfill
\begin{theorem}[Directed vertex problem, leading constant and order]
...
\vfill
\begin{theorem}[Directed multigraph value, every $n$ and $m$]
...
\vfill
\begin{theorem}[The directed hypergraph constant]
...
\begin{tikzpicture}[>=Stealth,
  ...
  \draw[flow] (mea) -- (dis);
\end{tikzpicture}}{%
A single model feeds the exact max-flow checker. Certification turns that measurement into finite upper bounds, while heuristic search uses it to discover lower bounds.}

% ============================================================================
%  fig:checker-gadgets  (three subfigures side by side)
% ============================================================================
\begin{frame}{The building blocks of the connectivity checker}
\centering
...
{\scriptsize\color{black!60} Split each vertex $v$ into $v^{\mathrm{in}}$ and $v^{\mathrm{out}}$ joined by a unit-capacity arc to transform vertex-disjointness into edge-disjointness.}
\end{minipage}%
\hfill
\begin{minipage}[t]{0.32\textwidth}
\centering
...
{\scriptsize\color{black!60} Transform a hyperedge into a gate with capacity equal to the multiplicity of the hyperedge.}
\end{minipage}%
\hfill
\begin{minipage}[t]{0.32\textwidth}
\centering
...
{\scriptsize\color{black!60} The directed hyperedge becomes a gate with one tail and two heads; capacity equal to the multiplicity, and every route must enter at the tail and leave at a head.}
\end{minipage}
\end{frame}

% ============================================================================
%  sec:certification-standard  (prose)
% ============================================================================
\begin{frame}{Checking every graph of a fixed size}
\small
...
On the small cases this exhaustive mode independently reproduces every value proved by hand.
\end{frame}
...
Candidate counts for blind enumeration on a logarithmic scale. Every model crosses the generous $10^9$ budget in single-digit order, and directed objects grow fastest because they have more independent entries to decide.}
...
% ============================================================================
%  prose: what dominates the count
% ============================================================================
\begin{frame}{What dominates the count}
\small
...
\end{tikzpicture}}{%
The augmented bipartite digraph at $m = 3$, $n = 10$. Every arc runs from $A$ to $B$, and inside $B$ each vertex takes one arc from its cyclic predecessor, which is $m - 2$ arcs per vertex. A pair $a_i, b_j$ then has the direct arc and one detour, so $\lec^{\max} = 2$, and the count is $24 + 6 = 30$ against the hub branch's $27$.}
...
\vspace{2pt}
\noindent{\tiny\color{black!60} Status of the sixteen variants. Colour marks the status of a leaf: blue a closed form proved for every $n$ and $m$, purple a value proved for a range of $m$ and open beyond it, red a conjectured exact formula with a proved lower bound, and amber a proved leading term with the exact value unknown.}\par
\end{frame}
...
The eight graph-model variants at $m=3$: simple and multigraph rows, undirected and directed columns. A blue curve is a theorem, a red one a conjecture and a yellow one an interpolation, filled squares are exact finite values, and open circles are witnesses.}

\figframetall{Eight hypergraph variants at $m=3$}{variant_bounds_m3_hypergraphs.png}{%
The eight hypergraph-model variants at $m=3$: hypergraph and multihypergraph rows, undirected and directed columns, same colour and marker key. The blue curve is a proved upper bound rather than a proved value, attained only inside the stated ranges, so a square or circle below it marks a gap that is open rather than an error.}

\figframetall{Eight graph variants at $m=6$}{variant_bounds_m6_graphs.png}{%
The same eight graph-model variants at $m=6$. Raising $m$ widens every feasible band, and the exact squares thin out as the enumeration range falls behind the constructions.}

\figframetall{Eight hypergraph variants at $m=6$}{variant_bounds_m6_hypergraphs.png}{%
The same eight hypergraph-model variants at $m=6$. The blue curve is again the proved upper bound, and at this $m$ it runs visibly above what has been reached: the exact square at $n=5$ is $8$ against the curve's $10$, and at $n=6$ the curve reads $12$ where the true maximum is $11$.}
