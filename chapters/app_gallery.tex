...
The last of the five is the one worth comparing against the others. It runs the same directed multigraph problem under the \emph{vertex} separation rather than the arc one. This is not a harder constraint in the sense of a smaller feasible region: Whitney's inequality makes the vertex-feasible family the larger of the two (\Cref{rem:whitney-direction}). What the trace shows is a harder \emph{search} rather than a harder problem: the walk spends a markedly larger share of its steps on the infeasible side of the boundary before the cooling pulls it back, evidence that the two separations reshape the search landscape without the vertex one confining it more.
...
\begin{figure}[htbp]
    ...
    \caption{Search trace for the multigraph directed problem at $n = 4$, $m = 3$ under the \emph{vertex} separation, so the horizontal axis is $\kap^{\max}$ rather than $\lec^{\max}$. Compared with the arc runs above, the walk lingers much longer in the infeasible region before the cooling drives it back, evidence of a harder search rather than a harder constraint, since the vertex-feasible family is in fact the larger of the two (\Cref{rem:whitney-direction}). File \code{trace\_multi\_directed\_n4\_m3\_vertex.png}.}
