...
The densest feasible graphs the program finds are not abstractions. \Cref{fig:extremal-gallery} draws a spread of them, one extremal graph per panel across the matrix variants, each annotated with its extremal value and the size of its automorphism group $|\mathrm{Aut}|$, computed as $n!$ divided by the number of distinct labellings of that shape. A multiplicity above one is shown by a $\times k$ label rather than parallel lines, and directed arcs carry arrowheads. The shapes are exactly the named extremisers the body argues about, the tree and Mader graph for the simple undirected problem, the star at full multiplicity for the multigraph, the bidirected star for the simple directed problem, and the double star for the directed multigraph, recovered here by enumeration rather than by hand.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{figures/extremal_graphs_gallery.png}
    \caption{Machine-found extremal graphs across the matrix variants. Each
    panel is one densest feasible graph at the stated $n$ and $m$, drawn from
    the enumeration output, with its extremal value and automorphism count
    $|\mathrm{Aut}|$ below it. Edge or arc multiplicities appear as $\times k$
    labels, and a note ``$1$ of $k$'' marks a variant with several
    non-isomorphic extremisers, of which one is drawn. File
    \code{extremal\_graphs\_gallery.png}.}
