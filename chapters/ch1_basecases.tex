...
For the upper bound, a \emph{reachability skeleton} is a subdigraph preserving which vertices can reach which others. \Cref{lem:reach-skeleton} bounds its size by $M(n)=\max(2(n-1),\floor{n^2/4})$. At $m=2$, omitting any arc would leave another route between its endpoints, giving two internally vertex-disjoint paths. A feasible digraph must therefore equal its skeleton. This proves the upper bound under vertex separation and hence under arc separation (\Cref{sec:dir-arc-m2}).
