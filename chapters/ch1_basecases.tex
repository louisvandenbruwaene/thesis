...
In simple graphs the edge and vertex problems give the same answer while $m$ stays small. At $m = 2$ both count the edges of a spanning tree. At $m = 3$ the vertex ceiling $\kap^{\max} \le 2$ forces every block to be a single edge or a cycle, which \Cref{app:proofs} turns into the value in a line. The substantial case is $m = 4$, settled by Bollob\'{a}s \cite{Bollobas66}, and the value there meets the edge value of \Cref{thm:mader} from below. On the \emph{edge} side the same small cases were done first and one at a time, $m = 3$ by B\'{a}rtfai \cite{Bartfai60} and $m = 5$ by Leonard \cite{Leonard72b}, before \Cref{thm:mader} covered every $m$ at once.

\begin{theorem}[Bollob\'{a}s \cite{Bollobas66}]\label{thm:leonard}
