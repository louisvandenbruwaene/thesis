...
\begin{theorem}[Leonard \cite{Leonard73BE}]\label{thm:leonard}
...
The restriction $n \ge 10$ is the range over which S{\o}rensen and Thomassen certify that $k_5(n)$ equals the formula. The formula itself already exceeds $\ell_5(n)$ for $n = 6$ (by one), for $n = 8$ (by one), and for all $n \ge 9$: at $n = 10$ the vertex formula gives $23$ and the edge formula gives $22$. At $n = 7$ both formulas coincidentally tie at $15$. For $n \le 9$ the true $k_5(n)$ must be determined by structural analysis rather than the formula, which is why the theorem's stated domain begins at $n = 10$. One caveat on this threshold, recorded rather than resolved: an external review of a late draft reported that the current Erd\H{o}s Problems entry states the result from $n = 13$ rather than $n = 10$. I have not been able to check that against the original paper, and nothing in this thesis depends on which of the two thresholds is correct, since $k_5$ enters only as the cited first divergence. A reader who needs the exact range should consult \cite{SorensenThomassen74} directly, and should treat $n \ge 13$ as the safe reading.
