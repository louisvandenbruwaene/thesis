...
The very same Gomory--Hu tree returns for the multigraph variant (\Cref{thm:multigraph-edge}). The hypergraph bound (\Cref{prop:hyper-edge}) is proved by induction on a hyperedge cut.
...
\begin{theorem}[Hypergraph edge bound]\label{prop:hyper-edge}
...
\begin{conjecture}[The quadratic bound with a linear error, with proof,\aimedal\readfirst{2}]\label{thm:dir-arc-linear-error}
...
\begin{conjecture}[Directed multigraph value, every $n$ and $m$, with proof,\aimedal\readfirst{4}]\label{thm:dir-multi-full}
For all $n \ge 2$ and $m \ge 2$,
\[
