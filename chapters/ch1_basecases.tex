...
$\kap_G^{\max}$. For hypergraphs, this thesis uses the incidence-graph
convention: the Berge routes counted by $\kap_\HH(u,v)$ share neither an
intermediate vertex nor a hyperedge copy. Repeated copies count as distinct
hyperedges. This extra condition is needed because two Berge routes can use the
same hyperedge without sharing an intermediate vertex.
Whitney's inequality $\kap_G(u, v) \le \lec_G(u, v)$ \cite{Whitney32} records that avoiding shared vertices is at least as demanding as avoiding shared edges and \Cref{fig:edge-vs-vertex} shows the gap on a graph where two routes are edge-disjoint yet forced through one common vertex. The same inequality holds for the hypergraph convention because every family counted by $\kap_\HH$ is hyperedge-copy-disjoint.
...
In the graph and digraph vertex-disjoint variants, multiplicity of an edge or arc is irrelevant compared to its existence,
so parallel edges never raise $\kappa$.\label{sec:incidence-convention}
...
\begin{table}[ht]
\centering
\caption{Notation for the extremal functions. For hypergraphs, the third
column uses the incidence convention defined above.}
\label{tab:notation}
\begin{tabular}{l|c|c}
\toprule
