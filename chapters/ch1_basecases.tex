...
The very same Gomory--Hu tree returns for the multigraph variant (\Cref{thm:multigraph-edge}). The hypergraph bound (\Cref{prop:hyper-edge}) is proved by an elementary induction on a hyperedge cut instead.
...
share no interior node. Interior nodes come in both kinds, so separated routes
...
At $r = 2$ these two theorems speak about multigraphs. \Cref{thm:hyper-vertex-m2}
...
(\Cref{lem:multi-vertex-split}), which turns the whole question into a knapsack
over the blocks of the underlying simple graph (\Cref{thm:multi-vertex-blocks}).
In the block-packing regime, complete bipartite blocks give an asymptotic rate of
