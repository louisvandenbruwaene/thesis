...
The restriction $n \ge 13$ is the range over which S{\o}rensen and Thomassen certify that $k_5(n)$ equals the formula, as the Erd\H{o}s Problems database records it \cite{ErdosProblems}, which quotes their result directly and resolves a discrepancy an external review of a late draft had flagged against this thesis's earlier, incorrect $n \ge 10$. The formula itself already exceeds $\ell_5(n)$ for $n = 6$ (by one), for $n = 8$ (by one), and for all $n \ge 9$: at $n = 10$ the vertex formula gives $23$ and the edge formula gives $22$. At $n = 7$ both formulas coincidentally tie at $15$. Below $n = 13$ the true $k_5(n)$ needs the case-by-case structural analysis \cite{SorensenThomassen74} carries out rather than the closed form, which is why the theorem's stated domain begins there.
