...
A \emph{path} from a vertex $u$ to a vertex $v$ is an alternating sequence
$v_0,e_1,v_1,\ldots,e_q,v_q$ of distinct vertices and specified edge copies,
where $v_0=u$, $v_q=v$ and each $e_i$ joins $v_{i-1}$ to $v_i$. Thus parallel
copies give distinct one-edge paths. In a simple graph the edge copies are
determined by their endpoints, so a path may be written as a sequence of
vertices alone.
...
The same convention applies to the other maximum local connectivities below.
...
The very same Gomory--Hu tree returns for the multigraph variant (\Cref{thm:multigraph-edge}). The hypergraph bound (\Cref{prop:hyper-edge}) has a proposed proof by induction on a hyperedge cut.
Hence its properties are recorded here.
Every undirected graph has a weighted tree on the same vertex set such that $\lec_G(u, v)$ equals the lightest edge on the tree path from $u$ to $v$.
Its tree edges need not be edges of $G$, so it is not generally a spanning subgraph of $G$.
...
The final assumption that will be considered is \emph{separation}. For graphs
and digraphs, routes may be required either to avoid sharing edges, as above,
or to be internally vertex-disjoint, sharing no intermediate vertex. This gives
the local vertex-connectivity $\kap_G(u, v)$ and its maximum
$\kap_G^{\max}$.
Whitney's inequality $\kap_G(u, v) \le \lec_G(u, v)$ \cite{Whitney32} records that avoiding shared vertices is at least as demanding as avoiding shared edges and \Cref{fig:edge-vs-vertex} shows the gap on a graph where two routes are edge-disjoint yet forced through one common vertex. The same inequality holds under the convention of \Cref{sec:incidence-convention} because every family counted by $\kap$ is edge-copy-disjoint.
...
\subsection{The incidence convention}\label{sec:incidence-convention}

Every model in this thesis measures vertex separation in the same place, the
incidence graph.
The incidence graph $I(\HH)$ has one node for each vertex and one node for each
edge copy, and it joins an edge copy to each of the vertices it contains. A route
from $u$ to $v$ is a path of $I(\HH)$, and two routes are separated when they
share no interior node. For a directed graph or hypergraph, use the directed
incidence graph instead: each tail has an arc to the edge-copy node, and that
node has an arc to each head. Routes must follow these directions. Thus an arc
$u\to v$ becomes $u\to e\to v$, and a forward hyperedge permits travel from
its tail to a head, but not backwards or between two heads.
Interior nodes come in both kinds, so separated routes
share neither an intermediate vertex nor an edge copy. The second half of that is
what the hypergraph needs, since two Berge routes can run through the same
hyperedge without ever meeting at a vertex, and a hyperedge that may carry only
one route at a time is the whole difficulty of the model.
For a simple graph the definition changes nothing, because an edge lies on a
route exactly when both of its endpoints do. For a multigraph it says that $q$
parallel copies of $uv$ are $q$ separated routes, exactly as $q$ copies of a
hyperedge are, since each copy supplies its own interior edge-node and no interior original vertex. So the
two multigraph vertex variants are questions of their own, written $K_m(n)$ and
$K_m^{\mathrm{dir}}(n)$, rather than the simple problems under another name. They
are finite for the same reason the edge variants are: $m$ parallel copies are
already $m$ separated routes, so no multiplicity in a feasible network can exceed
$m-1$.
...
\begin{table}[ht]
\centering
\caption{Notation for the extremal functions. The third column uses the
incidence convention defined above, in every model.}
\label{tab:notation}
\begin{tabular}{l|c|c}
\toprule
...
The results below do not all play the same role.
The classical backbone consists of Menger's theorem, the Gomory--Hu tree, and Mader's edge bound.
It also includes the vertex-connectivity results of B\'{a}rtfai, Bollob\'{a}s, Leonard, S{\o}rensen, and Thomassen.
The main results developed in this thesis concern three different frontiers.
They are the exact directed multigraph value, the hypergraph vertex problem at $m=2$ and $m=3$, and quadratic bounds for directed graphs and hypergraphs.
The full sixteen-variant framework shows how those results fit together.
Some remaining variants are included to complete that map rather than because they carry equal conceptual weight.

\section{Bounds and conjectures with proofs}
...
\subsection{Directed graphs at the first threshold}

\begin{conjecture}[Directed arc value at $m = 2$, with proof,\conditional]\label{thm:dir-arc-m2-exact}
For simple digraphs,
\[
...
Two constructions give the lower bound, one on each branch, and \Cref{fig:simple-digraph-m2} draws both. The first is a hub joined to every other vertex in both directions, which reaches $2(n-1)$ arcs. That branch has no unique extremiser, since any tree on $n$ vertices with every edge taken in both directions reaches $2(n-1)$ arcs too, the tree offering each pair only its one path in each direction, and the bidirected star is the case of it drawn here. The second sends every arc from one half of the vertices to the other and none back, which reaches $\floor{n^2/4}$. \Cref{sec:dir-constructions} defines both and checks that neither carries a second route between any pair. The proposed upper-bound proof is in \Cref{sec:dir-arc-m2} and takes a few lines. Every digraph has a thin \emph{reachability skeleton}, a subdigraph carrying exactly the same who-reaches-whom relation, and \Cref{sec:reach-skeleton} shows one can always be found with at most $M(n)$ arcs. At $m = 2$ a feasible digraph can spare none of its arcs to that thinning, since an arc whose endpoints are still joined by a longer route inside the skeleton would give that pair two independent routes. So the digraph is its own skeleton and inherits the skeleton's count. The argument never mentions $n$, so it needs no base cases, and it never mentions the separation, so it is run for $\kap$, the larger feasible family, and the arc value follows from it.

\begin{conjecture}[Directed vertex value at $m = 2$, with proof,\conditional]\label{thm:dir-vertex-m2-exact}
...
\subsection{Undirected hypergraphs}

\begin{conjecture}[Hypergraph edge bound, with proof,\conditional]\label{prop:hyper-edge}
For $n\ge r\ge2$ and $m\ge2$, an $r$-uniform multihypergraph on $n$ vertices with $\lec^{\max}\le m-1$ has size at most $\floorfrac{(m-1)(n-1)}{r-1}$. There are simple hypergraphs attaining the bound when $m - 1 \le \binom{n-2}{r-2}$, and multihypergraphs attaining it when $(r-1) \mid n - 1$.
\end{conjecture}

The proposed proof is an elementary induction on a hyperedge cut, and \Cref{app:proofs} gives it.
...
The star hypertree of \Cref{fig:hyper-star} attains the $m = 2$ case: a hub joined to disjoint blocks of $r-1$ vertices, Berge connectivity one between vertices in its nontrivial component, $\floor{(n-1)/(r-1)}$ hyperedges, exactly as a tree attained the multigraph bound.
For $r \ge 3$ the conjecture asserts attainment by \emph{simple} hypergraphs, with no repeated hyperedge, whenever $m-1\le\binom{n-2}{r-2}$ (\Cref{thm:simple-hyper-edge}).
That is a real difference between the two edge sizes.
For ordinary graphs simplicity is what separates Mader's $\floor{m(n-1)/2}$ from the multigraph value $(m-1)(n-1)$. One edge size higher the conjecture predicts equality while $m-1\le\binom{n-2}{r-2}$. That condition says there are enough distinct hyperedges to go around. When it fails the two may part company. At $n = r = m = 3$ there is only one hyperedge to choose from, so a simple hypergraph carries one where the multihypergraph carries two copies of it (\Cref{rem:simple-hyper-necessary}).
...
\begin{conjecture}[Hypergraph vertex value at $m = 3$, with proof,\conditional]\label{thm:hyper-vertex-m3}
...
The $m = 3$ case is the harder one and depends on an unchecked rank bound argued by induction over blocks and separating pairs.
...
At $r=2$ these statements concern multigraphs. \Cref{thm:hyper-vertex-m2}
gives $K_2(n) = n-1$, where the cap leaves every multiplicity at one and the
extremisers are the spanning trees, and \Cref{thm:hyper-vertex-m3} conjectures
$K_3(n) = 2(n-1)$, attained by a spanning tree with every edge doubled
(\Cref{cor:multi-vertex-m3}). The first threshold at which the multigraph vertex
value is known to differ from its edge counterpart is $m = 5$, where
$K_5(4) = 14$ against $L_5(4) = 12$. Whether they agree at $m = 4$ is open. From
$m = 5$ the value also parts company with the thickened tree, which a single
grafted triangle already beats
(\Cref{thm:clique-chain-vertex}). \Cref{app:proofs} works the problem out. The
measure splits into parallel copies plus longer detours
(\Cref{lem:multi-vertex-split}), which is used in a proposed reduction to a knapsack
over the blocks of the underlying simple graph (\Cref{thm:multi-vertex-blocks}).
In the block-packing regime, the proposed analysis of complete bipartite blocks gives an asymptotic rate of
$(3-2\sqrt{2}+o_m(1))m^2$ per added vertex (\Cref{thm:multi-vertex-bipartite}),
while the underlying simple graph gives the upper bound $K_m(n) < 2(m-1)^2n$
(\Cref{prop:multi-vertex-upper}).

\subsection{Directed bounds beyond the first threshold}

\begin{conjecture}[The quadratic bound with a linear error, with proof,\aimedal\readfirst{6}]\label{thm:dir-arc-linear-error}
...
\]
and, for $m\ge3$, both the leading constant $\tfrac14$ and the \emph{order} of the second-order term of the augmented bipartite count hold unconditionally. At $m=2$, \Cref{thm:dir-arc-m2-exact} instead gives $\ell_2^{\mathrm{dir}}(n)=n^2/4+O(1)$ as $n\to\infty$.
\end{conjecture}
...
\begin{conjecture}[Directed vertex problem, leading constant and order, with proof,\aimedal]\label{thm:dir-vertex-linear-error}
...
\begin{conjecture}[Directed multigraph value, every $n$ and $m$, with proof,\aimedal\readfirst{8}]\label{thm:dir-multi-full}
For all $n \ge 2$ and $m \ge 2$,
\[
...
Under the conjecture, extremisers include the thickened bidirected spanning trees on the linear branch, every edge taken in both directions at multiplicity $m-1$, and the balanced one-way bipartite multigraph at the same multiplicity on the quadratic one. On the linear branch these are constructions exhibited rather than a classification, and \Cref{app:proofs} says explicitly that not every linear-branch extremiser has been classified. The proposed proof deletes a \emph{subgraph} at a time rather than a vertex. Inside any feasible multigraph there is a sparse skeleton that alone preserves which vertices reach which, and removing one copy of each of its arcs costs every reachable pair at least one route. Peeling off $m-1$ such skeletons therefore accounts for every arc, and the parity obstruction that stopped the vertex-deletion induction never arises. The proposed equality argument would also classify the quadratic extremisers once $n\ge\max(8,2m)$ (\Cref{thm:dir-multi-uniqueness}), with classification at smaller orders left open even if that argument is verified.

\begin{conjecture}[The directed hypergraph constant, with proof,\aimedal]\label{thm:dir-hyper-constant}
...
edge- and the vertex-disjoint multihypergraph maxima are
$\bigl(1+o(1)\bigr)(m-1)n^2/\bigl(4(r-1)\bigr)$, and the bipartite construction
of \Cref{prop:dir-hyper-first} is asymptotically optimal for both. For simple directed hypergraphs the same conclusion holds when $r\ge3$. The case $r=2$ is the simple digraph problem, with leading term $n^2/4$.
\end{conjecture}
...
The same specialisation to $r = 2$ carries the directed arc results across to the
directed multigraph vertex problem. At $m = 2$ the cap leaves every multiplicity
at one, so $K_2^{\mathrm{dir}}(n) = k_2^{\mathrm{dir}}(n) = \ell_2^{\mathrm{dir}}(n)$.
For fixed $m$ the arc extremiser of \Cref{thm:dir-multi-full} is feasible here as
well, since $\kap \le \lec$, and the conjecture above at $r = 2$ supplies the proposed matching
upper bound, so $K_m^{\mathrm{dir}}(n) = (m-1)n^2/4 + O_m(n)$
(\Cref{cor:dir-multi-incidence}).

For the simple directed arc problem, a matching upper bound for the constructions of \Cref{const:directed-comparison} remains open here when $m\ge3$. The literature bound of Huang and Lyu, together with the displayed construction, independently establishes that the value is $n^2/4+\Theta_m(n)$. Determining the linear contribution, and then the bounded correction if such an expansion holds, would require sharper bounds. That coefficient is not out of reach either: a feasible digraph has at most $m-1$ two-step routes between any ordered pair, so it contains no $m$ such routes with common endpoints, and the Tur\'an theorem of Huang and Lyu for that family replaces the coefficient $4(m-1)$ above by $m-1$ once $n$ is large in terms of $m$ \cite{HuangLyu26}.
