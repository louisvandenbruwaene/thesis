...
This chapter gives the foundation for the question and its variants and goes over the results that can be established by proof.
...
A theorem of Menger \cite{Menger27}, recalled in \Cref{app:proofs}, gives an equivalent cut description: $\lec_G(u, v)$ also equals the least number of edges one must remove to cut every route from $u$ to $v$ (\Cref{fig:menger}).
...
Mader proved that no graph under the cap has more than $\floor{m(n-1)/2}$ edges (the upper bound) in the simple-graph case. The shared-clique construction of Bollob\'{a}s and Erd\H{o}s \cite{BollobasErdos62}, copies of $K_m$ sharing one vertex, achieves the (lower) bound but is only defined when $m-1$ divides $n-1$.
...
The inequality does not require $n \ge m$, but attaining equality generally does. For $2\le n<m$, the complete graph is feasible and gives the exact value $\ell_m(n)=\binom{n}{2}$. This is strictly below $\floor{m(n-1)/2}$ except at $(n,m)=(2,3)$, where $K_2$ attains $\floor{3/2}=1$.
...
The very same Gomory--Hu tree returns for the multigraph variant (\Cref{thm:multigraph-edge}), so its properties are recorded here.
...
The hypergraph bound (\Cref{prop:hyper-edge}) uses no tree at all and is proved by induction on a hyperedge cut.

\section{Variants of the problem}

Changing the assumptions gives eight network models, each with two notions of route separation.

The first is \emph{arity}: an edge joins exactly two vertices and a \emph{hyperedge} joins $r \ge 2$ vertices.
In an $r$-uniform hypergraph, every hyperedge contains exactly $r$ vertices. A \emph{Berge path} alternates distinct vertices and distinct hyperedge copies, with each hyperedge containing the vertices immediately before and after it.
...
These three choices give the eight models shown in \Cref{fig:eight-models}.
...
\begin{tikzpicture}[>=Stealth,
  ...
  \begin{scope}[shift={(8.6,-4.5)}]
    ...
    \node[font=\scriptsize, text=black!65] at (0,-2.25) {overlapping hyperedges};
  \end{scope}
...
For a simple graph the definition changes nothing: two internally vertex-disjoint
$u$--$v$ paths can share an edge only if that edge is $uv$ itself, and the single
copy of $uv$ carries at most one of them. For a multigraph it says that $q$
...
\section{Bounds and established results}

\subsection{Undirected graphs}

\begin{theorem}\label{thm:multigraph-edge}
For $n\ge1$ and $m\ge2$, undirected multigraphs satisfy $L_m(n) = (m-1)(n-1)$.
\end{theorem}
...
The two constructions in \Cref{fig:simple-digraph-m2} attain the two terms: a bidirected star has $2(n-1)$ arcs, and a balanced complete bipartite digraph with all arcs in one direction has $\floor{n^2/4}$. Each is feasible because every ordered pair has at most one directed path. Any bidirected tree attains the first count; the star is the illustrated example (\Cref{sec:dir-constructions}).

For the upper bound, a \emph{reachability skeleton} is a subdigraph preserving which vertices can reach which others. \Cref{lem:reach-skeleton} bounds its size by $M(n)=\max(2(n-1),\floor{n^2/4})$. At $m=2$, an arc outside the skeleton would leave a second route between its endpoints, and the two routes would be internally vertex-disjoint. A feasible digraph must therefore equal its skeleton. This proves the upper bound under vertex separation and hence under arc separation (\Cref{sec:dir-arc-m2}).
...
The hub leads for $n < 7$ and the one-way complete bipartite digraph leads for $n > 7$.
...
For $r \ge 2$ and $n\ge1$, every $r$-uniform multihypergraph on $n$ vertices with
...
Both proofs use the incidence graph (\Cref{sec:hyper-vertex-proof}). At $m=3$, the main step bounds its cycle rank by decomposing it at cut vertices and separating pairs. No corresponding bound for $m\ge4$ is established here.

To measure hyperedge-disjoint routes, the checker must allow each hyperedge copy to serve at most one route, even when routes enter it at different vertices. Chapter~2 enforces this with a capacity gate for each hyperedge (\Cref{fig:hyper-gadget}).
...
extremisers are the spanning trees, and \Cref{thm:hyper-vertex-m3} gives
...
$m = 5$ the value also parts company with the thickened tree, which the proposed construction with an attached triangle would beat
...
while a further proposed argument using the underlying simple graph gives the upper bound $K_m(n) < 2(m-1)^2n$
...
The proof is in \Cref{app:proofs}. Its engine is a count of two-step routes. The routes $u \to x \to v$ through distinct midpoints $x$, together with the arc $u \to v$, are disjoint, so a feasible digraph has at most $m-1$ of them for each ordered pair. Summed over all pairs, this caps $\sum_x d^+(x)\,d^-(x)$. Deleting at every vertex the smaller of its in-arcs and out-arcs then leaves a one-directional bipartite digraph, with at most $\floor{n^2/4}$ arcs, and the deleted arcs are the linear correction.
...
well, since $\kap \le \lec$, and \Cref{thm:dir-hyper-constant} at $r = 2$ supplies the matching
upper bound, so $K_m^{\mathrm{dir}}(n) = (m-1)n^2/4 + O_m(n)$
(\Cref{cor:dir-multi-incidence}).

For the simple directed arc problem, a matching upper bound for the constructions of \Cref{const:directed-comparison} remains open here when $m\ge3$. A feasible digraph has at most $m-1$ two-step routes between any ordered pair. Huang and Lyu's Tur\'an theorem therefore replaces the upper coefficient $4(m-1)$ by $m-1$ for sufficiently large $n$ \cite{HuangLyu26}. Together with the augmented bipartite construction, this independently gives $n^2/4+\Theta_m(n)$.
