...
hyperedge are, since a single edge is a path with nothing in its interior. So the
...
For ordinary graphs simplicity is what separates Mader's $\floor{m(n-1)/2}$ from the multigraph value $(m-1)(n-1)$. One edge size higher the two are equal, but only while $m-1\le\binom{n-2}{r-2}$. That condition says there are enough distinct hyperedges to go around. When it fails the two part company. At $n = r = m = 3$ there is only one hyperedge to choose from, so a simple hypergraph carries one where the multihypergraph carries two copies of it (\Cref{rem:simple-hyper-necessary}).
...
(\Cref{cor:multi-vertex-m3}). From $m = 5$ the multigraph vertex problem parts
company both with its edge counterpart, $K_5(4) = 14$ against $L_5(4) = 12$, and
with the thickened tree, which a single grafted triangle already beats
