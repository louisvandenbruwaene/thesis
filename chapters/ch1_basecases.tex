...
For simple graphs and all $n \ge 13$,
\[
    k_5(n) = \floorfrac{8n}{3} - 3 \;>\; \floorfrac{5(n-1)}{2} = \ell_5(n).
\]
\end{theorem}

The restriction $n \ge 13$ is the range over which S{\o}rensen and Thomassen certify that $k_5(n)$ equals the formula, as the Erd\H{o}s Problems database records it \cite{ErdosProblems}. Below it the true $k_5(n)$ needs the case-by-case structural analysis \cite{SorensenThomassen74} carries out rather than the closed form, which is why the stated domain begins there.

\begin{remark}[A counting convention that shifts every quoted constant by one]\label{rem:threshold-convention}
The additive constant above deserves a warning, because two conventions are in circulation and they differ by exactly one. This thesis defines $\ell_m(n)$ and $k_m(n)$ as the \emph{largest} number of edges a graph can carry while \emph{no} pair is joined by $m$ independent routes. The Erd\H{o}s Problems database \cite{ErdosProblems} states the same results the other way round, as the \emph{smallest} number of edges that \emph{forces} such a pair, which is one more.

The shift is visible in every entry that both sources state. The database gives $k_2(n) = n$ where a tree has $n-1$ edges, $k_3(2n) = 3n-1$ against $\floor{3(2n-1)/2} = 3n-2$ here, $k_4(n) = 2n-1$ against $2n-2$, $\ell_5(2n) = 5n-2$ against $5n-3$, $\ell_6(n) = 3n-2$ against $3n-3$, and it writes the Bollob\'{a}s-Erd\H{o}s conjecture as $1 + \binom{m}{2}n$ where the convention here gives $\binom{m}{2}n$. Every one of those is exactly one apart, and the values in this thesis are the ones an exhaustive search returns: $k_3(4) = 4$, $k_3(5) = 6$ and $k_4(5) = 8$ were each confirmed by walking every graph of that size.

Applied to \Cref{thm:sorensen-thomassen} the same shift would read $\floor{8n/3} - 4$ rather than the $\floor{8n/3} - 3$ printed above, which is the database's figure carried over unadjusted. Both candidates respect $k_5 \ge \ell_5$, which Whitney's inequality forces, so the thesis's own arithmetic cannot decide between them, and the original paper is the only thing that can.

One consistency point is worth recording, because it constrains how the two figures may be mixed. The threshold $n \ge 13$ was taken from the same database entry as the constant, and the two must be converted together or not at all. Left as printed, $-3$ with $n \ge 13$, the strict inequality of \Cref{thm:sorensen-thomassen} holds throughout, since $k_5(13) = 31$ against $\ell_5(13) = \floor{5 \cdot 12/2} = 30$. Converting only the constant to $-4$ while leaving the threshold at $n \ge 13$ fails at that same first size, since $\floor{8 \cdot 13/3} - 4 = 30 = \ell_5(13)$, so the divergence would begin only at $n \ge 14$ under that reading. The two admissible readings are therefore $(-3,\ n \ge 13)$ and $(-4,\ n \ge 14)$, and mixing a converted constant with an unconverted threshold is the one combination that is certainly wrong. Nothing here depends on which is right: $k_5$ enters only as the cited first divergence between the edge and vertex problems, and that divergence holds under either constant. A reader who needs the exact value should take it from \cite{SorensenThomassen74} directly and should note which convention that paper uses before comparing it with anything above.
\end{remark}
