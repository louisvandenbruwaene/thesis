...
For small $n$ the answer is not conjectured at all. The pruned exhaustion inside \code{solve} walks every simple digraph on $n$ vertices and returns the exact maximum, complete and certain, an upper bound with the standing of a hand proof. At $m = 3$ it \emph{proves} $6, 9, 12, 15$ for $n = 3, 4, 5, 6$ under both separations alike (\Cref{rem:dir-vertex-m3}), and there it stops within a reasonable budget, the pruned walk no longer closing $n = 7$ in tractable time. That is the proved region, finite and closed. It is proved rather than certified in the sense fixed above: the upper bound comes from an exhaustion, not from the cut-counting optimisation, which for this variant contributes nothing.
