...
A hypergraph is not a matrix at all, so it is enumerated over its own list: the candidates are the $\binom{n}{r}$ vertex sets of size $r$, or the oriented splits of them in the directed model, and a hypergraph is a choice of how many copies of each the list holds, at most one for a simple hypergraph and up to $m-1$ for a multihypergraph.
\Cref{fig:matrix-rep} shows both encodings.
Walking those possibilities, measuring each candidate with the checker, and keeping the densest feasible one settles a bound exactly.
If the enumeration hits its deadline before finishing, the returned graph is labelled as a lower bound.
On the small cases this exhaustive mode independently reproduces every value proved by hand in \Cref{ch:basecases} and those values are not repeated here.
...
\Cref{sec:search-wall} measures exactly how fast the search space grows. 
...
Historical runs gave $M^{*}(n)=2(n-1)$ with status \texttt{OPTIMAL} for $3\le n\le6$, and $n=7$ did not close. Their archived output lacks a separately checkable bound or gap, and no theorem now depends on it: \Cref{app:proofs} proves the value for all $n,m$ by hand and \Cref{sec:transcripts} retains the solver record.

% Former source boundary: chapters/ch3_discover.tex
Certification is exact only while it can cover every candidate. The second half of this chapter measures where that ends and replaces the exhaustive walk by a selective one: simulated annealing or tabu search explores a few matrix-model graphs well, while a randomised greedy routine handles hypergraphs. Each uses the same checker to keep the best feasible construction. Its output is a concrete object and therefore a lower bound, never an upper bound.
...
The paired grids of \Cref{fig:variant-bounds-m3,fig:variant-bounds-m6} apply that reading to all sixteen variants. Blue curves are theorems, red curves conjectures, and yellow curves interpolate lower bounds, while filled squares are exact finite values and open circles are witnesses. Directed vertex values were checked separately because Whitney's inequality transfers witnesses but not upper bounds.
...
% Each grid holds sixteen plot panels. Side by side at half the text width their
% axis labels printed at under 2pt, so each now takes a full sideways page,
% which is the only layout on A4 that leaves them legible. They are declared
% after the closing paragraph so the running text fills its own page first.
\begin{sidewaysfigure}[p]
    \centering
    \includegraphics[width=0.98\textheight]{figures/variant_bounds_m3.png}
    \caption{All sixteen variants at $m=3$. Rows select the model and columns the direction and separation. Colours encode theorem, conjecture, or interpolation, filled squares are exact finite values, and open circles are witnesses. The dotted lines in the simple directed arc panel are its two competing branches, the hub and the bipartite wall. \Cref{fig:variant-bounds-m6} is the same grid at $m=6$.}
    \label{fig:variant-bounds-m3}
\end{sidewaysfigure}

\begin{sidewaysfigure}[p]
    \centering
    \includegraphics[width=0.98\textheight]{figures/variant_bounds_m6.png}
    \caption{The same sixteen variants at $m=6$, to be read against \Cref{fig:variant-bounds-m3}. Raising $m$ widens every feasible band, and the exact squares thin out as the enumeration range falls behind the constructions.}
