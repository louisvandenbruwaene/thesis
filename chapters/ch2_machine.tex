...
Pattern recognition on the discovered values, or even a family of extremal graphs, then lets conjectures be made, although those conjectures are supported only by small graphs.
Larger instances use tabu search for graphs and random greedy growth for hypergraphs.
Random edge assignments do not enforce the connectivity cap. Guided search instead uses the checker to find dense feasible examples.
...
\Cref{fig:spine} diagrams the program's architecture, showing how the modelled graphs are fed to the checker and how the results are used to either certify or discover bounds.
\begin{figure}[ht]
\centering
\spinediagram
\caption{A single model feeds the exact max-flow checker. Certification turns that measurement into finite upper bounds, while heuristic search uses it to discover lower bounds.}
\label{fig:spine}
\end{figure}
...
Hyperedge lists store the edges that are present without allocating an entry for every possible hyperedge.
The enumeration considers graphs of a fixed order and maximises their edge count subject to the connectivity constraint.
...
Hypergraphs are implemented similarly from the ground up, over their lists of hyperedges rather than a matrix.
Depth-first search stores the current branch rather than all candidates at once.
...
This ensures that every variant shares the same integer max-flow engine.
Testing one pair reduces to a flow computation:
...
Menger's theorem (\Cref{thm:menger}) already turned $\lambda(u,v)$ into the number of edges or arcs in a minimum cut.
For a simple graph, $\lambda(u,v)$ is therefore the minimum cut capacity when every edge has unit capacity.
...
Each member vertex then gets an arc of unlimited capacity to the entry point and one back from the exit point,
so the single arc between them is the only bottleneck (\Cref{fig:hyper-gadget}).
A directed hyperedge is wired the same way, except that only its tail reaches the entry and only its heads leave the exit,
which is what forces a route to cross it from tail to head (\Cref{fig:dir-hyper-gadget}).
...
The two representations have the same maximum flow: a family of routes through
separate copies maps to a flow through the merged gates. Conversely, decompose
an integral flow through the merged network into paths. At most $q$ paths use
a gate of capacity $q$, so they can be assigned distinct copies of that
hyperedge, each with the same permitted entrances and exits.

The hypergraph Menger argument (\Cref{thm:menger-hyper}) explains that the resulting maximum flow is exactly the Berge connectivity.
...
The split network measures $\kap_G(u,v)$ for the supplied graph and endpoints.
Maximising over endpoint pairs tests feasibility, and maximising the edge count
over feasible graphs gives $K_m(n)$ or $K_m^{\mathrm{dir}}(n)$.
Notice how the undirected or hypergraph transformation is done beforehand and
the gates are not split by this step.
...
\section{Checking every graph of a fixed order}\label{sec:certification-standard}
...
A hypergraph is stored as a list rather than a matrix, so it is enumerated over that list: a subset of all $\binom{n}{r}$ possible vertex sets of size $r$.
...
Completed enumerations establish the small cases reported in the audit. For directed multigraphs under arc separation, \code{solve(exhaustive=True)} instead evaluates the established formula and returns a named witness, explicitly labelled as a lower bound. This branch supplies no enumeration markers in the figures. The specialised structural enumerators use that same formula for prefix pruning. Its proved bound justifies those prunings, but the enumerations cannot serve as independent proofs of the formula they assume.
...
These variants are particularly costly to enumerate, even where their leading term is established.
...
The enumeration is exact wherever it finishes and it does so only for the smallest orders.
...
\Cref{thm:dir-vertex-m2-exact,thm:dir-arc-m2-exact} give all-order formulas whose upper-bound proofs rest on the reachability skeleton. The sweep establishes only the listed finite cases.
...
\Cref{tab:generating-benchmark} records timings for three different single-core tasks. The distinction matters: they are not three equivalent ways of discovering an unknown optimum.
...
The \texttt{geng} pipeline is given the already known extremal arc count and lists the feasible isomorphism classes at that target, which does not rule out a larger feasible count.

The blind sweep is omitted after $n=4$, where $2^{n(n-1)}$ assignments already take $6.3$\,s.
Each additional vertex multiplies the assignment count by $2^{2n}$, and the
connectivity check on one assignment grows with it, the recorded step from
$n=3$ to $n=4$ costing about twice its count ratio.
Keeping the per-assignment cost fixed gives about $27$ minutes at $n=5$
and $19$ days at $n=6$. If that cost doubles at each step, the estimates
become about $54$ minutes and $76$ days respectively. These are extrapolations,
...
The program computes $M^{*}(n)$ at small $n$ as a mixed-integer linear program (MILP), solved by CBC through PuLP.
It has one weight variable $w(u,v)\in[0,1]$ per ordered pair.
For every ordered pair $(s,t)$ the model also chooses a cut, through a binary side variable for every other vertex with $s$ fixed on the source side and $t$ on the sink side, and caps the total weight crossing that cut at one.
Because the solver may choose the cut, the constraint holds exactly when some $s$--$t$ cut has capacity at most one, which by \Cref{thm:menger} means $\mathrm{maxflow}(s,t;w)\le1$.
The model is therefore exact rather than a relaxation.
Optional valid inequalities shorten the solve without moving the optimum. The main ones say that the direct arc and the two-step detours of each pair carry total weight at most one, and a degree ordering removes relabelled copies.
The same model with integer multiplicities in $\{0,\dots,m-1\}$ and cap $m-1$ decides whether a given arc count is feasible.
...
On the linear branch, \Cref{const:dir-multi-lower} supplies the bidirected star.
It has one hub joined bidirectionally to every other vertex and has $2(n-1)$ arcs.
For $n\le6$, this is the larger term of the established value $M(n)$.
The MILP returns the same value $M^{*}(n)=2(n-1)$ at $3\le n\le6$. The solver reports these optima as proved, but it emits no independently replayable certificate of the matching bound (\Cref{sec:transcripts}).
...
Thus the construction attaining $M(n)$ changes at the crossover.

The general statement is integrality.
\Cref{cor:mstar-integral} states that some $\{0,1\}$ matrix always attains $M^{*}(n)$, and that this optimum equals $M(n)$.
An explicit construction gives attainment from below.
Clearing denominators from a rational optimum reduces the reverse inequality to \Cref{thm:dir-multi-full}.
...
The MILP values for $3\le n\le6$ remain as supporting evidence, and \Cref{sec:transcripts} records the runs.
\[
...
So far, iterating all graphs of a fixed order that meet the connectivity constraint and counting their edges was a blind walk.
...
In a comparison with six seconds per method and case, recorded in \path{program/logs/tabu_vs_sa_log.txt}, annealing never finds more arcs than tabu.
For $n=7$ and $m=3$, tabu reaches the $24$-arc directed multigraph construction, while annealing reaches $20$ arcs.
For the simple digraph, tabu reaches $17$ arcs and annealing reaches $16$, both below the $18$-arc construction.
...
where agreement provides an additional check on the search implementations.

Every feasible graph can be reduced to the empty graph by deleting edges, and the reverse sequence stays feasible. Thus temporary infeasibility is not necessary to reach a target. It can nevertheless help the search: exchanging edges may require either a removal that lowers the edge count or an addition that temporarily exceeds the cap. Tabu search permits both while retaining only feasible results.

Several independent restarts run until the time budget expires, and the densest feasible witness is retained. Restarts share no state and can run on separate processor cores.
...
The tabu engine changes entries of a multiplicity matrix, so hypergraphs are
searched by random greedy growth instead. A full tabu neighbourhood scan there
would consider $\binom{n}{r}$ hyperedge types in an undirected simple
hypergraph and $r\binom{n}{r}$ in the forward directed model, with feasibility
measured in the expanded gate network. The equal-allocation experiment below
compares the implemented searches.
...
\paragraph{An optional compiled helper.}
The steps that do call the checker run inside the innermost loop, so the
program can load a small C helper for them, \code{\_erdos\_fast.so}, built from
the \code{\_erdos\_fast.c} shipped with the program. It supplies three routines:
the capped max-flow and the canonical form used for isomorphism reduction, both
taken at $n\le7$, and the pairwise excess predicate, taken at $n\le16$. Each
call is guarded. The flow routines are used only when the capacities and the
capped flow value fit in a signed $32$-bit integer, and the canonical key only
at that same width, because the key is a byte encoding and a wider one would
order the relabellings differently. When a guard fails, or when the helper was
never built, the program runs the pure-Python routine instead. The two paths
return the same values on every input either of them accepts, so the helper
changes how long a search runs and never what it reports.
...
In all seven validation cases in \Cref{tab:rediscovery}, it reaches the established value. This is a statement about those runs, not a claim that it succeeds at every order or throughout the sixteen-variant grids.
...
\caption{Examples from the extremal families discussed alongside \Cref{tab:rediscovery}, drawn at the reported sizes. The path illustrates the tree family but is not the saved search witness. \Cref{fig:simple-digraph-m2} shows a bidirected star, which ties the one-way complete bipartite digraph at $n=7$.}
\label{fig:rediscovery-gallery}
\end{figure}

The recorded witnesses can be inspected, rather than inferring their shapes from their edge counts. They are saved with methods and elapsed times in \code{program/data/rediscovery.json}. \code{program/scripts/rediscovery.py} runs the experiment. Each later restart uses the next integer seed. The supplied optimum is not passed to the search. All seven saved witnesses have star-shaped support, with the direction and multiplicity specified below.
...
At $n=7$ it finds twelve arcs, where the hub and the one-way complete bipartite digraph tie (\Cref{fig:simple-digraph-m2}).
...
At $n=8$ and $m=2$, a bidirected star has fourteen arcs, whereas a balanced one-way complete bipartite digraph has sixteen.
Moving from a completed hub to the one-way complete bipartite digraph requires many coordinated arc changes (\Cref{fig:simple-digraph-m2}).
Single-arc moves can therefore make the star a difficult state to escape.
...
The bidirected star family illustrated in \Cref{fig:rediscovery-gallery} also supplies witnesses at larger orders and multiplicities.
...
the three seeds instead of replacing a search value by $C$. That section also
records the raw witnesses and the measured search time against the
allocation.

\begin{table}[htbp]
\centering
\scriptsize
\input{figures/equal_budget_summary.tex}
...
small validation cases above do not imply reliable recovery of the dense
