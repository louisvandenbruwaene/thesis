...
The $m=6$, $n=6$, $r=3$ hypergraph exception in \Cref{fig:variant-bounds-m6-hyper} is settled by exhaustion: all $125\,970$ twelve-hyperedge simple $3$-uniform hypergraphs on six vertices are infeasible, so the curve's $12$ overstates the true maximum of $11$. The smallest instance of the same gap is $n=r=m=3$, where a simple hypergraph carries one hyperedge and a multihypergraph carries two.

\Cref{tab:variant-values} gives the same sixteen variants at both $m$ as numbers, colour-matched to the four grids above, for a reader who wants a value rather than a position on a curve.
