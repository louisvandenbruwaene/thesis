...
The historical solver runs reported the same value for $M^{*}(n)$ at $3\le n\le6$, without an independently replayable matching bound (\Cref{sec:transcripts}).
...
The historical finite optimisation is retained as supporting evidence for $3\le n\le6$, and \Cref{sec:transcripts} records what it ran.
\[
...
records the raw witnesses, the runs excluded for a clock discrepancy together
with their timing replacements, and the measured search time against the
allocation.
