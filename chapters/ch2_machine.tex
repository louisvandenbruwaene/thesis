...
Completed enumerations corroborate the small cases reported in the audit. There is one important exception to the driver's terminology: for directed multigraphs under arc separation, \code{solve(exhaustive=True)} returns the proved closed form and a named witness. That branch is a theorem evaluation, not an independent exhaustive check. It therefore supplies no enumeration markers in the figures. The standalone formula audit calls the enumerator directly when checking that theorem.
...
These variants are particularly costly to enumerate, even where a theorem already determines their leading term.
...
statistics. Each flagged trial is repeated with the same frozen search code
...
The replacement runs and final timing audit are still in progress in this
working version. Their tables are not presented as complete results.
}
