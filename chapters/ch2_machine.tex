...
The formula in \Cref{thm:dir-multi-full} now supplies a lower-bound witness in \code{solve}. The cut formulation can provide a separate finite check, provided its optional cuts are valid independently of that theorem.
...
Solver runs reported the same value for $M^{*}(n)$ at $3\le n\le6$, without an independently replayable matching bound (\Cref{sec:transcripts}).
...
The finite optimisation is retained as supporting evidence for $3\le n\le6$, and \Cref{sec:transcripts} records what it ran.
\[
...
In the reported comparison with equal time budgets, annealing finds fewer arcs.
