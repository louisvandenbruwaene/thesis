...
The last chapter ended with several variants stuck for different reasons, but all ask one machine to do the same three jobs. It must represent every model, measure connectivity exactly, and then use that measurement either to \emph{prove} a finite optimum by exhaustive certification or to \emph{discover} a dense feasible graph. This chapter builds that single pipeline in that order. The epistemic distinction matters more than the shared implementation: measurement is exact, completed enumeration proves, and heuristic search supplies only a lower bound. \Cref{fig:spine} is the map. The small cards below name each routine and its interface. The function bodies stay in the archive because the argument needs the contract, not the code listing. That archive is the repository at \url{https://github.com/louisvandenbruwaene/thesis}, and every file path quoted in this thesis is relative to it, with the commands and the audit trail collected in \Cref{sec:transcripts}.
...
{picks the method the case allows. Proving a matrix model runs the pruned branch and bound for simple digraphs, the closed form of \Cref{thm:dir-multi-full} for the directed multigraph arc value, and brute-force enumeration otherwise. Proving a hypergraph runs its own enumeration. Discovering runs tabu search or, with \code{method="sa"}, simulated annealing, again with a separate routine for hypergraphs. The cut-counting certifier is \emph{not} reached from here: it is called on its own as \code{prove\_directed\_multigraph}. The budget is checked between iterations, so a single iteration may overrun it.}
...
certifier, and search routines serve every variant, and only the capacity reduction
...
\end{figure}
Self-tests recover $n-1$ on $K_n$ and $3$ on $K_4^{(3)}$. Splitting the ordinary vertices as in \Cref{fig:vertex-split} gives the vertex-disjoint hypergraph checker, and only direction still has to be added.
...
hyperedges therefore use the same capacity-one gate, and only the incident wiring
changes.
...
\node[hnode, fill=metroA!35, minimum size=15pt] (em) at (2.05,  0.62) {$e^-$};
...
% the one bottleneck
\draw[gdir, line width=1.6pt, color=KULblauw3b] (em) -- node[right, font=\scriptsize]{cap $1$} (ep);
\end{scope}
\end{tikzpicture}}
...
\begin{scope}[xshift=5.4cm]
  ...
  \node[hnode, fill=metroB!35, minimum size=15pt] (gm) at (0.0,  0.42) {$e^-$};
  \node[hnode, fill=metroB!70, minimum size=15pt] (gp) at (0.0, -0.42) {$e^+$};
  ...
  \draw[gdir, line width=1.6pt, color=KULblauw3b] (gm) -- node[right, font=\scriptsize]{cap $1$} (gp);
  \draw[gdirfaint] (gp) -- node[left, font=\scriptsize, text=black!60]{$\infty$} (b2);
  \draw[gdirfaint] (gp) -- node[right, font=\scriptsize, text=black!60]{$\infty$} (c2);
\end{scope}
\end{tikzpicture}}
\caption{Orient the hyperedge gate.}
\label{fig:dir-hyper-gadget}
\end{subfigure}
\caption{The three capacity-one reductions used by the checker. Each forces competing routes through \emph{one} unit-capacity bottleneck, drawn in dark blue: the internal arc $e^- \to e^+$ of the gate, or $v^{\mathrm{in}} \to v^{\mathrm{out}}$ for a split vertex. The membership arcs joining the gate to the hyperedge's vertices are uncapped, marked $\infty$. Direction changes only which of them are drawn. Undirected, every member links to the gate both ways. Directed, tails enter $e^-$ and heads leave $e^+$.}
...
The gate is indifferent to how a hyperedge's vertices are split between tails and heads. It draws one uncapped arc in from every tail and one uncapped arc out to every head, and the single unit of capacity sits where it always sits, on the internal arc $e^- \to e^+$. That placement is what makes the count right, and it is worth saying why the obvious alternative does not. Capping each tail's own arc at one instead would let a hyperedge with several tails carry one route per tail, so a single Berge edge would be usable several times over and the flow would overcount the disjoint routes. One shared bottleneck, entered from any tail and left toward any head, is used at most once no matter how many arcs feed it. The single forward tail the gate was built for is therefore only the simplest case: give it a hyperedge with several tails and one head, or several of each, and it still counts the routes that enter at a tail and leave at a head. The same checker therefore measures every \emph{orientation model} of a directed Berge edge, not just the one-tail one, and \Cref{sec:orientation-models} takes up which of those models are worth distinguishing and what the program finds for them.
...
proved by hand in \Cref{ch:basecases}, and those values are not repeated here.
...
{builds the cut-counting MILP in \code{pulp}, runs CBC or Gurobi, and records the status and primal matrix, while \code{prove\_integral\_arc\_bound} supplies a separate integer cross-check.}
...
The classical method for this kind of landscape is \emph{simulated annealing} (SA), introduced as a combinatorial optimisation method by Kirkpatrick, Gelatt and Vecchi \cite{KirkpatrickGelattVecchi83} and named after the metallurgical process of cooling a heated material slowly so that its atoms settle into a low-energy crystal rather than a disordered glass. Applied to graphs, annealing escapes local optima by sometimes accepting a worsening move, and by accepting fewer of them as the run proceeds. Each graph $G$ is a state with energy
\[
    E(G) = -|E(G)| + \Lambda \cdot \max\bigl(0,\ \lambda^{\max}(G) - (m - 1)\bigr),
\]
which rewards edges and penalises any excess connectivity. The penalty is a soft constraint rather than a hard one, so the walk may pass briefly through infeasible graphs rather than being walled inside the feasible region, and we return below to why that freedom is worth having. A move toggles one adjacency. The Metropolis rule \cite{Metropolis53} accepts an improving move always and a worsening move only with probability $e^{-\Delta E / T}$, a chance that shrinks both as the move gets worse and as the temperature drops, and the temperature $T$ falls geometrically, multiplied by a fixed factor just below one at every step.
...
The paired grids of \Cref{fig:variant-bounds-m3,fig:variant-bounds-m6} apply that reading to all twelve variants. Blue curves are theorems, red curves conjectures, and yellow curves interpolate lower bounds, while filled squares are exact finite values and open circles are witnesses. Directed vertex values were checked separately because Whitney's inequality transfers witnesses but not upper bounds.
...
% Each grid holds twelve plot panels. Side by side at half the text width their
...
\begin{sidewaysfigure}[p]
    ...
    \caption{All twelve variants at $m=3$. Rows select the model and columns the direction and separation, while colours encode theorem, conjecture, or interpolation, and filled squares are exact finite values against open circles for witnesses. \Cref{fig:variant-bounds-m6} is the same grid at $m=6$.}
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The same twelve variants at $m=6$, to be read against \Cref{fig:variant-bounds-m3}. Raising $m$ widens every feasible band, and the open circles thin out as the certified range falls behind the constructions.}
