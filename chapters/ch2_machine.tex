...
Notice how depth first search is optimal for memory efficiency and finding extremal graphs as soon as possible.
...
It is therefore detrimental to distinguish between the densest graph possible and found.
...
Menger's theorem (\Cref{thm:menger}) already turned $\lambda(u,v)$ into the number of edges/arcs in a minimal cut. 
...
Undirected multigraphs have the natural identification to a directed multigraph by replacing 
every edge with a pair of arcs that are going in opposite directions.
...
Another representations for multi-hypergraphs is a separate gate with capacity one for each hyperedge.
...
Adding vertices is the most expensive change, followed by raising the connectivity
budget and, lastly, adding direction, which doubles the possibilities for each pair.
...
\end{scope}
\end{tikzpicture}
\caption{The two encodings, shown for the directed variants. The digraph is represented by the multiplicity matrix $\mu$, where $\mu_{ij}$ counts arcs from $i$ to $j$. The directed hypergraph is a list of hyperedges instead, where the tail of each is isolated. Multihypergraphs repeat at least one entry in the list of hyperedges. Since a matrix (or tensor) representation would need $n^r$ entries and those would be almost all zero, that generalization is worth deviation from for efficiency.}
...
of multiplicities $\mu(u,v)$ and $\mu(v,u)$. Each multiplicity lies between $0$ and $m-1$, 
and they cannot both be zero because every support edge must represent at least one arc. 
...
For the reverse inequality, we need a maximum-weight matrix whose entries can
be multiplied by $m-1$ to give integer arc multiplicities.  Here a
\emph{double star} is the directed $0$--$1$ matrix obtained by choosing two
distinct vertices $a$ and $b$, putting an arc from every other vertex to $a$
and from $b$ to every other vertex, and putting all other entries equal to
zero.  (The two centres may also be described as the sink and source of the
star.)  Thus it has one incoming star and one outgoing star, with no parallel
arcs.  The historical optima for $n\le6$ had exactly this form, so their
entries remain integral after multiplying by $m-1$.
The hand proof now proves the stronger statement that such an integral
optimum exists for every $n$ (\Cref{cor:mstar-integral}).  Consequently, for
every $n$ and every $m\ge2$, the maximum number of arcs in a directed
multigraph with pairwise maximum flow at most $m-1$ is exactly
$(m-1)M^*(n)$: the scaled double-star construction gives the lower bound,
and the preceding scaling argument gives the upper bound. 
The finite optimisation is retained only as an independent check of the cases $n\le6$.
\[
    L_m^{\mathrm{dir}}(n) = (m - 1)\, M^{*}(n)
    \qquad \text{for every } m \ge 2 .
\]
%the reasoning in this paragraph is wonky, please improve it. (don't give a full proof, just explain better)
\begin{figure}[ht]
\centering
...
likely to be an (almost) extremal graph.
...
\]
which rewards edges and penalises any excess connectivity. 
The search starts from the empty graph and moves by toggling an arc. 
...
Tabu always wins speedwise and there manages to close hard problems faster: at $n = 7$, $m = 3$ it reaches the $24$-arc directed multigraph extremiser, where annealing stalls at $20$ and $18$ arcs against $16$ on the simple digraph. 
