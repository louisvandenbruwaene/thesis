...
different route, clearing denominators from a rational optimum rather than
exhibiting a shape. Consequently, for every $n$ and every $m\ge2$, the
...
Removing an arc can never raise $\lambda^{\max}$, while adding one raises it by at most one. 
...
The paired grids of \Cref{fig:variant-bounds-m3-graphs,fig:variant-bounds-m3-hyper,fig:variant-bounds-m6-graphs,fig:variant-bounds-m6-hyper} apply that reading to all sixteen variants. Blue curves are theorems, red curves conjectures, and yellow curves interpolate lower bounds, while filled squares are exact finite values and open circles are witnesses. Directed vertex values were checked separately because Whitney's inequality transfers witnesses but not upper bounds. \Cref{tab:variant-values-m3-graphs,tab:variant-values-m3-hyper,tab:variant-values-m6-graphs,tab:variant-values-m6-hyper} give the same sixteen variants as numbers, in the same colours, for a reader who wants a value rather than a position on a curve.
...
\begin{table}[p]
    ...
    \footnotesize
    \resizebox{\textwidth}{!}{%
    \input{figures/variant_table_m3_graphs.tex}%
    }
    \caption{\Cref{fig:variant-bounds-m3-graphs} as numbers. Blue is proved, red conjectured, gold open, matching the figure's curves, and a bold green entry is machine-checked exact regardless of the row's colour. A plain (non-bold) proved entry is still the exact value for every $n$, since the closed form is a theorem there, not a bound reached only at small sizes.}
    \label{tab:variant-values-m3-graphs}
\end{table}
...
\begin{table}[p]
    ...
    \footnotesize
    \resizebox{\textwidth}{!}{%
    \input{figures/variant_table_m3_hypergraphs.tex}%
    }
    \caption{\Cref{fig:variant-bounds-m3-hyper} as numbers, same colour code. The two undirected rows are proved only as an upper bound (\Cref{prop:hyper-edge}): a $\le$-prefixed blue entry bounds the true value without being known to equal it, while a plain blue entry (or a bold green one) is the value itself. The two directed rows are open, and a $\ge$-prefixed gold entry is a verified construction rather than a proved optimum.}
    \label{tab:variant-values-m3-hyper}
\end{table}
...
\begin{table}[p]
    ...
    \footnotesize
    \resizebox{\textwidth}{!}{%
    \input{figures/variant_table_m6_graphs.tex}%
    }
    \caption{\Cref{fig:variant-bounds-m6-graphs} as numbers, same colour code as \Cref{tab:variant-values-m3-graphs}. Raising $m$ from $3$ to $6$ moves two rows, undirected simple vertex and its multigraph reduction, from proved to open (\Cref{thm:leonard}, valid only for $m\le4$), so the same variant that carried no $\ge$ prefix at $m=3$ carries one here past its exhausted range.}
    \label{tab:variant-values-m6-graphs}
\end{table}
...
\begin{table}[p]
    ...
    \footnotesize
    \resizebox{\textwidth}{!}{%
    \input{figures/variant_table_m6_hypergraphs.tex}%
    }
    \caption{\Cref{fig:variant-bounds-m6-hyper} as numbers, same colour code as \Cref{tab:variant-values-m3-hyper}. The undirected edge row's $n=6$ entry is the $\le12$ named above and in that figure's caption: the true maximum there is $11$, established by the full enumeration described above rather than by the timed exhaustion behind this table's exact entries.}
    \label{tab:variant-values-m6-hyper}
\end{table}
