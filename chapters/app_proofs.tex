...
with the limitations of its surviving record.
\end{remark}
...
\paragraph{Historical evidence at the next vertex threshold.}
\Cref{thm:hyper-vertex-m3} establishes the vertex bound at $m=3$; its rank
...
attained and one further hyperedge excluded in all eleven cases. No separate
raw completion transcript has been located, and these searches have not
been rerun for this account. The results are therefore retained as historical
reports, rather than added as independently audited exact values to the
tables. The note also reports that the script's $r=2$ cross-check finds
counterexamples at $m=5$, consistent with the multigraph witnesses discussed
in \Cref{sec:multi-vertex}. These checks concern multihypergraphs, not only
simple hypergraphs, and do not establish the formula for arbitrary $n,r$.
...
general bound; the finite search does not supply it.
