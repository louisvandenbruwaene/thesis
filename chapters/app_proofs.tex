...
The vertex measure at the first non-trivial ceiling is shorter still, and needs no tree at all.

\begin{proof}[Proof of \Cref{prop:leonard-m2}]
...
\begin{remark}[Audit of the counting convention]\label{rem:threshold-audit}
This remark completes \Cref{rem:threshold-convention}, which fixes the convention. What follows is the bookkeeping that convention forces on every constant quoted from the literature.
...
The \emph{vertex} problem $k_m(n)$ is the one open since 1974, and this short section records its block structure. Nothing here is deep. What it buys is a change of the object being searched: from graphs on $n$ vertices, with $n$ growing, to $2$-connected graphs alone.
...
\apppart{Part II. The directed results}{The exact value at $m=2$, the structural propositions that bound the frontier at $m \ge 3$, and the directed multigraph problem closed for every $n$ and every $m$.}
...
decomposition \cite{Tutte66} expresses it as a tree of cycle, bond, and
...
\Cref{ch:basecases}. In words: at the route limit $m = 3$ a hyperedge network can hold no more hyperedges under the stricter separation than the displayed edge bound. When $r \ge 3$ and $2 \le \binom{n-2}{r-2}$, that count is reached without repeating a hyperedge, so the two separations agree in this range exactly as they did at $m = 2$.
...
exists. Thus the proof is deliberately confined to $m\le3$, and no general
...
head vertices, a Berge route entering at the tail and leaving at a head. If every pair of vertices has at most $m-1$
...
\apppart{Part IV. One extension, and the audit}{A separate extremal problem that the parallel-copy convention of \Cref{sec:parallel-convention} sets aside, and the record of what was computed, with what, and to what standard.}
...
at the small sizes used below. The structural statements, rather than the
cell-by-cell output, are what matter here.
...
complete, which motivates the family below. The retained transcript is
\path{figures/multi_vertex_blocks_log.txt} in the repository named in
\Cref{sec:transcripts}.
...
the ceiling $m-1$. Finite block sweeps also show that adding a few edges inside
...
repository at \url{https://github.com/louisvandenbruwaene/thesis}. The snapshot
this document was built from is commit \thesiscommit, which is the immutable
reference for every number reported here. Anything later in that repository is a
continuation and not what was submitted. From \code{program/},
...
\code{python make\_figures.py} regenerates the computational figures.
