...
Passing \code{-{}-refresh} recomputes every entry and reconciles the new outcomes
with the record. A rerun may confirm or improve an entry. It cannot replace an
entry with a weaker result merely because one timed run performed worse.
Deleting the data file is the stronger operation. It discards the record before
...
by \code{BMAX=9 python3 scripts/simple\_vertex\_blocks.py}. The recorded
environment used nauty 2.9.3.

The three commands assume an interpreter with the dependencies installed, which
the repository provides as a virtual environment at its root and which
\code{program/} alone reproduces from its lock file.
