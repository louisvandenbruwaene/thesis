...
\emph{Conjecture, with proof}; the presence of an argument does not establish them.
...
In words: at $m = 2$ a feasible digraph can afford no arc that some other route already makes redundant, so it is its own reachability skeleton, and the count \Cref{lem:reach-skeleton} proves for skeletons in general is already a count for $D$ itself. Nothing here inspects $n$, and no base case is needed.
