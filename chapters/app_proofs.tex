...
It helps to first ask what a single vertex deletion actually buys at $m=2$: removing a vertex can only remove routes passing through it, so no pair's connectivity ever rises, and the whole $m=2$ induction rests on that one guarantee together with an averaging bound on how much one well-chosen vertex carries. The multigraph problem at a general $m$ wants the same two ingredients, a deletion that is guaranteed to lower every connectivity, paired with a bound on how much it removes, but there the natural analogue of ``one vertex'' breaks down: once multiplicities are free to sit anywhere between $0$ and $m-1$, no single vertex is ever guaranteed to carry a controllable share of the arcs, which is exactly the fractional remainder above. The fix is to stop insisting on a vertex and delete a whole \emph{subgraph} instead: one general enough to always exist, sparse enough to bound, and specific enough that removing it is guaranteed to lower every pairwise connectivity by exactly one.
...
\begin{scope}
  \node[apx] (a1) at (0,1)     {}; \node[apname,left=2pt]  at (a1) {$a_1$};
  \node[apx] (a2) at (0,-1)    {}; \node[apname,left=2pt]  at (a2) {$a_2$};
  \node[apx] (b1) at (2.6,1)   {}; \node[apname,above=2pt] at (b1) {$b_1$};
  \node[apx] (b2) at (2.6,-1)  {}; \node[apname,below=2pt] at (b2) {$b_2$};
  \node[apx] (c1) at (5.2,1)   {}; \node[apname,above=2pt] at (c1) {$c_1$};
  \node[apx] (c2) at (5.2,-1)  {}; \node[apname,right=2pt] at (c2) {$c_2$};
...
\begin{scope}[xshift=9.6cm]
  \node[apx] (ra1) at (0,1)     {}; \node[apname,left=2pt]  at (ra1) {$a_1$};
  \node[apx] (ra2) at (0,-1)    {}; \node[apname,left=2pt]  at (ra2) {$a_2$};
  \node[apx] (rb1) at (2.6,1)   {}; \node[apname,above=2pt] at (rb1) {$b_1$};
  \node[apx] (rb2) at (2.6,-1)  {}; \node[apname,below=2pt] at (rb2) {$b_2$};
  \node[apx] (rc1) at (5.2,1)   {}; \node[apname,above=2pt] at (rc1) {$c_1$};
  \node[apx] (rc2) at (5.2,-1)  {}; \node[apname,right=2pt] at (rc2) {$c_2$};
...
so $K_m^{\mathrm{multi}}(n) = \Theta(m^2 n)$: the bouquet of \Cref{thm:clique-chain-vertex} is a lower bound of the right order in $n$ and in $m$ both, and what separates the two sides is a constant factor tending to $16$. \Cref{thm:multi-vertex-bipartite} below improves the lower constant and brings that factor down to $6 + 4\sqrt{2} \approx 11.66$. The upper bound is deliberately crude, since it throws away the $-\sum \pi$ correction entirely and charges every edge the full $m-1$, and the first inequality $K_m^{\mathrm{multi}}(n) \le (m-1)k_m(n)$ is the more informative half: it says this variant can never outrun the simple vertex problem by more than the multiplicity ceiling, which ties the least understood of the twelve to the one with the longest history. It is tight at $m = 2$, where both sides read $n-1$.
...
\emph{The sizes fit.} If $G_0$ has $c$ components and blocks $B_1, \dots, B_t$, then $\sum_i (|V(B_i)| - 1) = n - c - (\text{number of isolated vertices contributed}) \le n-1$: within one component, contracting the block tree gives $\sum (|V(B_i)|-1)$ equal to the number of vertices of that component minus one, and summing over components loses one further vertex per extra component. With \Cref{lem:multi-vertex-objective} this proves $\le$.
