This appendix carries every proof in the thesis. The body states each result
and briefly explains its main idea. The full arguments are collected here.
So are the supporting lemmas and propositions that are not stated in the
body. Figures appear beside the arguments that need them.

\paragraph{How to read this appendix.}
The shared graph, route, and connectivity terminology is introduced in \Cref{ch:basecases}.
Terms specific to the appendix are defined when they first become necessary.
The appendix has four parts, and they do not carry equal weight.

\textsc{Part I} contains classical machinery: \Cref{thm:menger,thm:gomory-hu} and Mader's theorem.
A reader who trusts the cited results may skip it.
\textsc{Parts II} and \textsc{III} contain the original arguments.
Readers who want a selective route through them can focus on three proof ideas.
The checked reachability-skeleton argument establishes the directed multigraph arc formula.
The checked incidence-rank argument establishes the hypergraph vertex upper bound at $m=3$, with equality in the stated attainment ranges.
The checked two-step budget supplies elementary bounds for directed graphs and hypergraphs.
The arguments still marked as proposed remain subject to verification. Their statements are labelled
\emph{Conjecture, with proof}, and the presence of an argument does not establish them.

\textsc{Part IV} contains the search's connectivity bookkeeping and the computational audit.
Every construction named in the body is defined in this appendix.
The directed constructions appear in \Cref{sec:dir-constructions}.
A heading with the badge \aimedal{} contains a proof written by an AI model that the author has not verified line by line.
The Contribution Statement explains this status.
Unmarked results are cited classical results or arguments the author has checked.
The latter group includes the Gomory--Hu double count of \Cref{sec:mader-gomory-hu}.
That count proves \Cref{thm:mader,thm:multigraph-edge} from \Cref{lem:dist-1-count,lem:sum-dist,lem:double-count}.

\Cref{tab:conjecture-index} indexes every statement that carries the badge, so
a reader can separate the checked results from the proposed ones without
reading the appendix through. The table is generated from the same source walk
that enforces the marking, so a renamed or added conjecture changes its own row.

\begin{table}[htbp]
\centering\small
\renewcommand{\arraystretch}{1.2}
\input{figures/conjecture_table}
\caption{Index of the statements whose proofs are proposed rather than checked.
Each is stated as \emph{Conjecture, with proof} and carries \aimedal{} in its
heading. The proposed arguments are retained as research notes and do not
establish these statements. Some entries are dependencies of others: for
instance \Cref{lem:equality-forces,lem:skeleton-shallow} are the two steps of
\Cref{thm:dir-multi-uniqueness}.}
\label{tab:conjecture-index}
\end{table}
...
% ---------------------------------------------------------------------------
% Part dividers. The appendix is one chapter, so its thirteen sections would
...
\apppart{Part I. Classical machinery}{The two theorems every later argument runs on, stated with their citations and not reproved, and Mader's theorem proved here in full through the Gomory--Hu tree.}
...
Every \emph{undirected} capacitated graph $G$ has a weighted tree $T$ on the same vertex set, its \emph{Gomory--Hu tree}, with two properties. First, for all distinct $u,v$, the value $\lec_G(u,v)$ is the minimum edge weight on the $u$--$v$ path in $T$. Second, deleting a tree edge $e=xy$ partitions the vertices into two sets whose cut in $G$ is a minimum $x$--$y$ cut and has capacity equal to the weight of $e$. Thus the tree records both pairwise connectivity values and the specific cuts used below.
\end{theorem}

The theorem applies to undirected simple graphs and multigraphs, but not to asymmetric directed cuts or vertex connectivity. Its standard construction is proved in \cite{GomoryHu61}. The second property is essential to the double count: reproducing pairwise values alone would not justify treating every tree-edge partition as a cut of that weight.

We also use the immediate endpoint bound.

\begin{lemma}[Degree bound]\label{lem:degree-bound}
Every pair $u, v$ of distinct vertices of a graph $G$ satisfies $\lec_G(u, v) \le \min(\deg u, \deg v)$, and every ordered pair of a digraph $D$ satisfies $\lec_D(u, v) \le \min(d^+(u), d^-(v))$.
\end{lemma}
...
The vertex measure has the same unit step, and under the incidence convention of \Cref{sec:incidence-convention} it has the same one-line proof in every model. A route is a path of the incidence graph, and every copy the route uses is an interior node of that path, so two internally disjoint routes never share a copy. At most one member of a largest family therefore uses the new copy. Deleting that route leaves a family in $G$ and gives
\[
    \kappa_G(s,t) \le \kappa_{G^+}(s,t) \le \kappa_G(s,t)+1.
\]
Taking the maximum over all pairs proves the two asserted unit-step bounds. A copy parallel to an existing adjacency is one of the cases, not an exception to it: a new copy of $uv$ is one further route from $u$ to $v$, so it raises $\kappa(u,v)$ by exactly one, and by at most one at every other pair.
\end{proof}
...
\begin{scope}
  ...
  \draw[apgedge] (a)--(c);          % dist 1, a tree edge
  \draw[apgedge,vertexred] (e) to[bend right=22] (c);  % dist 2, highlighted
...
For integers $N \ge 1$ and $0 \le r \le N-1$ there is a graph on $N$ vertices with $\floorfrac{rN}{2}$ edges and maximum degree at most $r$.
\end{lemma}
...
The upper bound above proves \Cref{thm:mader}, and it uses no relation between $n$ and $m$: the Gomory--Hu tree has $n-1$ edges whatever $m$ is. \Cref{thm:extremal-existence} adds that the bound is attained once $n \ge m$, which is where that hypothesis is needed and where it belongs. For $n<m$, the complete graph is itself feasible and the bound is slack except at $(n,m)=(2,3)$, where the floor creates the isolated equality noted in the main text. Equality then holds systematically from $n=m$ onward. The proof also reveals which graphs are extremal: equality forces zero total slack when $m(n-1)$ is even and exactly one unit of slack when it is odd.
...
For $m = 2$ the characterisation says the extremal graphs are exactly the spanning trees: every tree edge must carry weight one and lie in $G$, so $G$ contains its own Gomory--Hu tree and has only $n-1$ edges to spend. For $m = 4$ and $n \equiv 1 \pmod{3}$ it is consistent with the trees of $K_4$ blocks glued at shared cut vertices, the case of \Cref{const:shared-clique}.

\begin{remark}[Forcing and avoiding counts]\label{rem:threshold-convention}\label{rem:threshold-audit}
This thesis maximises the number of edges while avoiding $m$ disjoint routes
between every pair. S{\o}rensen and Thomassen and the Erd\H{o}s Problems
database instead give the least edge count that forces such a pair
\cite{SorensenThomassen74,ErdosProblems}. Their forcing count is therefore
one more than the corresponding maximum here.

For example, Theorem~4 of \cite{SorensenThomassen74} gives the vertex forcing
count $\floor{8n/3}-3$ for $n\ge6$ except $n=7,12$, where it gives $16$ and
$28$. Subtracting one gives the values used here. The paper also states the
whole small range as $\floor{5(n-1)/2}+1$ for $6\le n\le13$, which yields
the two-range formulation of \Cref{thm:sorensen-thomassen}. Thus at $n=13$
both avoiding counts are $30$, and the edge and vertex values first diverge
at $n=14$.

The Bollob\'{a}s--Erd\H{o}s conjecture must also retain its vertex count:
the proposed forcing count $1+\binom m2 p$ is on $1+(m-1)p$ vertices.
In the notation of this thesis its proposed avoiding rate is $m/2$, not
$\binom m2$ per vertex.
\end{remark}
...
\begin{conjecture}[The vertex problem is a knapsack over blocks, with proof,\aimedal]\label{thm:simple-vertex-blocks}
For $b \ge 2$ let
\[
...
vertex. Write $a_n := k_m(n)$ and $b_t := a_{t+1}$ for $t\ge1$.
The gluing inequality gives $b_{s+t}\ge b_s+b_t$ for $s,t\ge1$.
Fekete's lemma \cite{Fekete23} therefore gives
$b_t/t\to\sup_{t\ge1}b_t/t$, or equivalently
$a_n/(n-1)\to\sup_{n\ge2}a_n/(n-1)$. Also
$a_n/(n-1) \ge h_m(b)/(b-1)$ whenever $(b-1) \mid (n-1)$, while
$a_n \le \sup_b h_m(b)/(b-1) \cdot (n-1)$ from the display. Hence the limit is
$\sup_b h_m(b)/(b-1)$.
\end{proof}

Three things follow at once.

At $m = 3$ it would \emph{reprove} \Cref{thm:leonard}'s value, and that
deduction is only as settled as the reduction it runs on. A block with
...
\[
    ...
    \qquad (n \ge m-1),
\]
halving the constant in the general upper bound used here. The range is where
...
$m$ finds no feasible one for $m \le 6$ and $n \le 10$, run from the standalone script
\code{program/scripts/vertex\_min\_degree.py}. It is stated here as a
...
\apppart{Part II. The directed results}{The three constructions the body names but does not define, the reachability skeleton that both exact directed results run on, the value at $m=2$ under both separations, the structural propositions that bound the frontier at $m \ge 3$, and the checked proof of the directed multigraph arc value at every $n$ and every $m$.}

\section{The directed constructions}\label{sec:dir-constructions}

Three basic shapes supply the directed lower bounds used below. The additional clique-core construction appears in \Cref{const:clique-core}. \Cref{ch:basecases} shows the first two side by side in \Cref{fig:simple-digraph-m2} and records what they reach. Here each is defined precisely and its connectivity checked.
...
\section{The reachability skeleton}\label{sec:reach-skeleton}

One checked lemma supports both directed formulas, so its statement and proof come first. The object is a thin subdigraph that carries the whole
reachability relation of $D$ and nothing else. \Cref{sec:dir-arc-m2} uses it
once in the $m=2$ argument under both separations, and
\Cref{sec:dir-multi} uses it $m-1$ times over, peeling one copy of each of its
arcs per unit of the cap.
...
A reachability skeleton keeps every yes-or-no fact about who can reach whom and discards everything else, in particular all but one copy of each arc it keeps. It is a caricature of $D$: thin, and faithful on reachability, but not on the number of disjoint paths. The lemma below says such a skeleton always exists and is never too big. That one fact is what both later arguments run on. At $m = 2$ the skeleton turns out to be the whole digraph, which is the content of \Cref{thm:dir-vertex-m2-exact}, and at a general $m$ deleting it repeatedly is what \Cref{lem:reach-delete} makes pay.
...
\end{scope}
\end{tikzpicture}}
\caption[A reachability skeleton]{The construction of \Cref{lem:reach-skeleton}. Each bidirected pair $A,B,C$ is a strongly connected component. Contracting the pairs gives $A\to B\to C$ and the redundant arc $A\to C$. Delete that arc to obtain the triangle-free transitive reduction. The skeleton $R$ keeps the internal arcs of each pair and the two remaining connections between pairs. Although $a_1\to c_2$ is removed, $a_1$ still reaches $c_2$ through $B$.}
\label{fig:skeleton-example}
\end{figure}

\section{The directed arc value at \texorpdfstring{$m = 2$}{m=2}}\label{sec:dir-arc-m2}

This section proves the two $m=2$ values.
The vertex upper bound uses \Cref{lem:reach-skeleton}.
The arc upper bound then follows because every arc-feasible digraph is vertex-feasible.
The completed enumerations in \Cref{tab:basecase-search} establish the values independently for $2\le n\le7$.
They establish only those finite cases. The all-order values are what the two theorems below prove.
...
\Cref{lem:subdigraph-monotone} justifies the low-degree vertex deletion in \Cref{sec:two-step-budget}.
...
\]
Every digraph feasible for the \emph{arc} problem is feasible for the \emph{vertex} problem; Whitney's inequality alone does not give the converse. The vertex-feasible family contains the arc-feasible one, so the inequality between the extremal numbers it hands over for free is
\[
    k_m^{\mathrm{dir}}(n) \;\ge\; \ell_m^{\mathrm{dir}}(n),
\]
and an upper bound for the arc problem says nothing at all about the vertex problem. Local arc and vertex connectivities can differ, since two routes that share an interior vertex but no arc are counted by $\lec$ and not by $\kap$. That alone does not show that the two families differ at a given threshold, and at $m=2$ they coincide, as \Cref{lem:directed-m2-families} shows. Whitney therefore gives one thing, that an arc construction is a vertex \emph{lower} bound, and the vertex upper bound has to be proved separately. The proof below does that, and \Cref{ch:synthesis} treats the two separations the same way for $m \ge 3$, where the vertex problem stays open without assuming that the two optima coincide.
\end{remark}

\begin{proof}[Proof of \Cref{thm:dir-vertex-m2-exact}]
Write $M(n) = \max(2(n-1), \floor{n^2/4})$.
...
\emph{Upper bound.} Let $D$ be a simple digraph on $n \ge 2$ vertices with $\kap^{\max}_D \le 1$, and let $R \subseteq D$ be a reachability skeleton with $|A(R)| \le M(n)$, which \Cref{lem:reach-skeleton} supplies for every loopless directed multigraph and so in particular for this one.

We claim $A(D) = A(R)$. One inclusion is the definition of $R$. For the other, take an arc $(u,v) \in A(D)$ and suppose $(u,v) \notin A(R)$. The arc itself shows that $u$ reaches $v$ in $D$, so $u$ reaches $v$ in $R$ as well, along some directed path $P$ using only arcs of $R$. Since $(u,v) \notin A(R)$, the path $P$ is not that single arc, so it has length at least two. Now $P$ and the one-arc route $u \to v$ are two distinct $u$--$v$ routes, and they are internally vertex-disjoint, because the one-arc route has no interior at all and the empty set meets nothing. Hence $\kap_D(u,v) \ge 2$, against feasibility. So $(u,v) \in A(R)$.

Therefore $|A(D)| = |A(R)| \le M(n)$, which with the lower bound gives $k_2^{\mathrm{dir}}(n) = M(n)$.
\end{proof}

In words: at $m = 2$ a feasible digraph can afford no arc that some other route already makes redundant, so it is its own reachability skeleton, and the count \Cref{lem:reach-skeleton} gives for skeletons in general is already a count for $D$ itself. Nothing here inspects $n$, and no base case is needed.

The arc value costs one further line.

\begin{proof}[Proof of \Cref{thm:dir-arc-m2-exact}]
The two constructions above give $\ell_2^{\mathrm{dir}}(n) \ge M(n)$. For the upper bound, \Cref{rem:whitney-direction} places every arc-feasible digraph inside the vertex-feasible family, so \Cref{thm:dir-vertex-m2-exact} bounds it. Chaining the three,
\[
    M(n) \;\le\; \ell_2^{\mathrm{dir}}(n) \;\le\; k_2^{\mathrm{dir}}(n) \;\le\; M(n),
\]
so all three are equal. The same argument also proves it directly: in the claim above, $P$ shares no \emph{arc} with $(u,v)$ either, since $P$ runs inside $R$ and $(u,v)$ does not, so $\lec_D(u,v) \ge 2$ follows by the identical sentence.
\end{proof}

\begin{lemma}[The two directed feasible families at $m=2$]\label{lem:directed-m2-families}
A digraph satisfies $\lec^{\max}\le1$ if and only if it satisfies $\kap^{\max}\le1$ under the incidence convention.
\end{lemma}
\begin{proof}
If two arc-disjoint directed $u$--$v$ paths exist, follow the first path to its first vertex $w\ne u$ on the second.
Their $u$--$w$ prefixes have no common internal vertex and remain arc-disjoint.
Thus some pair has two internally disjoint routes whenever some pair has two arc-disjoint routes.
The converse follows from $\kap\le\lec$.
\end{proof}
This argument explains the identical node counts in \Cref{tab:basecase-search}: equivalent feasibility tests accept the same partial digraphs, so the same branching and pruning rules visit the same nodes.


The exhaustive searches independently give the maxima $2,4,6,8,10,12$ at $2\le n\le7$.
The general multigraph theorem uses the same skeleton argument, so it is not independent corroboration of that argument.

\begin{remark}[The directed vertex problem at $m \ge 3$]\label{rem:dir-vertex-m3}
The skeleton argument gives the value at $m=2$ (\Cref{thm:dir-vertex-m2-exact}, through \Cref{lem:reach-skeleton}). For $m\ge3$, the vertex problem is separate: the arc constructions belong to the smaller arc-feasible family and give no upper bound on $k_m^{\mathrm{dir}}(n)$. What can be said is computational. Running the exhaustive search of \Cref{ch:certify} under both feasibility tests at $m = 3$ gives
\[
\begin{array}{r|ccccc}
    n & 2 & 3 & 4 & 5 & 6 \\ \hline
...
\end{array}
\]
with every entry proved complete, so the two separations agree at every size the exhaustion reaches, and from $n = 3$ onwards both match the construction lower bound $\max(m(n-1), \floor{(n+m-2)^2/4})$. The table stops at $n = 6$ because that is where the exhaustion stops: at $n = 7$ neither sweep closes inside an hour and a half, and each returns the construction's $18$ as a bare lower bound. That construction formula applies only for $n\ge m$. At $n=2$, the complete digraph instead gives the maximum of $2$ arcs. Agreement over five sizes is evidence and not a proof, and the natural next question is structural rather than numerical: to separate the two problems one needs a digraph where some pair carries strictly more arc-disjoint routes than internally disjoint ones \emph{and} where that slack pays for extra arcs. The two demands pull against each other, which is perhaps why no small example does both.
\end{remark}
...
For the connectivity, fix $a \in A$ and $b \in B$. Consider the following routes from $a$ to $b$: the direct arc, and the two-step detours $a \to b' \to b$ that pass through an in-neighbour $b'$ of $b$ inside $B$. There are $m - 2$ such detours, giving a family of $m - 1$ arc-disjoint routes. Longer routes may exist, but they cannot enlarge this family beyond the cut bound that follows. No larger arc-disjoint family is possible, because every route from $a$ to $b$ must finish with one of those $m-1$ in-arcs. Deleting them therefore cuts every route from $a$ to $b$. For a pair $b_i, b_j$ inside $B$ the same cut argument gives an upper bound of $m-2$: the total in-degree of $b_j$ is $|A| + m - 2$, but nothing leaves $B$ towards $A$, so the only in-arcs of $b_j$ that a route starting at $b_i$ can reach are the $m-2$ inside $B$, and deleting those cuts the pair. Pairs inside $A$ and pairs from $B$ to $A$ have no routes at all, so $\lec^{\max} = m - 1$.
\end{proof}

\section{Structural propositions on the directed frontier}

The augmented bipartite construction gives a quadratic lower bound, but it
does not describe every extremal digraph. The following construction is denser
than both the hub and the augmented bipartite families on a range of orders, so
$\ell_m^{\mathrm{dir}}$ is not the maximum of those two counts, and the
decomposition proposed alongside that formula does not hold either.

\begin{construction}[Clique-core counterexample]\label{const:clique-core}
Let $n\ge m\ge2$, put $p=n-m$, $a=\ceil{p/2}$ and $b=\floor{p/2}$.
Take disjoint sets $S,C,T$ of sizes $a,m,b$, with
$C=\{c_1,\ldots,c_m\}$. Include exactly the arcs
\[
 c_i\to c_j\ (i\ne j),\qquad
 S\to\{c_1,\ldots,c_{m-1}\},\qquad
 \{c_1,\ldots,c_{m-2}\}\to T,\qquad S\to T.
\]
An arrow between sets means that all arcs in that direction are present.
\end{construction}

The following cuts verify feasibility. In each row, $X$ contains the starting
vertex and excludes the terminal vertex, and the capacity counts arcs leaving $X$.
\[
\begin{array}{c|c|c}
\text{ordered pair}&X&\text{capacity}\\ \hline
c_i,c_j\ (i\ne j)&\{c_i\}\cup T&m-1\\
s,c_j&\{s\}\cup T&m-1\\
...
\end{array}
\]
All other ordered pairs are unreachable. Each pair of distinct core vertices
also has the direct path and $m-2$ two-step paths inside $C$, so
$\lec_D^{\max}=m-1$. The arc count is
\[
 |A(D)|=m(m-1)+(m-1)a+(m-2)b+ab
       =\floorfrac{(n+m-3)^2}{4}+2(m-1)=:B_m(n).
\]
Indeed, $a(b+1)=\floor{(p+1)^2/4}$. Subtracting the old branches gives
\[
 B_m(n)-m(n-1)=\floorfrac{(p+1)^2}{4}-2p,\qquad
 B_m(n)-\floorfrac{(n+m-2)^2}{4}=m-1-\floor{p/2}.
\]
Both differences are positive when $m\ge5$ and $7\le p\le2m-3$.
At $m=5$, $n=12$, the four arc classes contribute $20+16+9+12=57$,
whereas the earlier formula gives $\max(55,56)=56$.
This establishes a lower bound, not the exact value of $\ell_5^{\mathrm{dir}}(12)$.
The independent script \code{program/scripts/clique\_core\_check.py} checks the
displayed cuts and identities and enumerates all $4094$ nontrivial cuts of the
twelve-vertex example.

The same example refutes the earlier decomposition conjecture. A partition
with all arcs from a non-empty $A$ to $B$, no arcs inside $A$, and no arcs
from $B$ to $A$ would give
\[
 |A(D)|\le |B|(n-|B|)+(m-2)|B|
          \le\floorfrac{(n+m-2)^2}{4}.
\]
For the first inequality, fix $a\in A$ and $b\in B$. The direct path and
the two-step paths through the internal in-neighbours of $b$ force
$d_B^-(b)\le m-2$. At $m=5$, $n=12$, this would bound every extremiser
by $56$, contradicting the feasible graph with $57$ arcs.
At $n=3m-3$, the excess over the quadratic value is still one, so any
threshold beyond which the augmented bipartite count is always optimal would have to be at least $3m-2$ for $m\ge5$. This necessary condition does not establish that such a threshold exists.
...
\begin{lemma}[Feasible digraphs are budgeted]\label{lem:feasible-budgeted}
...
\begin{lemma}[The budget, read on degrees]\label{lem:budget-degree-sum}
...
For $a,b\ge0$ with $a+b>0$, the estimate
$\min(a,b)\le2ab/(a+b)$ follows from $\max(a,b)\ge(a+b)/2$.
We also use
$\floor{(n-1)^2/4}+\floor{n/2}=\floor{n^2/4}$, checked separately for even
and odd $n$.

\begin{lemma}[Deleting the smaller half of every vertex]\label{lem:smaller-half-deletion}
Every simple digraph $D$ on $n$ vertices can be made \emph{one-directional bipartite}, meaning that every remaining arc runs from one part to the other and none returns, by deleting at most $\sum_{x} \min(d^+(x), d^-(x))$ arcs. Consequently
\[
...
\begin{proof}
Call $x$ \emph{source-like} if $d^+(x) \ge d^-(x)$ and \emph{sink-like} otherwise, and write $S$ and $T$ for the two classes, so $V = S \cup T$. Delete every in-arc of a source-like vertex and every out-arc of a sink-like vertex. A source-like $x$ marks $d^-(x) = \min(d^+(x), d^-(x))$ arcs for deletion and a sink-like $x$ marks $d^+(x) = \min(d^+(x), d^-(x))$ arcs, so the number deleted is at most the displayed sum, an arc counted twice by that estimate only making it safer. An arc $(a,b)$ survives only if $a$ is not sink-like and $b$ is not source-like, that is, only if $a \in S$ and $b \in T$. So what remains runs entirely from $S$ to $T$ and has at most $|S|\,|T|$ arcs. Two non-negative integers of sum $n$ have product at most $\floor{n^2/4}$: writing them as $n/2 \pm t$ gives the product $n^2/4 - t^2$, largest at the smallest admissible $|t|$, which is $0$ for even $n$ and $1/2$ for odd $n$, giving $n^2/4$ and $(n^2-1)/4$ respectively, and both equal $\floor{n^2/4}$. Hence at most $\floor{n^2/4}$ arcs remain. Adding the two counts gives the display.
\end{proof}
...
If some vertex $v$ has $d(v) < n/2$, then $d(v) \le \floor{n/2}$ because $d(v)$ is an integer. Deleting a vertex only removes routes, so $D - v$ is $C$-budgeted too, and the induction hypothesis together with the floor identity above gives
\[
\begin{aligned}
    |A(D)|
...
Otherwise $d(x) \ge n/2$ for every vertex. By the estimate for $\min(a,b)$ above and then \Cref{lem:budget-degree-sum},
\[
...
\Cref{thm:dir-arc-linear-error}, stated in \Cref{ch:basecases}, says in words that a feasible simple digraph is never more than a linear amount denser than the balanced one-way complete bipartite digraph, which holds $\floor{n^2/4}$ arcs. Its quadratic term is therefore the leading term, and what remains in question is how large the linear correction is. Neither proof below assumes anything about the shape of $D$.
...
The second application is the directed \emph{vertex} problem. \Cref{thm:dir-vertex-m2-exact} gives its value at $m=2$, and for $m \ge 3$ this thesis has been careful to record that it does not follow from the arc case: Whitney's $\kap \le \lec$ makes the vertex-feasible family contain the arc-feasible one, so an arc upper bound restricts nothing there. That remains true of the \emph{bound}. What carries across is the route-counting, because the family \Cref{lem:feasible-budgeted} exhibits happens to be disjoint in the stronger sense as well, which is why that lemma is stated for both separations at once and why the argument never needs the arc value.
...
These bounds leave a gap in the linear coefficient. Exact eventual equality would also determine the bounded correction. The augmented bipartite construction supplies a lower linear contribution of $(m-2)n/2+O_m(1)$, whereas \Cref{thm:dir-arc-linear-error} allows $4(m-1)n$ above the quadratic term. For $m\ge3$, the ratio of these linear coefficients is $8(m-1)/(m-2)$: sixteen at $m = 3$, and tending to eight as $m$ grows. This compares coefficients, not the finite bounds including their constant and rounding terms. Once $n$ is large in terms of $m$, the Tur\'an theorem of Huang and Lyu \cite{HuangLyu26} cited in \Cref{ch:basecases} lowers the upper end to $m-1$, and the factor with it to $2(m-1)/(m-2)$: four at $m = 3$, and tending to two. \Cref{rem:case2-tight} explains why the existing degree estimates alone do not close that gap.

\begin{remark}[The dense case's coefficient needs a new inequality]\label{rem:case2-tight}
One might hope to cut the coefficient of the dense case, the one where every $d(x) \ge n/2$, using the observation that in the augmented bipartite construction one whole side has $\min(d^+, d^-) = 0$, so summing the worst-case bound for $\min(d^+,d^-)$ at every vertex looks wasteful. That observation alone does not cut the coefficient. The value four is the best universal coefficient obtainable from the two facts that case uses, the budget $\sum_x d^+(x)\,d^-(x) \le C n(n-1)$ of \Cref{lem:budget-degree-sum} and the floor $d(x) \ge n/2$. Read $C = m-1$ throughout what follows.
...
The crude per-vertex bound therefore loses nothing within this relaxation, and the reason is that its two sources of slack close at that very point. The estimate $\min(a,b)\le2ab/(a+b)$ is an equality only when $d^+(x) = d^-(x)$, and the floor substitution $d(x) \to n/2$ is an equality only when $d(x) = n/2$. Both hold at once exactly when $d^+(x) = d^-(x) = n/4$, which is the profile the relaxed optimum concentrates on. Cutting the constant therefore needs a second and independent inequality, one that constrains interference among the handful of expensive near-balanced vertices themselves rather than a sharper reading of the one already in hand.
\end{remark}

The linear-error bound controls the number of arcs. The next proposition also controls their arrangement: after deleting $O(\sqrt m\,n^{3/2})$ arcs, every remaining arc goes from one part of a partition to the other. This is called a \emph{bipartisation bound}. It does not guarantee balanced parts or that all possible cross-part arcs remain. For fixed $m$, its error is larger than the linear error of \Cref{thm:dir-arc-linear-error}, although it can give a smaller numerical bound at small orders.

\begin{proposition}[Quadratic bound and bipartisation]\label{prop:dir-arc-stability}
...
Equality in both $\min(a,b)\le\sqrt{ab}$ and Cauchy--Schwarz would require balanced in- and out-degrees and the same degree product at every vertex. Saturating the total budget would then give degrees of order $\sqrt{mn}$, incompatible with quadratic size for fixed $m$. The budget is a sum over vertices, not a bound of $(m-1)n$ on each individual product. \Cref{lem:two-step-budget} obtains its linear error by combining that sum with a lower bound on the minimum total degree, and deleting a vertex when that lower bound fails.
...
\section{The directed multigraph arc formula}\label{sec:dir-multi}

The exact formula has a linear branch from thickened bidirected trees and a quadratic branch from one-way complete bipartite digraphs. These constructions attain the lower bound. The upper proof uses the checked reachability-skeleton lemma.

Removing a reachability skeleton lowers every positive local arc-connectivity by at least one. Repeating the deletion controls the total multiplicity.
...
An upper bound needs a deletion that is guaranteed to lower every positive connectivity, paired with a bound on how much it removes. Deleting a vertex preserves feasibility, but need not lower every positive pairwise connectivity. Once multiplicities are free to sit anywhere between $0$ and $m-1$, no single vertex is guaranteed to carry a controllable share of the arcs. The fix is to stop insisting on a vertex and delete a whole \emph{subgraph} instead, and the reachability skeleton of \Cref{sec:reach-skeleton} is one: general enough to always exist, sparse enough to bound by \Cref{lem:reach-skeleton}, and, as the next lemma shows, specific enough that removing it lowers every positive pairwise connectivity by at least one.


\begin{lemma}[Deleting a skeleton lowers every positive connectivity]\label{lem:reach-delete}
...
\begin{corollary}[The directed multigraph value at $n = 7$, $m = 3$]\label{cor:dir-multi-n7}
$L_3^{\mathrm{dir}}(7) = 24$.
\end{corollary}

\begin{proof}
Substitute $m=3$ and $M(7)=12$ in \Cref{thm:dir-multi-full}.
\end{proof}

Neither historical computation settled this cell. CBC timed out when testing whether a feasible $25$-arc multigraph exists at $(n,m)=(7,3)$. A fourteen-hour enumeration was also stopped without a verdict. Those unfinished
runs do not independently validate the general formula.
...
\begin{proof}
For $M^{*}(n) \ge M(n)$, use either construction from
\Cref{const:dir-multi-lower} at multiplicity one. Both give $\{0,1\}$
matrices in which every pairwise maximum flow is at most one. Their total
weights are $2(n-1)$ and $\floor{n^2/4}$, respectively.
...
\begin{theorem}[The $m=2$ case collapses to the simple digraph]\label{thm:dir-multi-m2}
At $m=2$, that is at $k=1$, \Cref{thm:dir-multi-full} reads $L_2^{\mathrm{dir}}(n) = M(n)$, which is exactly the value \Cref{thm:dir-arc-m2-exact} states for the simple digraph. The agreement is forced rather than coincidental, so \Cref{thm:dir-multi-full} recovers \Cref{thm:dir-arc-m2-exact} as its $k=1$ case rather than needing it as a separate input.
\end{theorem}

\begin{proof}
A directed multigraph with $\lec^{\max} \le 1$ can carry no parallel arcs at all, since two parallel copies of one arc are already two arc-disjoint routes between their endpoints. At $m=2$ the multigraph problem and the simple digraph problem are therefore the same problem, and the two statements are two readings of one value. Both readings rest on \Cref{thm:dir-multi-full}, so the agreement is not independent confirmation of either statement.
\end{proof}

\subsection{Extremal classification on the quadratic branch}\label{sec:quadratic-uniqueness}

\Cref{thm:dir-multi-full} establishes the value at every $n$ and $m$, but a value is not a full description of the extremal digraphs. The question addressed here is which multigraphs attain it on the quadratic branch at a general $n$. The reachability-skeleton proof looks unpromising for this, since it is designed never to need to know which multigraph it is taking apart. But that indifference is only in the direction of the \emph{bound}. Run backwards, from the assumption that the bound is met exactly, equality in the same construction imposes three restrictions: it pins down the number of strongly connected components, then the shape of the skeleton, and finally the multigraph.
...
\begin{conjecture}[What equality forces, with proof,\aimedal]\label{lem:equality-forces}
Let $m\ge2$ and $n \ge 8$, so that $\floor{n^2/4} > 2(n-1)$ and $M(n) = \floor{n^2/4}$, and let $D$ be a directed multigraph on $n$ vertices with $\lec^{\max}_D \le m-1$ and $|A(D)| = (m-1)\floor{n^2/4}$. Then
\begin{enumerate}[label=(\arabic*)]
...
The next lemma is where the skeleton's minimality, which had only been used to make it small, is used to make it \emph{shallow}. This restricts the paths that can occur in an extremal skeleton.

\begin{conjecture}[A minimal bipartite skeleton has no long path, with proof,\aimedal]\label{lem:skeleton-shallow}
...
\begin{conjecture}[Extremal classification on the quadratic branch, with proof,\aimedal]\label{thm:dir-multi-uniqueness}
...
\begin{theorem}[Menger for multihypergraphs]\label{thm:menger-hyper}
For a multihypergraph $\HH$ and distinct vertices $u, v$, regard parallel hyperedge copies as distinct. The maximum number of hyperedge-copy-disjoint Berge $u$--$v$ paths equals the minimum number of individual copies whose deletion separates $u$ from $v$. Equivalently, if a cut $C$ is specified as a set of distinct hyperedge types, its cost is $\sum_{e\in C}\mu(e)$, because every copy of each selected type must be deleted. A simple hypergraph is the case where every multiplicity is one.
\end{theorem}
...
\begin{lemma}[Cut induction for multihypergraphs]\label{lem:hyper-cut-induction}
...
Let $r \ge 3$, $n \ge r$, and $2 \le m$ with $m-1 \le \binom{n-2}{r-2}$. Then the maximum number of hyperedges in a \emph{simple} $r$-uniform hypergraph on $n$ vertices with $\lec^{\max}\le m-1$ is exactly $\floorfrac{(m-1)(n-1)}{r-1}$.
\end{theorem}
...
\begin{construction}[Twelve hyperedge copies on six vertices]\label{const:multihyper-six}
Take a hub $h$ and five other vertices $a,b,c,d,e$. Include all ten triples
consisting of $h$ and two of the other vertices. Add a second copy of
$\{h,d,e\}$ and one copy of $\{a,b,c\}$. The resulting $3$-uniform
multihypergraph has $12$ hyperedge copies and $\lec^{\max}\le5$.
Consequently, the multihypergraph edge value at $(n,m,r)=(6,6,3)$ is exactly
$12$, even though $(r-1)$ does not divide $(n-1)$.
\end{construction}

\begin{proof}
Initially each non-hub vertex lies in four of the ten triples. The repeated
triple raises the degrees of $d$ and $e$ to five, while the final triple does
the same for $a$, $b$ and $c$. Every pair of distinct vertices therefore
includes a non-hub vertex whose five incident hyperedge copies form a cut
separating it from the other endpoint. Thus no pair has more than five
hyperedge-disjoint routes. The construction has $10+1+1=12$ copies, matching
the upper bound $\floor{5(6-1)/(3-1)}=12$ from \Cref{prop:hyper-edge}.
\end{proof}

The construction was derived after a longer unaided search found a
twelve-copy witness in this cell. Its optimality follows from the degree
cuts and the upper bound, not from an exhaustive search. In the
vertex problem it supplies a lower bound of $12$ by Whitney's inequality,
but the edge upper bound does not prove vertex optimality.

The sparse hypergraph used in the proof of \Cref{thm:simple-hyper-edge}
follows from a direct degree-smoothing argument.

\begin{lemma}[Sparse uniform hypergraphs exist]\label{lem:sparse-hypergraph}
Let $r$, $n$ and $d$ be integers with $r \ge 1$, $n \ge r$ and $0 \le d \le \binom{n-1}{r-1}$. There is a simple
...
because each non-hub vertex used in the construction has incidence degree one, while all hyperedges meet only at the hub. Thus no incidence cycle exists and the floor is attained.
\end{proof}
...
The simple construction uses distinct $(r-1)$-sets on the non-hub vertices. The condition $m-1\le\binom{n-2}{r-2}$ bounds the requested degree cap by the number of such sets containing a fixed vertex. A multihypergraph does not need distinct sets, because it
may repeat a hyperedge instead, and that closes a different range of cases.
...
vertex-disjoint routes cannot both use $h$ there, so $\kap(u,w) \le 1$. Hence $\kap^{\max}=m-1$ when at least one hyperedge is present. At $n=1$ the construction is empty and $\kap^{\max}=0$. In both cases it is feasible.
...
At $m=3$ the upper bound covers every multihypergraph, and equality follows in the construction range stated below. Both rest on the rank inequality proved next, whose engine is a bound for
graphs in which one side of a partition must stay below local
$3$-connectivity. The hypergraph bound then falls out of the incidence
...
Let $G$ be a connected loopless multigraph with a partition $V(G)=X \mathbin{\dot\cup} Z$ such that
...
$G$. No cycle can pass through two different blocks: the block-cut tree forces
any excursion out of a block to return through the same cut vertex,
which would repeat that vertex on the cycle. So the cycle rank adds
up over blocks, $\mathrm{rank}(G) = \sum_i \mathrm{rank}(B_i)$, and it is
enough to bound each $\mathrm{rank}(B_i)$ by $|X \cap B_i| - 1$ and sum.

Non-bridge blocks inherit the hypotheses. A bridge block is one edge, so its rank
...
forbids. Both torsos are smaller in $|V|+|E|$. Neither side can consist of a single vertex: in this simple graph it would have only $a$ and $b$ as possible neighbours, contradicting minimum degree three. Forming a torso therefore removes at least two vertices and at least three incident edges, while adding at most one vertex and two edges. This justifies the induction even in the fresh-vertex case.
...
That rank bound gives \Cref{thm:hyper-vertex-m3}, stated in
...
At $r = 2$ the theorem speaks of multigraphs, where the incidence convention
of \Cref{sec:incidence-convention} makes $q$ parallel edges $q$ distinct one-step
routes. It gives $K_3(n) = 2(n-1)$, the same number as the multigraph edge
value of \Cref{thm:multigraph-edge}, and \Cref{cor:multi-vertex-m3} below records
that specialisation. The proof of
...
Let $G$ be a multigraph with multiplicity function $\mu$ and underlying simple graph $G_0$. For any two vertices $u \ne v$, write $\pi(u,v)$ for the largest number of pairwise internally disjoint $u$--$v$ paths of length at least two in $G_0$. Then
\[
...
\begin{proof}
A route of length one is a single copy of the edge $uv$, and its interior is empty, so it is internally disjoint from every other route whatever the length. We prove the two inequalities separately.

\emph{Lower bound.} Take all $\mu(u,v)$ copies of $uv$, and take a maximum family $P$ of pairwise internally disjoint $u$--$v$ paths of length at least two in $G_0$, so $|P| = \pi(u,v)$. Lift each path of $P$ into $G$ by choosing one copy of each of its edges, which is possible because every edge of $G_0$ carries multiplicity at least one. Lifting leaves interior vertices unchanged, so the lifted routes are pairwise internally disjoint, and they are internally disjoint from the copies of $uv$ because those have empty interior. That is a family of $\mu(u,v) + \pi(u,v)$ pairwise internally disjoint routes, so $\kap_G(u,v) \ge \mu(u,v) + \pi(u,v)$.

\emph{Upper bound.} Let $F$ be any family of pairwise internally disjoint $u$--$v$ routes in $G$, and split it into the routes $F_1$ of length one and the routes $F_2$ of length at least two. The members of $F_1$ are distinct copies of the edge $uv$ and there are only $\mu(u,v)$ such copies, so $|F_1| \le \mu(u,v)$. Send each route of $F_2$ to its vertex sequence, a $u$--$v$ path of $G_0$ of length at least two. That map is injective on $F_2$: two routes with the same vertex sequence have the same interior, which is non-empty because the length is at least two, and $F$ is internally disjoint, so no two such routes can both belong to it. Distinct members of $F_2$ have disjoint interiors, so their images are pairwise internally disjoint paths of $G_0$ of length at least two, giving $|F_2| \le \pi(u,v)$. Hence $|F| \le \mu(u,v) + \pi(u,v)$, and taking $F$ maximum gives $\kap_G(u,v) \le \mu(u,v) + \pi(u,v)$.

The two bounds give the equality.
\end{proof}
...
\begin{theorem}[The multigraph incidence value at $m = 2$]\label{thm:multi-vertex-m2}
$K_2(n) = n-1$, attained exactly by the spanning trees.
\end{theorem}
...
\begin{remark}[Why $m = 2$ is not a multigraph question at all]\label{rem:multi-vertex-m2-simple}
The proof shows more than the count. At $m = 2$ the cap $m-1$ is one, so a second copy of any edge is a second route on its own, and every feasible multigraph is simple. The multigraph problem is therefore the simple problem, $K_2(n) = k_2(n) = n-1$, which is also what \Cref{thm:hyper-vertex-m2} says at $r = 2$. This is the one value of $m$ at which the two coincide for a reason rather than by accident, and \Cref{cor:dir-multi-incidence} makes the same observation in the directed model.
\end{remark}

The next value is the one the hypergraph theorem supplies, and no separate argument is needed for it.

\begin{corollary}[The multigraph incidence value at $m = 3$]\label{cor:multi-vertex-m3}
$K_3(n) = 2(n-1)$ for every $n \ge 2$.
\end{corollary}

\begin{proof}
Apply \Cref{thm:hyper-vertex-m3,prop:hyper-vertex-lower-multi} at $r=2$.
The divisibility condition is automatic, and the doubled star attains the
upper bound $2(n-1)$.
\end{proof}

At $m = 3$ this is the multigraph \emph{edge} value $L_3(n)$ of \Cref{thm:multigraph-edge} as well. The first threshold at which the two values are known to differ is $m = 5$: $K_5(4) = 14$ against $L_5(4) = 12$, and $K_5(5) = 19$ against $L_5(5) = 16$, both by exhaustion. At $m = 4$ the completed block computation gives $g_4(b) = 3(b-1)$ throughout the range of \Cref{tab:multi-vertex-blocks}. Turning these block scores into values of $K_4(n)$, even within the tested block range, depends on the unchecked block reduction; agreement for every $n$ remains open. For $m \ge 3$ at least three constructions compete, and which one wins depends on how $n$ compares with $m$, exactly as in every other variant of this thesis.
...
\begin{itemize}
    ...
    \item \emph{The thickened complete graph.} Give every edge of $K_n$ the same multiplicity $q$. Here $\pi(u,v) = n-2$, so feasibility asks $q \le m+1-n$, and the count is $\binom{n}{2}(m+1-n)$, positive only when $n \le m$. At $n=m+1$ the multiplicity is zero and the construction is empty.
...
The small-$n$ regime $n \le m+1$, where the theta construction is feasible, is tempting to read as the \emph{only} place a cycle pays for itself, with the thickened tree optimal once $n \ge m+2$. It is not: a single triangle grafted anywhere onto the thickened tree already beats it, for every $m \ge 5$ and every such $n$, which the next conjecture makes precise.
...
\]
The obvious guess is that cycles must always pay for themselves against this bound: each independent cycle costs $m-1$ units of route capacity. The next conjecture shows a single triangle already fails to pay, for $m \ge 5$, and packing many of them compounds the failure into a gap that grows with $n$.

\begin{conjecture}[A bouquet of thickened cliques, and how it beats the thickened tree, with proof,\aimedal]\label{thm:clique-chain-vertex}
Fix $3 \le r \le m$, the order of the clique block rather than the hyperedge uniformity of \Cref{tab:notation}, and set $q = m+1-r \ge 1$, the multiplicity of a thickened $K_r$ that saturates the vertex cap on $r$ vertices (the ``thickened complete graph'' construction above, restricted to a block of size $r$ rather than all of $n$). Let $1 \le k \le \lfloor (n-1)/(r-1) \rfloor$. Take $k$ copies of $K_r$ that all share one common vertex and are otherwise disjoint, thicken every edge of every copy to multiplicity $q$, and attach any leftover vertices as a pendant path at multiplicity $m-1$. This multigraph is feasible, $\kap^{\max} \le m-1$, and its total multiplicity is exactly
\[
...
\emph{Feasibility.} Different clique copies and pendant edges meet only at their attachment vertices. Whenever such an attachment separates two non-empty pieces, it is a cut vertex. So a path of length $\ge 2$ between two vertices of the same block cannot leave that block and come back: to do so it would have to pass through the shared vertex twice. Hence for an edge $uv$ inside a block, every route counted by $\pi(u,v)$ stays inside its $K_r$, where deleting $uv$ leaves $u,v$ joined by exactly $r-2$ pairwise internally disjoint two-step routes, one through each of the other block vertices, and by Menger no more, since those same $r-2$ vertices are a cut of that size. So $\pi(u,v) = r-2$ and, by \Cref{lem:multi-vertex-split}, $\kap_G(u,v) = q + (r-2) = m-1$ exactly, never over. For $u,v$ in different blocks, or non-adjacent with either endpoint on the pendant path, a single cut vertex separates them, so $\kap_G(u,v) \le 1$. If $u,v$ are adjacent along the pendant path, then the edge lies on no cycle, and \Cref{lem:multi-vertex-split} gives $\kap_G(u,v)=\mu(u,v)+\pi(u,v)=(m-1)+0=m-1$.
...
At $r = 3$, $\mathrm{gain}(3,m) = m-4$: zero at $m = 4$, matching the absence of counterexamples at the tested orders for $m \le 4$, and strictly positive for every $m \ge 5$. So the guess that the thickened tree is optimal once $n \ge m+2$ is false for every $m \ge 5$: it held only within the reach of the exhaustive table, $m \le 4$. The smallest witness is small enough to check by hand. Take $m = 5$ and $n = 7$, the first size the guess covers. Put a triangle on $\{u_1, u_2, u_3\}$ at multiplicity $3$ and hang a path of four further vertices off $u_1$ at multiplicity $4$. Two triangle vertices have three parallel copies plus one route through the third vertex, so $\kap = 4$, and every other pair is separated by a single cut vertex or joined only by its own parallel copies, so nothing exceeds $4 = m-1$. The total multiplicity is $3 \cdot 3 + 4 \cdot 4 = 25$, one more than the $(m-1)(n-1) = 24$ the guess allows. Packing $k$ blocks instead of one multiplies the excess by $k$, and $k$ may be taken as large as $\lfloor (n-1)/(r-1) \rfloor$, so the gap over the thickened tree grows linearly in $n$.
...
Optimising $r$ widens the gap further. Treated as continuous, the gain per added vertex, which is $\mathrm{gain}(r,m)/(r-1)$, peaks near $r \sim (m+1)/2$, where it is $\Theta(m^2)$. Subject to the proposed construction, packing blocks of that size would give $K_m(n) \ge \bigl((m-1) + \Theta(m^2)\bigr)(n-1-O(m))$. In particular the per-vertex rate is quadratic in $m$ once $n/m\to\infty$, a different order from the tree's linear rate.

The clique bouquet need not be optimal. At $m = 5$, $n = 7$ the best clique bouquet uses three triangles and gives
$27$ and the block computation of \Cref{tab:multi-vertex-blocks} gives $29$.
The proposed construction would give a quadratic-in-$m$ lower bound once many
blocks fit. A matching upper bound would require the next, unchecked
proposition. It comes from the underlying simple graph rather than cycle rank.

\begin{conjecture}[An upper bound of the same order, with proof,\aimedal]\label{prop:multi-vertex-upper}
For all $m \ge 2$ and $n \ge 2$,
\[
    K_m(n) \;\le\; (m-1)\,k_m(n) \;<\; 2(m-1)^2\,n .
\]
\end{conjecture}
...
If the proposed lower construction and the proposed upper bound of
\Cref{prop:multi-vertex-upper} are both eventually verified, then first letting $n/m\to\infty$ so that optimised blocks
can be packed with only $O(m)$ unused vertices, and then letting $m\to\infty$,
would yield
\[
...
so the proposed result would give $K_m(n) = \Theta(m^2 n)$ in that large-$n$
regime. This qualification is necessary: for example $K_m(2)=m-1$, so no
estimate of order $m^2n$ can hold uniformly when $n$ is fixed. The bouquet of
\Cref{thm:clique-chain-vertex} has the right joint order once many blocks fit,
conditional on that upper bound. \Cref{thm:multi-vertex-bipartite} below
proposes a stronger lower constant. The upper-bound argument is crude, since
it throws away the $-\sum \pi$ correction entirely and charges every edge the
full $m-1$. The first inequality, $K_m(n) \le (m-1)k_m(n)$, is the more
informative half. It says that this variant can never outrun the simple vertex
problem by more than the multiplicity cap, which ties the least understood of
the sixteen to the one with the longest history. It is tight at $m = 2$, where
both sides read $n-1$ by \Cref{rem:multi-vertex-m2-simple}.

\subsection{The value is a block problem}\label{sec:multi-vertex-blocks}

The lower bounds above are constructions and the upper bound is a density count, and neither says what an extremiser looks like. The proposed reduction would turn this into an optimisation over $2$-connected graphs and single-edge blocks: the number of vertices in play stops being $n$ and becomes the size of a single block.

Two observations do it. The first is already in the section, one line further than it was taken.

\begin{conjecture}[The objective in closed form, with proof,\aimedal]\label{lem:multi-vertex-objective}
...
\[
    K_m(n) \;=\; \max\bigl\{\, W_m(G_0) \;:\; G_0 \text{ simple on } n \text{ vertices},\ \kap^{\max}_{G_0} \le m-1 \,\bigr\} .
\]
\end{conjecture}
...
\begin{conjecture}[The multigraph incidence problem is a knapsack over blocks, with proof,\aimedal]\label{thm:multi-vertex-blocks}
...
If verified, the reduction would change what has to be searched: enumerate $2$-connected simple
graphs, evaluate $W_m$, and solve the resulting knapsack. A sweep through eight
vertices gives \Cref{tab:multi-vertex-blocks} and agrees with the independent
exhaustive routine wherever both finish. It runs from a standalone script,
\code{program/scripts/multi\_vertex\_blocks.py}, and its retained
transcript is \path{program/logs/multi_vertex_blocks_log.txt} in the repository named
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}r rrrrrrr@{}}
    ...
    \bottomrule
  \end{tabular}
  \caption[Best score of one block]{$g_m(b)$, the largest $W_m$ a single
  $2$-connected block on $b$ vertices can score, by exhaustion over every
  $2$-connected graph of that order. A dash marks a size at which no feasible
  block exists, which at $m=2$ is every $b \ge 3$. Under the proposed reduction,
  feeding this row into the knapsack of \Cref{thm:multi-vertex-blocks} would give $K_m(n)$ for $n \le 8$. At
  $m=5$, $n=7$ it returns the $29$ quoted above, against the $27$ that
  \Cref{thm:clique-chain-vertex} reaches. The per-vertex rate $g_m(b)/(b-1)$ is
  still rising at $b=8$ in every row from $m=5$ on, while at $m=3$ it falls and
  at $m=4$ it is flat, so this table does not extrapolate: a larger $n$ needs
  larger blocks enumerated.}
  \label{tab:multi-vertex-blocks}
\end{table}

One cell beyond the table is known. An exhaustive sweep of the $2$-connected
graphs on nine vertices with at least fourteen edges gives $g_6(9) = 54$, attained by a block with fourteen
edges, run from the standalone script
\code{program/scripts/multi\_vertex\_blocks\_b9.py}. A single block of that order
beats every split of the same budget, the best of which reach $52$, so
$K_6(9) = 54$ would follow from the proposed block reduction. Supports with
at most thirteen edges cannot improve the score: each edge contributes at
most $m-2=4$, so their score is at most $52$.
The rate $54/8$ exceeds the $47/7$ of $b=8$, which is the $m=6$
row still rising at $b=9$ exactly as the caption warns.

Within the tested range $2\le m\le8$, $5\le b\le8$, no winning block is complete.
The recorded representatives often have a few vertices of high degree beside
many at degree two or three, but this is not a universal degree pattern:
at $m=8$, $b=5$, the unique maximiser has degrees $(4,4,4,3,3)$.
At $m = 3$ the feasible blocks with $b\ge3$ are the cycles, every degree two. What
the sweep does \emph{not} show is that they are complete bipartite graphs with a
few edges added. That description fits at $m = 7$ and $m = 8$, where $K_{3,4}$
or $K_{3,5}$ with two further edges inside the side of size three attains the
maximum at $b = 7$ and $b = 8$. It is the unique maximiser at three of those
four cells and one of six at $m = 8$, $b = 7$, where two of the six carry the
shape. The description fails at $m = 4$, $5$ and $6$, where from $b = 7$ onwards
the maximum is attained by many non-isomorphic blocks at once, up to $38$ of
them, and across those six cells exactly one tied block is complete bipartite,
the $K_{2,5}$ at $m = 6$, $b = 7$, which scores the $st(m-s) = 40$ of the family
below at $s = 2$, $t = 5$. So the family of \Cref{thm:multi-vertex-bipartite}
below is a construction suggested by the largest $m$ in reach, and it is a lower
bound rather than a description of the extremisers. The script prints each maximiser's
\code{graph6} string, its degree sequence and the count of blocks tied with it,
so the reader can check which of these cells is which.

\begin{conjecture}[Thickened complete bipartite blocks, with proof,\aimedal]\label{thm:multi-vertex-bipartite}
...
\[
    K_m(n) \;\ge\; \frac{st\,(m-s)}{s+t-1}\,(n-1) .
\]
...
\emph{The connectivities of $K_{s,t}$.} For $u \ne u'$ in $S$, the $t$ two-step routes through distinct vertices of $T$ are internally disjoint. They give $\kap(u,u') \ge t$. Every route must also meet $T$, which is a cut of size $t$, so $\kap(u,u') = \pi(u,u') = t$. Longer routes may exist but cannot increase this maximum. Symmetrically $\kap(v,v') = s$ for $v \ne v'$ in $T$. For adjacent $u \in S$ and $v \in T$, delete the edge $uv$. The sets $T \setminus \{v\}$ and $S \setminus \{u\}$ both separate $u$ from $v$, so $\pi(u,v) \le \min(s-1, t-1) = s-1$. Choose distinct $u_i \in S \setminus \{u\}$ and distinct $v_i \in T \setminus \{v\}$ for $1 \le i \le s-1$, possible because $s \le t$. The routes $u, v_i, u_i, v$ attain the bound. Thus $\pi(u,v) = s-1$ and $\kap(u,v) = s$.
...
the cap $m-1$. The finite sweep of \Cref{tab:multi-vertex-blocks} shows that
adding a few edges inside the small side improves this clean family at $m = 7$
and $m = 8$, and that at $m = 4$, $5$ and $6$ the extremal blocks are mostly
of other shapes, the $K_{2,5}$ tying at $m = 6$, $b = 7$ being the one that is
not. Under the proposed block reduction the answer becomes a block optimisation
problem. If the proposed lower and upper bounds are verified, then in the regime
$m\to\infty$ with $n/m\to\infty$ the construction gives
$K_m(n)/(m^2n) \ge 3-2\sqrt2-o(1)$ and the upper bound gives
$K_m(n)/(m^2n)<2$. These inequalities do not establish that this ratio has a
limit. Its possible limiting value and the blocks needed for sharper bounds
remain open here.

\subsection{The directed problem}\label{sec:multi-vertex-dir}

Everything above splits the measure into copies and detours, and that split does not
use the absence of orientation. The proposed splitting identity also has a directed version, the same one with
directed routes. The checked corollary below separately gives the exact value at
$m=2$ and the leading term for every fixed $m$.

\begin{conjecture}[Splitting the directed vertex measure, with proof,\aimedal]\label{lem:multi-vertex-split-dir}
Let $D$ be a multidigraph with multiplicity function $\mu$ and underlying simple
digraph $D_0$. For an ordered pair $u \ne v$, write $\pi(u,v)$ for the largest number
of pairwise internally disjoint directed $u$--$v$ paths of length at least two in
$D_0$. Then $\kap_D(u,v) = \mu(u,v) + \pi(u,v)$, and consequently
\[
    K_m^{\mathrm{dir}}(n) \;\le\; (m-1)\,k_m^{\mathrm{dir}}(n) .
\]
\end{conjecture}

\begin{proof}[Proposed proof]
A directed $u$--$v$ route of length one is a parallel copy of the arc $uv$ and has empty
interior, and no step of \Cref{lem:multi-vertex-split} uses the absence of orientation,
so its proof applies with directed routes throughout. The lower bound lifts a maximum
family of internally disjoint directed $u$--$v$ paths of length at least two out of
$D_0$ and adjoins the $\mu(u,v)$ copies. The upper bound splits a family into copies of
the arc $uv$ and routes of length at least two, the latter mapping injectively to
directed paths of $D_0$ because two routes with the same vertex sequence share an
interior that is non-empty. Note that a
directed $u$--$v$ path uses no copy of the arc $vu$, so the reverse multiplicity plays
no part.

For the bound, let $D$ be feasible. The identity gives $\mu(u,v) \le \kap_D(u,v) \le m-1$
on every ordered pair, so the total multiplicity is at most $(m-1)|A(D_0)|$. And $D_0$
is itself feasible: on an arc, $\kap_{D_0}(u,v) = 1 + \pi(u,v) \le \mu(u,v) + \pi(u,v)
\le m-1$, and on a pair with no arc from $u$ to $v$, $\kap_{D_0}(u,v) = \pi(u,v) \le m-1$
directly. So $|A(D_0)| \le k_m^{\mathrm{dir}}(n)$ by definition.
\end{proof}

\begin{corollary}[The directed multigraph incidence value]\label{cor:dir-multi-incidence}
For every $n \ge 2$,
\[
    K_2^{\mathrm{dir}}(n) \;=\; k_2^{\mathrm{dir}}(n) \;=\; \ell_2^{\mathrm{dir}}(n)
    \;=\; \max\!\Bigl(2(n-1),\ \floorfrac{n^2}{4}\Bigr) ,
\]
and for each fixed $m \ge 2$,
\[
    (m-1)M(n) \;\le\; K_m^{\mathrm{dir}}(n)
    \;\le\; (m-1)\floorfrac{n^2}{4} + 4(m-1)^2(n-1) ,
\]
so that $K_m^{\mathrm{dir}}(n) = (m-1)n^2/4 + O_m(n)$ and the bipartite digraph of
\Cref{thm:dir-multi-full} is asymptotically optimal for this variant too.
\end{corollary}

\begin{proof}
At $m=2$, two parallel arcs would themselves give two internally disjoint
routes, so every feasible multidigraph is simple. The exact value therefore
follows from \Cref{thm:dir-vertex-m2-exact,thm:dir-arc-m2-exact}.
For general $m$, \Cref{thm:dir-multi-full} and $\kap\le\lec$ give the lower
bound, while \Cref{thm:dir-hyper-constant} at $r=2$ gives the upper bound.
Since $M(n)=\floor{n^2/4}$ for $n\ge7$, these bounds imply the stated asymptotic.
\end{proof}

The exhaustive values agree and stay strictly inside these bounds where both are known:
$K_6^{\mathrm{dir}}(3) = 24$ against the lower bound $(m-1)M(3) = 20$, so the arc
extremiser read through $\kap \le \lec$ is a lower bound for the incidence problem and
not its value. What beats it at that size is a block, and \Cref{prop:dir-multi-vertex-blocks} proposes a corresponding block argument for directed routes.

\begin{construction}[Thickened complete digraph, and its bouquet]\label{const:dir-multi-vertex-blocks}
For $2 \le b \le m$, take the complete digraph on $b$ vertices and give every arc
multiplicity $m+1-b$. At $b=2$ this is one bidirected edge at multiplicity $m-1$.
Hanging $k$ such blocks off one shared vertex, pairwise disjoint otherwise, uses
$k(b-1)+1$ vertices, and blocks of different sizes may be mixed.
\end{construction}

\begin{conjecture}[The directed incidence value is a block problem too, with proof,\aimedal]\label{prop:dir-multi-vertex-blocks}
Let $D$ be a multidigraph with underlying simple digraph $D_0$, and take the blocks of
the underlying undirected graph of $D_0$. Then $\kap_D$ agrees with the value measured
inside a block on every pair joined by an arc, and is at most $1$ on every pair split
between two blocks, so the count of \Cref{lem:multi-vertex-split-dir} is additive over
blocks exactly as in \Cref{thm:multi-vertex-blocks}. Each block of
\Cref{const:dir-multi-vertex-blocks} is feasible and carries $b(b-1)(m+1-b)$ arcs, so
\[
    K_m^{\mathrm{dir}}(n) \;\ge\; \max\Bigl\{\, \sum_i b_i(b_i-1)(m+1-b_i)
      \;:\; 2 \le b_i \le m,\ \sum_i (b_i - 1) \le n-1 \,\Bigr\} .
\]
\end{conjecture}

\begin{proof}[Proposed proof]
For the block structure, let $u,v$ be joined by an arc of $D_0$ and let $B$ be the block
of the underlying undirected graph that contains it. A directed $u$--$v$ path of $D_0$
leaves $B$ only by crossing a cut vertex, and returning crosses that same vertex a
second time, which no path does. So every route counted by $\pi(u,v)$ stays inside $B$.
For two vertices that lie in no common block, either no path joins them or a
cut vertex on the intervening block chain separates them. Every directed
route must pass through that vertex, giving $\kap_D \le 1$ there. The two
blocks need not themselves share a vertex.

For one block, the internally disjoint directed $u$--$v$ routes of length at least two
in the complete digraph on $b$ vertices have pairwise disjoint interiors drawn from the
other $b-2$ vertices, so there are at most $b-2$ of them, and the two-step routes attain
that. Hence $\pi(u,v) = b-2$, and \Cref{lem:multi-vertex-split-dir} gives
$\kap_D(u,v) = (m+1-b) + (b-2) = m-1$ on each of the $b(b-1)$ ordered pairs, which sits
at the cap. The count is $b(b-1)(m+1-b)$. Attaching the blocks at one shared vertex uses
$1 + \sum_i (b_i-1)$ vertices, and any spare vertices are left isolated.
\end{proof}

The blocks and the arc extremiser lead on different ranges, and the grids of
\Cref{sec:variant-grids} plot whichever is larger. At $m=3$ the best block is the
bidirected edge, the bouquet is the thickened bidirected tree, and the arc extremiser
takes over from $n=8$. At $m=6$ the best block has three or four vertices, the bouquet
leads through $n=8$ at $84$ arcs against the extremiser's $80$, and the extremiser leads
from $n=9$.
...
$n$, so the extremal value is $\Theta_{r,m}(n^2)$.
\end{proposition}

\begin{proof}
Upper bound: a hyperedge with tail $u$ serves the $r-1$ ordered pairs
$(u, h)$, $h$ a head, so each hyperedge contributes $r-1$ tail-head pairs and
the actual number of such pairs is
\[
    (r-1)|E|.
\]
If some ordered pair $(u, v)$ were served by $m$ distinct hyperedges, those
would be $m$ hyperedge-disjoint one-step routes, so for any ordered pair
$(u,v)$ at most $m-1$ hyperedges can connect $u$ directly to $v$. Since there
are $n(n-1)$ ordered pairs, the maximum possible number of tail-head pairs is
\[
    (m-1)\,n(n-1).
\]
Therefore
...
\begin{construction}[Bounded outgoing hyperedge count]\label{const:bounded-outdegree-hyper}
For $n\ge r\ge2$ and $m\ge2$, give every vertex
$d=\min\{m-1,\binom{n-1}{r-1}\}$ distinct outgoing forward hyperedges.
Their head sets may be any $d$ distinct $(r-1)$-subsets of the other vertices.
This gives a simple directed hypergraph with $nd$ hyperedges, feasible for
both route separations. If repeated hyperedges are allowed, give every
vertex $m-1$ copies of any one outgoing hyperedge instead, for a total of
$n(m-1)$ copies.
\end{construction}

\begin{proof}
Every directed route from $u$ must begin with a hyperedge whose tail is $u$.
Deleting all outgoing copies therefore separates $u$ from every other
vertex. There are at most $m-1$ such copies, so
$\lec(u,v)\le m-1$ for every ordered pair. The vertex-separated measure is
no larger because those routes must also be hyperedge-disjoint.
Different tails define different forward hyperedges, which gives the
stated counts and ensures simplicity when the head sets are distinct.
\end{proof}

This linear construction can beat the two-layer construction at small
orders. For example, at $n=m=6$ and $r=3$ it gives $30$ hyperedges even in
the simple model. The construction markers take the larger of the two
counts. Neither count is asserted to be optimal in general.
...
\begin{proof}
Reversal is an involution on directed hyperedges. It maps every directed
$u$--$v$ Berge path bijectively to a $v$--$u$ path using the same edge copies
and the same interior vertices. It therefore preserves both kinds of
disjointness, all maximum local connectivities and the hyperedge count.
\end{proof}
...
for both models. Their exact finite values may still differ.
...
has a complete one-directional bipartite part and only $O_m(n)$ additional internal arcs. There, \Cref{thm:dir-arc-linear-error}
establishes that the bipartite pattern is right to first order, that no
...
The argument of \Cref{thm:dir-arc-linear-error} needs a different route-counting step here.
For digraphs, two-step routes through distinct midpoints were
...
$t \in T$ to each such hyperedge containing it, and form a maximal matching $M$ in it by repeatedly adding an edge whose two endpoints are still unmatched. Here \emph{maximal} means that no further edge can be added. Finding a matching of largest possible size is unnecessary.
...
since it is $\lec$-feasible and $\kap \le \lec$. For fixed $r\ge3$, the simplicity condition of \Cref{prop:dir-hyper-first} holds for large $n$, so the same leading term is attained by simple hypergraphs. At $r=2$ and $m\ge3$, repeats are needed for this leading constant, and the simple digraph value instead has leading term $n^2/4$. (The integer maximum is
...
carries a constant of $4(m-1)^2$, and the theorem above is an asymptotic
improvement rather than a uniform one. Second, at $r = 2$ the shadow is the
digraph itself and $\mathrm{cod}$ is the multiplicity, so the theorem above
recovers the quadratic branch of \Cref{thm:dir-multi-full} up to its own linear
error term, which is the consistency check one wants, though it does not
establish the exact value that \Cref{thm:dir-multi-full} states there.
...
The repair is to take a maximal family and ask what stopped it. Here maximal
means that no further route can be added while preserving the required disjointness.
It need not have the largest possible size. Every target it
...
its hyperedges are pairwise distinct. Build such a family greedily until no route with an unused target and unused hyperedges can be added. Let $\mathcal{F}$ be the resulting maximal family, of size $M$, serving the target set $S$, and let $U$ be the hyperedges it uses, so
...
$|\mathcal{T}|/\kap(u,v)$ above $r-1$, the forward model's own constant. That search runs from the standalone script
\code{program/scripts/general\_orientation\_check.py}. Nothing
...
constant. The second point is what the theorem above does to the
orientation axis of \Cref{sec:orientation-models}. Forward and backward are
equal, exactly, by \Cref{lem:dir-hyper-duality}. Without it the general
model would sit only between the same two bounds a factor of four apart, which
leaves room for it to be denser to first order. The theorem rules that out:
all three orientation models share the same leading
term, and the computed differences between them at $n \approx r$ are a
small-size effect that the asymptotics cannot see.
...
connectivity over all pairs, and the same discussion applies with $\kap^{\max}$
under vertex separation. A \emph{capped predicate} is the yes-or-no test
...
repository at \url{https://github.com/louisvandenbruwaene/thesis}, which
serves this document and the \code{program/} directory it refers to.
The source revision recorded for this PDF build is \thesiscommit. Before an
unrecorded working build, the revision marker is reset to ``not recorded''.
For a recorded build, \code{record\_revision.sh} is run after committing the
sources, so the printed tag identifies the source snapshot.
Everything this thesis refers to sits in one directory, \code{program/}: the
unified program and its test suite, the standalone scripts behind the block
sweeps and the conjecture checks (\code{program/scripts/}), the recorded machine
values (\code{program/data/}) and the run transcripts (\code{program/logs/}).
This document and that directory are the whole of it.

Three commands cover the main audit. Run them from \code{program/}.
The first performs the principal checks, the second runs the regression suite
and the third regenerates the computational figures.
...
The four variant grids require several hundred \code{solve} calls. Their values
are stored in \code{program/data/machine\_values.json}. This readable file records
what each run returned. It lets the grids redraw in seconds. It also ensures
that the document uses the recorded points rather than the outcome of a new
timed search.

Rendering and recomputing are separate operations. The third command draws
only the figures and tables used by this thesis, using the frozen observations
and stated formulas. It does not run searches or refresh archived experiments.
A missing observation is an error, not permission to compute a replacement.
Passing \code{-{}-rebuild} performs a new experiment and writes a candidate
file beside the published record. Passing \code{-{}-compare} lists the
differences. \code{-{}-promote} replaces the record only after review.
Promotion is refused if two completed exact values disagree.
Such a disagreement is a defect to investigate, not a timing fluctuation.

For new rebuilds, each run records its method, requested and elapsed time,
completion status and witness, together with source and environment information.
The source fingerprint now covers the full program, the C source and the
experiment driver. The earlier chapter-four cutoff omitted flow helpers
defined later in the program. The historical observations below predate these
additional per-run fields. Their missing information cannot be reconstructed
by editing metadata, so it remains unknown.

The block sweep is independent of that file and of the main program. Its
self-contained implementation is
\code{program/scripts/simple\_vertex\_blocks.py}. It requires nauty's
\code{geng} on the path. From \code{program/}, the quoted range is reproduced
by \code{BMAX=9 ../.venv/bin/python3 scripts/simple\_vertex\_blocks.py}. The recorded
environment used nauty 2.9.3.

The three commands name the virtual environment that holds the dependencies. It
is machine-specific and is not included in the repository.
Starting from Python 3.14.6, create one at the repository root with
\code{python3 -m venv .venv}, then run
\code{.venv/bin/python3 -m pip install -r program/requirements-lock.txt}.
The required dependency ranges are listed in
\code{program/requirements.txt}, and \code{program/requirements-lock.txt} pins
the exact verified versions. They are Python 3.14.6, NumPy 2.4.6,
SciPy 1.18.0, and PuLP 3.3.2. The two optional roles use Matplotlib 3.11.0 and
NetworkX 3.6.1. Installing from the lock file reproduces that environment.

\subsection{Equal-budget search records}\label{sec:equal-budget-details}

The protocol is stated in \Cref{sec:equal-budget}. Its raw records are in
\path{program/data/equal_budget_2026-09-06/}. The original checkpoints are in
\code{trials/} and timing replacements are in \code{replacements/trials/}.
Both sets include full witnesses. The replacement manifest records which
original record each new trial replaces and the clock change used to run it.
...
The report separates original outcomes from replacement outcomes and names
the selected record for every seed. An independent validator rebuilds the
flow networks from the saved witnesses using a separate gate construction
and NetworkX's implementation of the Edmonds--Karp algorithm. It calls neither the thesis
connectivity checker nor its compiled helper. Agreement is a cross-check of
feasibility and counts, not a proof of search optimality.

The construction comparison used in the tables is frozen separately in
\path{manuscript_baseline.json}. It includes constructions added during the
audit and is distinct from the earlier comparison snapshot in
\path{comparison_baseline.json}. Neither file is read by the search runner.
The historical observations in \Cref{sec:variant-grids} remain unchanged.

From \code{program/}, the reporting commands are:
...
\end{lstlisting}
The table renderer refuses incomplete timing coverage, unvalidated selected
witnesses or a search count above a recorded upper bound. Its provenance file
records the hashes of the report, comparison snapshot and witness audit.

Each row of \Cref{tab:equal-budget-m3-graphs,tab:equal-budget-m6-graphs,tab:equal-budget-m3-hypergraphs,tab:equal-budget-m6-hypergraphs} contains exactly three timing-qualified runs. Search counts
are printed without adjustment, even when a supplied construction is better.
\begin{table}[p]
\centering\scriptsize
\input{figures/equal_budget_m3_graphs.tex}
\caption{Equal-budget graph results at $m=3$. Each row receives three
$150$-second searches. Construction counts are supplied lower bounds, not
search inputs or assertions of optimality.}
\label{tab:equal-budget-m3-graphs}
\end{table}
\begin{table}[p]
\centering\scriptsize
\input{figures/equal_budget_m6_graphs.tex}
\caption{Equal-budget graph results at $m=6$, under the same protocol as
\Cref{tab:equal-budget-m3-graphs}.}
\label{tab:equal-budget-m6-graphs}
\end{table}
\begin{table}[p]
\centering\scriptsize
\input{figures/equal_budget_m3_hypergraphs.tex}
\caption{Equal-budget rank-three hypergraph results at $m=3$. Directed
hypergraphs use the forward convention. Each row receives three
$150$-second searches and construction counts are reported separately.}
\label{tab:equal-budget-m3-hypergraphs}
\end{table}
\begin{table}[p]
\centering\scriptsize
\input{figures/equal_budget_m6_hypergraphs.tex}
\caption{Equal-budget rank-three hypergraph results at $m=6$, under the same
protocol as \Cref{tab:equal-budget-m3-hypergraphs}.}
\label{tab:equal-budget-m6-hypergraphs}
\end{table}
\clearpage

\subsection{Detailed values across the sixteen variants}\label{sec:variant-grids}

\Cref{fig:variant-bounds-m3-graphs,fig:variant-bounds-m3-hyper,fig:variant-bounds-m6-graphs,fig:variant-bounds-m6-hyper} record the finite data behind \Cref{fig:variant-tree-status,tab:summary}.
Every hypergraph panel and comparison table in this section uses $r=3$.
Directed hypergraphs use the forward orientation, with one tail and two heads.
A blue curve represents an established value or upper bound, as specified in its panel. A red curve represents a conjectured formula. An upper-bound curve does not assert attainment.
A filled square is a value established by completed enumeration. The directed multigraph arc formula is drawn as an established curve on the strength of \Cref{thm:dir-multi-full}, not of an independent enumeration.
An open circle is the value returned by the unaided search.
A red plus is the value of a separately supplied construction.
Both give lower bounds, but they answer different questions: the circle measures
what the program found, while the plus records what is known to be feasible
independently of that run. The search is not raised to the construction,
to a completed enumeration or to a value found at another order or in another
variant. A downward step in the circles can therefore reflect a weaker timed
run. It does not mean that the true extremal function decreases.
Open cases have no interpolated curve.
This keeps parity effects in the discrete data from looking like a proposed formula.

\paragraph{Budgets and the limits of the record.}
These are historical experiments, not a controlled equal-time benchmark.
Every matrix search starts from the empty graph and uses tabu search.
Every hypergraph search uses randomised greedy growth. The figure driver uses
initial seed zero. Most search cases were allocated $0.4$ seconds each.
The open simple undirected vertex panel and open undirected hypergraph vertex
panels use $4$ seconds, as do all directed hypergraph panels.
Here a \emph{case} means one fixed variant, $n$ and $m$, not a whole row of sizes.
The comparison tables below print the applicable limit on every search row.

Enumeration has a separate budget: $60$ seconds per case by default,
$1800$ seconds for simple directed graphs and $120$ seconds for multigraph
vertex separation. Enumeration stops sampling a panel after its first timeout.
This is a sampling rule, not a proof that every larger case would also time out.
The standalone computation at $(m,n,r)=(6,6,3)$ is a separate experiment.

The original record saves values and requested budgets, but not each run's
elapsed time or witness. Its limits are cooperative stopping targets:
a connectivity check already in progress can finish after the deadline.
Consequently, neither exact one-hour runs nor equal computational effort can
be inferred from these data. Fixed seeds do not make timed runs repeatable
across different machines or loads. The frozen file makes the \emph{reported
observations} reproducible. Rerunning the searches is a new experiment.

\code{program/data/search\_evidence.json} exports the raw search and supplied
construction values in distinct fields for every plotted order. Missing
historical elapsed times and witnesses are explicitly marked as unknown.
\Cref{tab:search-evidence-m3-graphs,tab:search-evidence-m3-hypergraphs,tab:search-evidence-m6-graphs,tab:search-evidence-m6-hypergraphs} distinguish the same sources numerically.
A construction entry of zero means that the selected formula supplies no
positive lower bound in that parameter range, not that the optimum is zero.

\begin{sidewaysfigure}[p]
    \centering
    \includegraphics[width=0.98\textheight]{figures/variant_bounds_m3_graphs.png}
    \caption{The eight graph-model variants at $m=3$. The simple directed arc and vertex panels show verified lower bounds and completed exact values only, with no formula curve at all. The construction marks assert feasibility, not optimality.}
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The eight hypergraph-model variants at $m=3$. The blue curves are the established upper bounds of \Cref{prop:hyper-edge,thm:hyper-vertex-m3}. They remain upper bounds outside the stated attainment ranges. A lower-bound point below it does not alone prove that the bound is unattainable. Only an exact square below it establishes such a gap.}
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The eight graph-model variants at $m=6$. Fewer exact squares appear because exhaustive search reaches fewer vertices as the feasible range grows.}
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The eight hypergraph-model variants at $m=6$. In the simple undirected edge panel the upper bound is $10$ at $n=5$ and $12$ at $n=6$, while the true maxima are $8$ and $11$. Both are drawn as squares. The $8$ comes from the exhaustive solve, which finished at $n=5$ and not at $n=6$, and the $11$ from the twelve-hyperedge enumeration below.}
    \label{fig:variant-bounds-m6-hyper}
\end{sidewaysfigure}

\Cref{prop:hyper-edge} bounds every simple hypergraph from above.
It does not assert attainment outside its stated range.
At $m=6$, $n=6$, and $r=3$, all $125\,970$ simple twelve-hyperedge candidates are infeasible, and an eleven-hyperedge family is not.
The true maximum is therefore $11$, one below the bound of $12$.
That enumeration runs from its own standalone script, \code{program/scripts/hyper\_edge\_m6\_n6\_gap.py}, because the main program's branch and bound does not finish this cell inside the figure budget.
It supplies the exhaustive computation the branch and bound could not, so the value is recorded as exact.
The smallest analogous gap occurs at $n=r=m=3$.
A simple hypergraph then has one hyperedge, while a multihypergraph attains the bound with two copies.

\paragraph{Finite evidence at the next vertex threshold.}
\Cref{thm:hyper-vertex-m3} establishes the vertex bound at $m=3$, and its rank
argument does not establish the $m=4$ case
(\Cref{rem:hyper-vertex-m3-scope}). The standalone script
\code{program/scripts/hyper\_vertex\_m4\_search.py} tests the proposed value
$K_4^{(r)}(n)=\floorfrac{3(n-1)}{r-1}$ by searching multisets of $r$-sets,
using its own incidence model and max-flow routine independently of the main
program. It searches both for a witness attaining the target and for one
with one further hyperedge. Its eleven parameter pairs satisfy
$4\le n\le9$, $r\in\{n-2,n-1\}$ and $r\ge3$.

An archived research note dated 31 July 2026 reports that the target was
attained and one further hyperedge excluded in all eleven cases. Both halves
were rerun for this account and reproduce that outcome, in fourteen and
thirteen seconds, and their transcript is
\code{program/logs/hyper\_vertex\_m4\_log.txt}. Exhaustiveness rests on the
script's own two prunings, that feasibility is monotone under adding hyperedges
and that no prefix may leave two vertices in $m$ common hyperedges, so these
pairs are recorded here rather than entered in the generated tables, which
report the main program's own enumerations. The $r=2$ cross-check holds at
$m=3$ and at $m=4$ and fails at $m=5$, where it returns a feasible multigraph
of $13$ edges against the formula's $12$ at $n=4$ and $17$ against $16$ at
$n=5$, consistent with the multigraph witnesses discussed in
\Cref{sec:multi-vertex}. These checks concern multihypergraphs, not only simple
hypergraphs, and do not establish the formula for arbitrary $n$ and $r$.

For a hypergraph on $n$ vertices with $b$ hyperedges of size $r$ and connected
incidence graph, the cycle rank is $b(r-1)-n+1$. At $m=4$, exceeding the
displayed edge-count target is therefore equivalent to exceeding cycle rank
$2(n-1)$. A stronger incidence-rank argument is one possible route to a
general bound, and the finite search does not supply it.

\Cref{tab:variant-values} gives the data from the four grids numerically.
Blue entries are established, red entries are conjectural, and gold entries record lower bounds in open cases.
A bold green entry is machine-checked exact.
The prefix $\le$ marks an established upper bound whose attainment is not supplied at that cell. The prefix $\stackrel{?}{=}$ marks a conjectured value.
The prefix $\ge$ marks a verified construction without a matching upper bound.
A blank hypergraph cell at $n=2$ omits the degenerate zero value: no $3$-element hyperedge can be formed.
The simple hypergraph cell at $m=6$, $n=6$ reads a bold $11$ against the bound of $12$ drawn as the curve above it, which is the gap the enumeration just described settles.
The corresponding multihypergraph edge cell is the established value $12$: the twelve-copy construction meets the checked cut upper bound. It is printed as a theorem-derived value, not as an independent enumeration.
The vertex construction also gives $12$ without settling that optimum.
...
\begin{table}[p]
    ...
    \caption{Finite values and bounds for all sixteen variants at $m=3$ and $m=6$. The colours and markers match the four grids above.}
    \label{tab:variant-values}
\end{table}


\paragraph{Search performance versus supplied constructions.}
The next four tables report the two lower-bound sources separately.
Unlike \Cref{tab:variant-values}, they do not replace either source with a
proved optimum. For example, a search value of $s$ below a construction value
of $c$ says that the unaided run found $s$ edges although a feasible example
with $c$ edges is known. It is a limitation of that run, not a contradiction.
These tables show orders through eight. The data file includes the full
plotted ranges.

\begin{table}[p]
    \centering
    \small
    \resizebox{\textwidth}{!}{\input{figures/search_evidence_m3_graphs.tex}}
    \caption{Raw search and separately supplied construction counts for the graph-model variants at $m=3$. The search limit is per $(n,m,\text{variant})$ case. A dash means no search budget applies to the construction.}
    \label{tab:search-evidence-m3-graphs}
\end{table}

\begin{table}[p]
    \centering
    \small
    \resizebox{\textwidth}{!}{\input{figures/search_evidence_m3_hypergraphs.tex}}
    \caption{Raw search and separately supplied construction counts for the hypergraph-model variants at $m=3$. The search limit is per $(n,m,\text{variant})$ case. A dash means no search budget applies to the construction.}
    \label{tab:search-evidence-m3-hypergraphs}
\end{table}

\begin{table}[p]
    \centering
    \small
    \resizebox{\textwidth}{!}{\input{figures/search_evidence_m6_graphs.tex}}
    \caption{Raw search and separately supplied construction counts for the graph-model variants at $m=6$. The search limit is per $(n,m,\text{variant})$ case. A dash means no search budget applies to the construction.}
    \label{tab:search-evidence-m6-graphs}
\end{table}

\begin{table}[p]
    \centering
    \small
    \resizebox{\textwidth}{!}{\input{figures/search_evidence_m6_hypergraphs.tex}}
    \caption{Raw search and separately supplied construction counts for the hypergraph-model variants at $m=6$. The search limit is per $(n,m,\text{variant})$ case. A dash means no search budget applies to the construction.}
    \label{tab:search-evidence-m6-hypergraphs}
\end{table}
...
The directed $m=2$ values for $2\le n\le7$ were established by completed pruned exhaustive searches, separately under arc and vertex separation. Each completed row in \Cref{tab:basecase-search} is a finite upper-bound computation with a matching witness. The all-order values do not follow from these rows alone. The two runs have identical node counts because their feasible families agree at $m=2$, as explained in \Cref{sec:dir-arc-m2}.
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}rrrrrl@{}}
    ...
    \bottomrule
  \end{tabular}
  \caption{The directed $m = 2$ values at small $n$ under both separations:
  exhaustive branch and bound over all simple digraphs on $n$ vertices with
  ...
  completed. Neither column is load-bearing. These were the base cases of an
  earlier induction, which \Cref{sec:dir-arc-m2} has replaced by an argument
  from \Cref{lem:reach-skeleton} that runs at every $n$ at once, so both columns
  now corroborate rather than prove. The node counts coincide because the two feasible
  families coincide at $m=2$ (\Cref{lem:directed-m2-families}): the feasibility
  prune switches between the arc and the vertex flow test with the separation,
  but at $m=2$ the two tests accept the same partial digraphs and so cut the
  ...
  \code{program/logs/basecase\_search\_log.txt} and
  \code{program/logs/basecase\_search\_vertex\_log.txt}.}
...
theorem rests on it. Its raw log is \path{program/logs/certificate_log.txt}. Exact
