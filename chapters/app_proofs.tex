...
For a multihypergraph $\HH$ and vertices $u, v$, the maximum number of hyperedge-disjoint Berge $u$--$v$ paths equals the minimum size of a set $C$ of hyperedges whose removal separates $u$ from $v$. Parallel copies count separately on both sides: two routes may not ride the same copy, and cutting a hyperedge means cutting every copy of it. A simple hypergraph is the case where every multiplicity is one.
\end{theorem}
...
\emph{Cuts.} Finally, the member arcs are uncapped, so any cut of finite capacity uses helper arcs only, and a set of helper arcs separates $u$ from $v$ in the network exactly when the corresponding hyperedges separate them in $\HH$. Its capacity is $\sum \mu(e)$ over those hyperedges, which is the number of copies removed, so cut capacity and the size of $C$ are the same count. The max-flow min-cut theorem equates the largest number of arc-disjoint flow paths with the smallest helper-arc cut, which is precisely the claim.
\end{proof}
...
\]
That range neither contains nor is contained in the simple model's
$2 \le \binom{n-2}{r-2}$, so the two models close different cases.
\end{proposition}
