...
\begin{conjecture}[A sparse reachability skeleton always exists, with proof,\aimedal\readfirst{7}]\label{lem:reach-skeleton}
...
does not describe every extremal digraph. The following construction, supplied
by Stijn Cambie, refutes the earlier formula and the decomposition proposed
alongside it.
...
\begin{conjecture}[Deleting the smaller half of every vertex, with proof,\aimedal\readfirst{4}]\label{lem:smaller-half-deletion}
...
\begin{conjecture}[The counting core, with proof,\aimedal\readfirst{5}]\label{lem:two-step-budget}
...
\begin{conjecture}[Menger for multihypergraphs, with proof,\aimedal\readfirst{1}]\label{thm:menger-hyper}
...
\begin{conjecture}[Cut induction for multihypergraphs, with proof,\aimedal\readfirst{2}]\label{lem:hyper-cut-induction}
...
\begin{proof}[Conditional proof of \Cref{prop:hyper-edge}]
...
\begin{conjecture}[Simplicity is free for $r \ge 3$, with proof,\aimedal\readfirst{3}]\label{thm:simple-hyper-edge}
...
\begin{conjecture}[Twelve hyperedge copies on six vertices, with proof,\aimedal]\label{const:multihyper-six}
...
cuts and the conjectured upper bound, not from an exhaustive search. In the
...
\begin{conjecture}[General lower bound, with proof,\conditional]\label{prop:hyper-vertex-lower}
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The eight hypergraph-model variants at $m=3$. The red curve is the conjectured upper bound of \Cref{prop:hyper-edge}, with attainment claimed only on the stated range. A lower-bound point below it does not alone prove that the bound is unattainable. Only an exact square below it establishes such a gap.}
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The eight hypergraph-model variants at $m=6$. In the simple undirected edge panel the conjectured upper bound is $10$ at $n=5$ and $12$ at $n=6$, while the true maxima are $8$ and $11$. Both are drawn as squares. The $8$ comes from the exhaustive solve, which finished at $n=5$ and not at $n=6$, and the $11$ from the twelve-hyperedge enumeration below.}
    \label{fig:variant-bounds-m6-hyper}
\end{sidewaysfigure}

\Cref{prop:hyper-edge} proposes an upper bound on every simple hypergraph.
It does not assert attainment outside its stated range, and its proof is unchecked.
At $m=6$, $n=6$, and $r=3$, all $125\,970$ simple twelve-hyperedge candidates are infeasible, and an eleven-hyperedge family is not.
The true maximum is therefore $11$, one below the proposed bound of $12$.
