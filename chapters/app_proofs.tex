...
\begin{table}[htbp]
  ...
  still rising at $b=8$ in every row, so this table does not extrapolate: a
  larger $n$ needs larger blocks enumerated.}
  \label{tab:multi-vertex-blocks}
\end{table}

The winning blocks are never complete once $b \ge 5$, and their degree
sequences are lopsided at every cell, a few vertices of high degree beside a
majority at degree two or three. What the sweep does \emph{not} show is that
they are complete bipartite graphs with a few edges added. That description is
exact at $m = 7$ and $m = 8$, where the maximiser at $b = 7$ and $b = 8$ is
$K_{3,4}$ or $K_{3,5}$ with two further edges inside the side of size three. It
is the unique maximiser in three of those four cells, $m = 8$ at $b = 7$ having
six. The description fails at $m = 4$, $5$ and $6$, where from $b = 7$ onwards
...
and $m = 8$, and that at $m = 4$, $5$ and $6$ the extremal blocks take some
other shape entirely. So the answer is a block problem, its asymptotic constant
...
entry the file does not hold is reported as an error rather than computed. That
run does rebuild three further caches, behind figures this thesis does not print,
and those carry timed searches, so it takes minutes rather than seconds. Passing
