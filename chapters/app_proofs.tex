...
\]
which for $r \ge 3$ is smaller than $m/n$ by a factor of order $n^{r-2}$. Reading $m/n$ as the hypergraph balance point would therefore place it far above the expected-degree balance: at $r = 3$ and $n = 8$, for instance, $m/n$ is $3/8$ while $m/\binom{7}{2}$ is $3/21$. \Cref{fig:threshold-3d} accordingly sweeps each model in units of its own $p^*$ rather than a shared $p$. A second limit of scope is worth repeating here: the theorem assumes $m/\ln n \to \infty$, so any picture at a fixed small $m$ illustrates the shape of the transition without being an instance of the theorem.
\end{remark}
