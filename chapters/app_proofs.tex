...
The first is the reachability-skeleton argument proving \Cref{thm:dir-multi-full} for every $n$ and $m$.
...
Optimising $r$ widens the gap further. Treated as continuous, the gain per vertex $\mathrm{gain}(r,m)/(r-1)$ peaks near $r \sim (m+1)/2$, where it is $\Theta(m^2)$. Packing blocks of that size therefore gives $K_m(n) \ge \bigl((m-1) + \Theta(m^2)\bigr)(n-1-O(m))$. In particular the per-vertex rate is quadratic in $m$ once $n/m\to\infty$, a different order from the tree's linear rate.

Even this is not the truth. At $m = 5$, $n = 7$ the construction above gives
$27$ and the exact block computation below gives $29$.
...
\code{program/requirements.txt}. The exact verified versions are pinned in
\code{program/requirements-lock.txt}. They are Python 3.14.6, NumPy 2.4.6,
