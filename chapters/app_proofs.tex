...
The first is the reachability skeleton of \Cref{lem:reach-skeleton}, which settles \Cref{thm:dir-vertex-m2-exact,thm:dir-arc-m2-exact} in a few lines and \Cref{thm:dir-multi-full} for every $n$ and $m$.
The second is the incidence-rank argument of \Cref{lem:incidence-rank,thm:hyper-vertex-m3}.
It settles the hypergraph vertex problem at $m=3$ by induction over blocks and separating pairs.
The third is the two-step budget of \Cref{lem:two-step-budget}.
It drives \Cref{thm:dir-arc-linear-error,thm:dir-vertex-linear-error,thm:dir-hyper-constant,thm:dir-hyper-general-constant} and pins six variants asymptotically.
...
An unbadged heading is either a cited classical result or an argument the author has checked.
...
One tool serves both directed results that follow, so it is proved here before
either of them. The object is a thin subdigraph that carries the whole
reachability relation of $D$ and nothing else. \Cref{sec:dir-arc-m2} uses it
once, to settle $m = 2$ in a few lines under both separations, and
...
This section proves the two $m = 2$ values, and it proves them in the order the logic runs rather than the order they are stated. The \emph{vertex} case comes first, in a few lines from \Cref{lem:reach-skeleton}. The \emph{arc} case then follows from it through \Cref{rem:whitney-direction}, because an upper bound on the larger feasible family is an upper bound on the smaller one, and the same argument also proves it directly. Neither proof uses induction on $n$, so neither needs a base case, and neither theorem rests on a machine run. The lemmas below are used elsewhere rather than here: the first two show that a hub forces a linear bound for every $m$, which explains the hub lower bound discussed in \Cref{ch:synthesis}, the third rules out transitive triangles, and the fourth is the deletion monotonicity that \Cref{sec:two-step-budget} runs on. These auxiliary statements and the two $m=2$ theorems carry no badge. \Cref{tab:basecase-search} records an exhaustive sweep over $2 \le n \le 7$ under both separations. It agrees with both theorems at every size and is corroboration rather than an ingredient.
...
Three independent confirmations sit beside these proofs and none is needed by them. The exhaustive search of \Cref{ch:certify} was run under both feasibility tests, reporting the same maxima $2, 4, 6, 8, 10, 12$ at $n \le 7$, and the two smallest of those are immediate by hand: at $n = 2$ there are at most $2$ arcs, and at $n = 3$ a fifth arc would leave at most one arc of the complete bidirected triangle missing, so three of the remaining arcs form a transitive triangle, which \Cref{lem:no-transitive-triangle} forbids. Separately, \Cref{thm:dir-multi-full} proves $L_2^{\mathrm{dir}}(n) = M(n)$ for directed multigraphs, and \Cref{thm:dir-multi-m2} records that at $m = 2$ the multigraph and the simple digraph problems are the same problem. Those runs and that theorem corroborate the values, and the proofs above are what establish them.
...
\begin{proof}[Proof of \Cref{thm:dir-arc-linear-error}\aimedal]
...
\begin{proof}[Proof of \Cref{thm:dir-vertex-linear-error}\aimedal]
...
\begin{proof}[Proof of \Cref{thm:dir-multi-full}\aimedal]
...
\begin{corollary}[The directed multigraph value at $n = 7$, $m = 3$,\aimedal]\label{cor:dir-multi-n7}
$L_3^{\mathrm{dir}}(7) = 24$.
\end{corollary}
...
\begin{construction}[Twelve hyperedge copies on six vertices,\aimedal]\label{const:multihyper-six}
...
cuts and the proved upper bound, not from an exhaustive search. In the
...
\begin{corollary}[The multigraph incidence value at $m = 3$,\aimedal]\label{cor:multi-vertex-m3}
...
\begin{theorem}[The multigraph incidence problem is a knapsack over blocks,\aimedal]\label{thm:multi-vertex-blocks}
...
\begin{lemma}[Splitting the directed vertex measure,\aimedal]\label{lem:multi-vertex-split-dir}
...
\begin{corollary}[The directed multigraph incidence value,\aimedal]\label{cor:dir-multi-incidence}
For every $n \ge 2$,
\[
...
\begin{proposition}[The directed incidence value is a block problem too,\aimedal]\label{prop:dir-multi-vertex-blocks}
...
\begin{construction}[Bounded outgoing hyperedge count,\aimedal]\label{const:bounded-outdegree-hyper}
...
\begin{proof}[Proof of \Cref{thm:dir-hyper-constant}\aimedal]
...
A blue curve is proved.
A filled square is a value established by completed enumeration. In particular, the directed multigraph arc formula is drawn as a theorem curve, not as a supposedly independent enumeration result.
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The eight hypergraph-model variants at $m=3$. The blue curve is the proved upper bound of \Cref{prop:hyper-edge}, not always a proved value. A lower-bound point below it does not alone prove that the bound is unattainable. Only an exact square below it establishes such a gap.}
...
Blue entries are proved, and gold entries mark open cases.
A bold green entry is machine-checked exact.
The prefix $\le$ marks a proved upper bound that no recorded exact value matches at that point.
...
The corresponding multihypergraph \emph{edge} cell is $12$ by
\Cref{const:multihyper-six}. It is blue rather than a green square because
its exactness comes from an explicit construction matching the proved
upper bound, not from completed enumeration. The historical search value
is unchanged. The new construction also raises the multihypergraph vertex
lower bound in this cell to $12$, without settling its optimum.
