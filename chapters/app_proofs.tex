...
build is: \thesiscommit. When this status contains a commit, that commit is the
immutable reference for every number reported here. Later repository changes
are then continuations rather than part of the submission. A build with no
recorded revision is a working build, not a submission snapshot.

Three commands cover the main audit. From \code{program/},
\code{python erdos915\_unified.py} runs the principal checks.
The command \code{python -m unittest discover -s tests -v} runs the regression
suite. The command \code{python make\_figures.py} regenerates the computational
figures.
