...
The word \emph{stability} below refers to that second conclusion: the digraph
can be changed into the extremal bipartite shape by a lower-order number of
deletions.
...
\begin{proof}
First sum the budget over all ordered pairs of distinct vertices. If $\Delta$
is the number of digons, then counting two-step walks by their midpoint gives
\[
\begin{aligned}
    \sum_x d^+(x)d^-(x)
    &= \sum_{u\ne v}p_2(u,v)+2\Delta \\
    &\le Cn(n-1)-|A(D)|+2\Delta
    \le Cn(n-1),
\end{aligned}
\]
because every digon uses two arcs, so $2\Delta\le |A(D)|$.

Now induct on $n$. The claim is immediate at $n = 2$. If some vertex $v$ has
$d(v) < n/2$, then the budget is inherited by $D-v$, and the induction
hypothesis together with \Cref{lem:floor-identity} gives
\[
\begin{aligned}
    |A(D)|
...
\end{aligned}
\]
Otherwise $d(x) \ge n/2$ for every vertex. By \Cref{lem:small-side},
\[
    \sum_x\min\bigl(d^+(x),d^-(x)\bigr)
    \le \frac4n\sum_x d^+(x)d^-(x)
    \le 4C(n-1).
\]
Delete every in-arc of a vertex with $d^+ \ge d^-$ and every out-arc of each
remaining vertex. At most the displayed number of arcs is deleted, and every
surviving arc runs from the first class to the second. The surviving digraph
therefore has at most $\floor{n^2/4}$ arcs, which proves the claim.
\end{proof}
