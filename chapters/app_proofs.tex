...
This section proves the two $m = 2$ values, and it proves them in the order the logic runs rather than the order they are stated. The \emph{vertex} case comes first, in a few lines from \Cref{lem:reach-skeleton}. The \emph{arc} case then follows from it through \Cref{rem:whitney-direction}, because an upper bound on the larger feasible family is an upper bound on the smaller one, and the same argument also proves it directly. Neither proof uses induction on $n$, so neither needs a base case, and no result in this thesis rests on a machine run. The lemmas below are used elsewhere rather than here: the first two show that a hub forces a linear bound for every $m$, which is the proof behind the hub branch of \Cref{conj:dir-arc}, the third settles $n = 3$ by hand, and the fourth is the deletion monotonicity that \Cref{sec:two-step-budget} runs on. All four carry no badge, while the two theorems keep one apiece at their statements. \Cref{tab:basecase-search} records an exhaustive sweep over $2 \le n \le 7$ under both separations; it agrees with both theorems at every size and is corroboration rather than an ingredient.
...
What is left open is therefore a constant factor in one coefficient, not a shape. \Cref{conj:dir-arc} says the second-order coefficient is exactly $(m-2)/2$, and \Cref{thm:dir-arc-linear-error} pins it between that and $4(m-1)$, a factor of $8(m-1)/(m-2)$: sixteen at $m = 3$, and tending to eight as $m$ grows. That is the factor at every $n$. Once $n$ is large in terms of $m$, the Tur\'an theorem of Huang and Lyu \cite{HuangLyu26} cited in \Cref{ch:basecases} lowers the upper end to $m-1$, and the factor with it to $2(m-1)/(m-2)$: four at $m = 3$, and tending to two. Closing that gap is where the structure of \Cref{open:decomposition} lives, and \Cref{rem:case2-tight} rules out the most obvious way to try.
...
An upper bound needs a deletion that is guaranteed to lower every positive connectivity, paired with a bound on how much it removes. The vertex deletion that suggests itself supplies the first of those and not the second. Once multiplicities are free to sit anywhere between $0$ and $m-1$, no single vertex is guaranteed to carry a controllable share of the arcs, which is exactly the fractional remainder above. The fix is to stop insisting on a vertex and delete a whole \emph{subgraph} instead, and the reachability skeleton of \Cref{sec:reach-skeleton} is one: general enough to always exist, sparse enough to bound by \Cref{lem:reach-skeleton}, and, as the next lemma shows, specific enough that removing it lowers every positive pairwise connectivity by at least one.
...
For $2 \le b \le m+1$, take the complete digraph on $b$ vertices and give every arc
...
\[
    K_m^{\mathrm{dir}}(n) \;\ge\; \max\Bigl\{\, \sum_i b_i(b_i-1)(m+1-b_i)
      \;:\; 2 \le b_i \le m+1,\ \sum_i (b_i - 1) \le n-1 \,\Bigr\} .
\]
\end{proposition}
...
The source revision recorded for this PDF build is \thesiscommit. A working
build for which \code{record\_revision.sh} was not run prints ``not recorded''
here instead of implying that an unrecorded binary is a submission snapshot.
...
A red curve is the formula of \Cref{conj:dir-arc}.
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The eight graph-model variants at $m=3$. The simple directed arc panel carries the only conjectured curve. The directed vertex panels remain open even where their verified lower bounds agree with that curve.}
...
Blue entries are proved, red entries belong to \Cref{conj:dir-arc}, and gold entries are open.
