...
Now read the count of \Cref{lem:reach-skeleton} backwards. It gives $|A(R)| \le f(q) := 2(n-q) + \floor{q^2/4}$, where $q$ is the number of strongly connected components of $D$, so $f(q) \ge \floor{n^2/4} = f(n)$. The increments $f(q+1) - f(q) = -2 + \floor{(q+1)/2}$ are non-decreasing, so $f$ is convex on $q \in \{1, \dots, n\}$, and a convex function attains its maximum over an interval at one of the two endpoints. Since $f(1) = 2(n-1)$ and the hypothesis $n \ge 8$ gives $\floor{n^2/4} > 2(n-1)$, we have $f(1) < f(n)$, so that endpoint loses and $f(n)$ is the maximum of $f$ on the whole interval: $f(q) \le f(n)$ for every $q$. Equality can only hold where every increment from $q$ up to $n$ vanishes, and the last of those increments is $f(n) - f(n-1) = -2 + \floor{n/2} \ge 2 > 0$ for $n \ge 8$, so no $q < n$ reaches $f(n)$. Hence $f(q) < f(n)$ strictly whenever $q < n$, and combined with $f(q) \ge f(n)$ from above this forces $q = n$: every strongly connected component is a single vertex, which is (1).
