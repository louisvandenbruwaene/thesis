...
Let $n\ge m\ge2$, put $r=n-m$, $a=\ceil{r/2}$ and $b=\floor{r/2}$.
...
\]
Indeed, $a(b+1)=\floor{(r+1)^2/4}$. Subtracting the old branches gives
\[
 B_m(n)-m(n-1)=\floorfrac{(r+1)^2}{4}-2r,\qquad
 B_m(n)-\floorfrac{(n+m-2)^2}{4}=m-1-\floor{r/2}.
\]
Both differences are positive when $m\ge5$ and $7\le r\le2m-3$.
...
At $m = 3$ this is the multigraph \emph{edge} value $L_3(n)$ of \Cref{thm:multigraph-edge} as well. The two separations part company at $m = 5$: $K_5(4) = 14$ against $L_5(4) = 12$, and $K_5(5) = 19$ against $L_5(5) = 16$, both by exhaustion. For $m \ge 3$ at least three constructions compete, and which one wins depends on how $n$ compares with $m$, exactly as in every other variant of this thesis.
