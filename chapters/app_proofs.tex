...
The proposed reachability-skeleton argument supports the directed formulas.
The proposed incidence-rank argument supports the hypergraph vertex value at $m=3$.
The two-step budget supplies proposed elementary bounds for directed models.
These arguments remain subject to verification. Their statements are labelled
...
The marker \conditional{} identifies deductions that depend on an unchecked conjecture. Unmarked results are cited classical results or arguments the author has checked.
...
\apppart{Part II. The directed results}{The three constructions the body names but does not define, the reachability skeleton that both exact directed results run on, the value at $m=2$ under both separations, the structural propositions that bound the frontier at $m \ge 3$, and the proposed proof that covers the directed multigraph problem at every $n$ and every $m$.}
...
One proposed tool supports both directed formulas, so its conjecture and proposed proof come first. The object is a thin subdigraph that carries the whole
reachability relation of $D$ and nothing else. \Cref{sec:dir-arc-m2} uses it
once in the proposed $m=2$ argument under both separations, and
...
They do not establish the all-order conjectures.
...
\begin{conjecture}[The two directed feasible families at $m=2$, with proof,\aimedal]\label{lem:directed-m2-families}
...
The multigraph formula is a related conjecture built on the same skeleton argument, so it is not independent corroboration of that argument.

\begin{remark}[The directed vertex problem at $m \ge 3$]\label{rem:dir-vertex-m3}
The skeleton argument proposes the value at $m=2$ (\Cref{thm:dir-vertex-m2-exact}, conditional on \Cref{lem:reach-skeleton}). For $m\ge3$, the vertex problem is separate: the arc constructions belong to the smaller arc-feasible family and give no upper bound on $k_m^{\mathrm{dir}}(n)$. What can be said is computational. Running the exhaustive search of \Cref{ch:certify} under both feasibility tests at $m = 3$ gives
\[
\begin{array}{r|ccccc}
    n & 2 & 3 & 4 & 5 & 6 \\ \hline
...
The second application is the directed \emph{vertex} problem. \Cref{thm:dir-vertex-m2-exact} proposes its value at $m=2$, and for $m \ge 3$ this thesis has been careful to record that it does not follow from the arc case: Whitney's $\kap \le \lec$ makes the vertex-feasible family the larger of the two, so an arc upper bound restricts nothing there. That remains true of the \emph{bound}. What carries across is the route-counting, because the family \Cref{lem:feasible-budgeted} exhibits happens to be disjoint in the stronger sense as well, which is why that lemma is stated for both separations at once and why the argument never needs the arc value.

\begin{proof}[Proposed proof of \Cref{thm:dir-vertex-linear-error}]
...
\section{A proposed formula for the directed multigraph problem}\label{sec:dir-multi}

The conjectured formula has a linear branch from thickened bidirected trees and a quadratic branch from one-way complete bipartite digraphs. The construction proves a lower bound. The proposed upper-bound proof below depends on the unchecked reachability-skeleton argument.

The proposed argument deletes a sparse subgraph preserving reachability. If that skeleton has the claimed size, repeating the deletion controls the total multiplicity.
...
Taking whichever of the two constructions is larger gives $L_m^{\mathrm{dir}}(n) \ge (m-1)M(n)$ for every $n$ and $m$, which is the lower bound half of \Cref{thm:dir-multi-full}. The rest of this section proposes the matching upper bound.
...
\begin{proof}[Proposed proof of \Cref{thm:dir-multi-full}]
...
\begin{conjecture}[The directed multigraph value at $n = 7$, $m = 3$, with proof,\aimedal\readfirst{2}]\label{cor:dir-multi-n7}
$L_3^{\mathrm{dir}}(7) = 24$.
\end{conjecture}
...
runs do not independently validate the proposed general formula.

\begin{conjecture}[The fractional relaxation is integral, with proof,\conditional]\label{cor:mstar-integral}
...
\begin{conjecture}[The $m=2$ case collapses to the simple digraph, with proof,\conditional]\label{thm:dir-multi-m2}
...
\begin{conjecture}[The multihypergraph lower bound, with proof,\conditional]\label{prop:hyper-vertex-lower-multi}
...
At $m=3$ the proposed upper bound covers every multihypergraph, and equality follows in the construction range stated below. Both rest on the unchecked rank inequality proved next, whose engine is a bound for
graphs in which one side of a partition must stay below local
$3$-connectivity. The hypergraph conjecture then falls out of the incidence
...
\begin{conjecture}[Incidence rank lemma, with proof,\aimedal\readfirst{10}]\label{lem:incidence-rank}
...
Conditionally on that rank bound, \Cref{thm:hyper-vertex-m3} follows, stated in
...
\begin{proof}[Conditional proof of \Cref{thm:hyper-vertex-m3}]
...
At $r = 2$ the conjecture speaks of multigraphs, where the incidence convention
of \Cref{sec:incidence-convention} makes $q$ parallel edges $q$ distinct one-step
routes. It would give $K_3(n) = 2(n-1)$, the same number as the multigraph edge
value of \Cref{thm:multigraph-edge}, and \Cref{cor:multi-vertex-m3} below records
that specialisation, conjecturally with it. The proof of
...
The next value is the one the hypergraph conjecture supplies, and no separate argument is needed for it.
...
At $m = 3$ this is the multigraph \emph{edge} value $L_3(n)$ of \Cref{thm:multigraph-edge} as well. The first threshold at which the two values are known to differ is $m = 5$: $K_5(4) = 14$ against $L_5(4) = 12$, and $K_5(5) = 19$ against $L_5(5) = 16$, both by exhaustion. At $m = 4$ they agree for every $n \le 8$, since $g_4(b) = 3(b-1)$ throughout the range of \Cref{tab:multi-vertex-blocks}, and whether they agree at every $n$ is open. For $m \ge 3$ at least three constructions compete, and which one wins depends on how $n$ compares with $m$, exactly as in every other variant of this thesis.
...
Optimising $r$ widens the gap further. Treated as continuous, the gain per added vertex, which is $\mathrm{gain}(r,m)/(r-1)$, peaks near $r \sim (m+1)/2$, where it is $\Theta(m^2)$. Packing blocks of that size therefore gives $K_m(n) \ge \bigl((m-1) + \Theta(m^2)\bigr)(n-1-O(m))$. In particular the per-vertex rate is quadratic in $m$ once $n/m\to\infty$, a different order from the tree's linear rate.
...
What is not open is the order, and to say that the bouquet is
...
so $K_m(n) = \Theta(m^2 n)$ in that large-$n$ regime. This qualification is necessary: for example $K_m(2)=m-1$, so no estimate of order $m^2n$ can hold uniformly when $n$ is fixed. The bouquet of \Cref{thm:clique-chain-vertex} has the right joint order once many blocks fit, and what separates the two sides is a constant factor tending to $16$. \Cref{thm:multi-vertex-bipartite} below improves the lower constant and brings that factor down to $6 + 4\sqrt{2} \approx 11.66$. The upper bound is crude, since it throws away the $-\sum \pi$ correction entirely and charges every edge the full $m-1$. The first inequality, $K_m(n) \le (m-1)k_m(n)$, is the more informative half. It says that this variant can never outrun the simple vertex problem by more than the multiplicity cap, which ties the least understood of the sixteen to the one with the longest history. It is tight at $m = 2$, where both sides read $n-1$ by \Cref{rem:multi-vertex-m2-simple}.
...
\begin{table}[htbp]
  ...
  block exists, which at $m=2$ is every $b \ge 3$. Feeding this row into the
  knapsack of \Cref{thm:multi-vertex-blocks} gives $K_m(n)$ for $n \le 8$. At
...
graphs on nine vertices gives $g_6(9) = 54$, attained by a block with fourteen
...
$K_6(9) = 54$. Its rate $54/8$ exceeds the $47/7$ of $b=8$, which is the $m=6$
...
\begin{conjecture}[Directed hypergraphs: first bounds, with proof,\aimedal\readfirst{4}]\label{prop:dir-hyper-first}
...
\begin{conjecture}[Bounded outgoing hyperedge count, with proof,\aimedal\readfirst{5}]\label{const:bounded-outdegree-hyper}
...
\begin{conjecture}[Backward is the reverse of forward, with proof,\aimedal\readfirst{6}]\label{lem:dir-hyper-duality}
...
\begin{conjecture}[The general model obeys the forward bound, with proof,\aimedal\readfirst{7}]\label{prop:dir-hyper-general}
...
proposes that the bipartite pattern is right to first order, that no
...
\begin{proof}[Proposed proof of \Cref{thm:dir-hyper-constant}]
...
carries a constant of $4(m-1)^2$, and the conjecture above is an asymptotic
improvement rather than a uniform one. Second, at $r = 2$ the shadow is the
digraph itself and $\mathrm{cod}$ is the multiplicity, so the conjecture above
...
\begin{conjecture}[The general orientation model shares the constant, with proof,\aimedal\readfirst{8}]\label{thm:dir-hyper-general-constant}
...
constant. The second point is what the conjecture above would do to the
orientation axis of \Cref{sec:orientation-models}. Forward and backward are
proposed equal, exactly, by \Cref{lem:dir-hyper-duality}. Otherwise the general
model is placed only between the same two bounds a factor of four apart, which
leaves room for it to be denser to first order. The conjecture above predicts
that it is not: all three orientation models would then share the same leading
...
A blue curve represents an established result. A red curve represents a conjectured formula or upper bound.
A filled square is a value established by completed enumeration. In particular, the directed multigraph arc formula is drawn as a conjectural curve, not as a supposedly independent enumeration result.
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The eight graph-model variants at $m=3$. The simple directed arc and vertex panels show verified lower bounds and completed exact values only. The construction marks assert feasibility, not optimality. No conjectured finite-order formula is drawn.}
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The eight hypergraph-model variants at $m=3$. The red curve is the proved upper bound of \Cref{prop:hyper-edge}. Attainment is established only on the stated range, so the curve is drawn as a conjectured value outside it. A lower-bound point below it does not alone prove that the bound is unattainable. Only an exact square below it establishes such a gap.}
...
The prefixes $\stackrel{?}{\le}$ and $\stackrel{?}{=}$ mark a conjectured upper bound and a conjectured value respectively.
...
The corresponding multihypergraph edge cell records a conjectured upper bound of $12$.
The twelve-copy construction reaches that count, but its optimality still depends on the proposed upper-bound proof.
...
\begin{table}[ht]
  ...
  now corroborate rather than prove. The node counts coincide as a computed fact, not by
