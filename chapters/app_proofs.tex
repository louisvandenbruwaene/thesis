...
\]
halving the constant in the only general upper bound known. It holds for
...
One thing \Cref{thm:dir-arc-linear-error} does not say is how far a feasible digraph is from the bipartite shape, since its dense case deletes arcs but its sparse case argues by induction instead. \Cref{lem:smaller-half-deletion} gives that structural statement directly, at the cost of a weaker bound, and the two are not comparable: at $m = n = 3$ the display below reads $11$ against the theorem's $18$, and the theorem only becomes the smaller of the two past $n \approx 16(m-1)^2/m$, which is between $19$ and $20$ at $m = 3$ and between $33$ and $34$ at $m = 4$. The word \emph{stability} names the second conclusion, that the digraph can be changed into the extremal bipartite shape by a lower-order number of deletions.

\begin{proposition}[Unconditional quadratic bound and stability,\aimedal]\label{prop:dir-arc-stability}
Let $D$ be a simple digraph on $n \ge 2$ vertices with $\lec^{\max}_D \le m-1$. Then $D$ can be made one-directional bipartite by deleting at most $\sqrt{m}\, n^{3/2}$ arcs, and
\[
