...
The shift is visible in every entry the database and this thesis both state. It gives $k_2(n) = n$ where a tree has $n-1$ edges, $k_3(2n) = 3n-1$ against $\floor{3(2n-1)/2} = 3n-2$ here, $k_4(n) = 2n-1$ against $2n-2$, $\ell_5(2n) = 5n-2$ against $5n-3$, $\ell_6(n) = 3n-2$ against $3n-3$, and it writes the Bollob\'{a}s-Erd\H{o}s conjecture as $1 + \tfrac{m(n-1)}{2}$, the edge count of a bouquet of $K_m$ blocks, $\binom{m}{2}$ edges for every $m-1$ vertices, where the convention here gives $\tfrac{m(n-1)}{2}$, restated as $c_m = m/2$ in \Cref{thm:simple-vertex-blocks}. Every one is exactly one apart, and the values in this thesis are the ones an exhaustive search returns: $k_3(4) = 4$, $k_3(5) = 6$ and $k_4(5) = 8$ were each confirmed by walking every graph of that size.
