...
\]
hyperedges, the maximum taken at $\alpha = \floor{n/2}$, and which is feasible for both
separations at once, since it is $\lec$-feasible and $\kap \le \lec$. Upper and
...
check one wants, though it does not reprove that theorem, which is exact. Third, the argument uses the
forward model twice, once for the entering hyperedges and once for the leaving
ones, in both cases to know that a hyperedge has a single tail. The backward
model is covered by \Cref{lem:dir-hyper-duality}, but the general model of
\Cref{prop:dir-hyper-general} is not: with several tails allowed, two routes
leaving through different midpoints may leave through one shared hyperedge, and
Step 2 would need a second matching argument on that side too, on a family whose
conflicts run both ways at once. Whether the general model shares this leading
constant is therefore still open. Nothing here rules it out, and the route to it
is visible rather than blocked, which is a better position than the factor of
four the general model sat in before.
...
so $K_m^{\mathrm{multi}}(n) = \Theta(m^2 n)$: the bouquet of \Cref{thm:clique-chain-vertex} is a lower bound of the right order in $n$ and in $m$ both, and what separates the two sides is a constant factor tending to $16$. The upper bound is deliberately crude, since it throws away the $-\sum \pi$ correction entirely and charges every edge the full $m-1$, and the first inequality $K_m^{\mathrm{multi}}(n) \le (m-1)k_m(n)$ is the more informative half: it says this variant can never outrun the simple vertex problem by more than the multiplicity ceiling, which ties the least understood of the twelve to the one with the longest history. It is tight at $m = 2$, where both sides read $n-1$.
