...
$n-1$, meaning $a_{p+q-1} \ge a_p+a_q$. Fekete's lemma \cite{Fekete23} says that the ratios of
...
The skeleton argument settles $m=2$. For $m\ge3$, the vertex problem is separate: \Cref{conj:dir-arc} concerns the smaller arc-feasible family and gives no upper bound on $k_m^{\mathrm{dir}}(n)$. What can be said is computational. Running the exhaustive search of \Cref{ch:certify} under both feasibility tests at $m = 3$ gives
\[
\begin{array}{r|ccccc}
    n & 2 & 3 & 4 & 5 & 6 \\ \hline
...
threshold in \Cref{conj:dir-arc} must be at least $3m-2$ for $m\ge5$.
...
These bounds leave a gap in the linear coefficient. Exact eventual equality would also determine the bounded correction. \Cref{conj:dir-arc} says the second-order coefficient is exactly $(m-2)/2$, and \Cref{thm:dir-arc-linear-error} pins it between that and $4(m-1)$, a factor of $8(m-1)/(m-2)$: sixteen at $m = 3$, and tending to eight as $m$ grows. That is the factor at every $n$. Once $n$ is large in terms of $m$, the Tur\'an theorem of Huang and Lyu \cite{HuangLyu26} cited in \Cref{ch:basecases} lowers the upper end to $m-1$, and the factor with it to $2(m-1)/(m-2)$: four at $m = 3$, and tending to two. \Cref{rem:case2-tight} explains why the existing degree estimates alone do not close that gap.
...
not. So the answer is a block problem, its asymptotic constant
lies between $3-2\sqrt2$ and $2$, and which blocks attain it is open along with
the constant.
...
and a pair split between two blocks is separated by the vertex they share, giving
$\kap_D \le 1$ there.
...
Rendering and recomputing are separate operations. The third command above draws
the four variant grids from the recorded values without calling the solver, so an
entry the file does not hold is reported as an error rather than computed. Outside
the four grids, the same command regenerates several offcut-only outputs. It
caches two exhaustive enumerations and the timed surface searches, while the
gallery search and the annealing-against-tabu comparison run under wall-clock
budgets every time. So the full no-flag command is not a pure rendering
operation, and it takes minutes rather than seconds. Passing
\code{-{}-rebuild} recomputes every entry from scratch, without consulting the
record, and writes a candidate file beside it. Passing \code{-{}-compare} lists
every entry on which the candidate departs from the record, and
\code{-{}-promote} replaces the record with the reviewed candidate. The
separation keeps the figures reproducible, since these are timed measurements and
the same search on a slower machine can return a different lower bound. A full
rebuild takes about an hour, of which two directed simple exhaustions at $n=6$,
$m=3$ account for a little over half of it, the edge one running about nine
minutes and the vertex one about twenty-four.
...
A filled square is a machine-checked exact value.
An open circle is a verified construction and therefore a lower bound.
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The eight graph-model variants at $m=3$. The simple directed arc and vertex panels show verified lower bounds and completed exact values only. The eventual conjecture has no specified threshold and is not plotted as a finite-order formula.}
...
\begin{sidewaysfigure}[p]
    ...
    \caption{The eight hypergraph-model variants at $m=3$. The blue curve is the proved upper bound of \Cref{prop:hyper-edge}, not always a proved value. A point below it marks a genuine gap.}
...
A blank cell means that the model does not exist at that order.
...
The multihypergraph cell beside it is the one $\le$ point of the table, since repeats are proved to attain the bound only when $(r-1) \mid (n-1)$, which $n = 6$ fails.
