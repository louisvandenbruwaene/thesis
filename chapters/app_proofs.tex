...
At $m = 4$ the picture is $h_4(b) = 2(b-1)$ for every $b \ge 4$, so the rate is exactly $2$ at every block size and $k_4(n) = 2(n-1)$. This one is a repackaging rather than a second proof, and it is worth separating the two halves. The upper bound $h_4(b) \le k_4(b) = 2(b-1)$ is \Cref{thm:leonard} itself read on a block, so it cannot turn round and reprove it, and no elementary description of the blocks with $\kap^{\max} \le 3$ is available to replace the theta argument above. What the reformulation does add is the extremal block, and it is one the appendix has already built: the hub-and-spokes graph $H_{4,b}$ of \Cref{const:hub-spokes}, a hub joined to every other vertex with a cycle on the rest. Unlike the shared-clique graph it is a single block, and it meets $2(b-1)$ at every size. An exhaustive sweep of the $2$-connected graphs confirms that it is optimal, and the unique optimum, at every $b$ from $4$ to $8$.
...
\begin{theorem}[Extremal classification on the quadratic branch]\label{thm:dir-multi-uniqueness}
...
Three remarks. First, on where the hypothesis $n \ge 2m$ is used and what it costs. It enters once, in Step 1, where a two-arc path in the skeleton is converted into $\floor{n/2}$ arc-disjoint routes between one pair, and feasibility kills that only when $\floor{n/2}$ reaches $m$. For $n < 2m$ the theorem is not proved, and it is not known to fail either. What can be said is that nothing in the argument suggests a real boundary there: the step is a lower bound on one pair's connectivity, and losing it removes a contradiction rather than supplying a counterexample. Second, a degree fact falls out of the same equality and is worth recording on its own, since it is what makes a direct search for extremisers finite in practice. Deleting a vertex leaves at most $(m-1)M(n-1)$ arcs, so every vertex of an extremiser carries degree at least $(m-1)\floor{n/2}$. For even $n$ the degree sum $2(m-1)\floor{n^2/4} = n \cdot (m-1)(n/2)$ leaves no slack at all, so every vertex has degree exactly $(m-1)n/2$: an extremiser at even $n$ is regular. Third, the shape of the argument is worth noticing. \Cref{thm:dir-multi-full} is indifferent to which multigraph it dismantles, and \Cref{ch:synthesis} records that indifference as the reason the value says nothing about uniqueness. That is true of the proof read forwards. Read backwards from equality, the same skeleton is a rigid object, and the two facts that had been mere bookkeeping, that $f$ is maximised at one end and that a minimal skeleton is triangle-free, become a determination of $q$ and an application of the equality case of Mantel's theorem.
...
In words: at the route limit $m = 3$ a hyperedge network can hold no more hyperedges under the stricter separation than the displayed edge bound. When $r \ge 3$ and $2 \le \binom{n-2}{r-2}$, that count is reached without repeating a hyperedge, so the two separations agree in this range exactly as they did at $m = 2$.
...
\]
and, writing $A(n,r,m)$ for the set of tail-class sizes $\alpha$ with
$1 \le \alpha \le n-1$ and $m - 1 \le \binom{n-\alpha-1}{r-2}$, there are feasible
constructions with $\max_{\alpha \in A(n,r,m)}\ \alpha \floorfrac{(m-1)(n-\alpha)}{r-1}$
hyperedges. The balanced choice $\alpha = \floor{n/2}$ lies in $A(n,r,m)$ as soon as
$m-1 \le \binom{\ceil{n/2}-1}{r-2}$ and already gives ${\approx}\,(m-1) n^2 / (4(r-1))$,
so the value is quadratic in $n$.
\end{proposition}
...
Combining the two directions, first letting $n/m\to\infty$ so that optimised blocks can be packed with only $O(m)$ unused vertices, and then letting $m\to\infty$,
\[
...
Below the threshold, $\kap^{\max} \le \lec^{\max}$ gives the undirected vertex statement immediately. Above it, a standard random-graph connectivity theorem gives $\kappa(G(n,p))=\delta(G(n,p))$ with high probability in this regime \cite[Ch.~7]{BollobasRandomGraphs}, and see also the hitting-time form of Bollob\'as and Thomason \cite{BollobasThomason85}. A Chernoff bound and a union bound give $\delta(G(n,p))\ge m$ with high probability when $c>1$. Global vertex-connectivity at least $m$ means that every eligible pair has at least $m$ internally vertex-disjoint paths, and therefore in particular $\kap^{\max}\ge m$. Thus the same threshold governs the undirected vertex separation.

No directed conclusion is claimed here. A corresponding assertion for $D(n,p)$ requires a directed connectivity theorem stated for the precise arc- or internally-vertex-disjoint convention in use. The undirected theorem above does not supply it, and the edge-count proof of \Cref{thm:gnp-threshold} does not transfer to sparse random digraphs. Finally, the hypothesis $m/\ln n\to\infty$ is essential to the proof. For fixed $m$ the union bound is vacuous and finer tools would be needed.
\end{remark}

\begin{remark}[How far the threshold reaches, and where it stops]\label{rem:threshold-scope}
\Cref{thm:gnp-threshold} is a statement about $G(n,p)$, and \Cref{rem:threshold-analogues} carries it to the undirected vertex separation. It does not claim a directed analogue, and it does not reach the hypergraph models. The reason for the hypergraph exclusion is a change of scale rather than a gap in the argument. The threshold sits where a vertex's expected degree reaches $m$, since below that the degree bound caps every local connectivity and above it the extremal bound forces a high pair. In $G(n,p)$ a vertex has expected degree $p(n-1)$, so the balance point is $p^* = m/n$ as stated. In the binomial $r$-uniform hypergraph, where each of the $\binom{n}{r}$ possible hyperedges is included independently with probability $p$, a vertex lies in an expected $p\binom{n-1}{r-1}$ hyperedges, so the balance point is
\[
...
The cut-counting optimisation formulation that certifies \Cref{thm:dir-multi-small} is given in \Cref{ch:certify}. Here are the solver transcripts that back it, together with the exhaustive-search output that settles the directed base cases. The complete commented source is maintained with the document as \texttt{erdos915\_unified.py} in the \texttt{program/} directory and should be included in the archived submission described in \Cref{app:code}. Its core runs on \texttt{numpy} and \texttt{scipy} alone, the certifier adds \texttt{pulp}, and \texttt{matplotlib} and \texttt{networkx} are optional analysis and visualisation dependencies. An optional compiled helper accelerates selected small hot paths but is not needed for correctness.
