...
The Bollob\'{a}s--Erd\H{o}s conjecture, discussed in \Cref{sec:simple-vertex-blocks}, must also retain its vertex count:
...
Let $G$ be feasible. Adjacent vertices $u,v$ lie in a common block $B$, and no $u$--$v$ path leaves $B$: leaving means crossing a cut vertex, and returning means crossing that same cut vertex a second time, which no path does. So $\kap_G(u,v) = \kap_B(u,v)$. Every block is a subgraph of $G$, so $\kap_B \le \kap_G$ on every pair of its vertices and it inherits feasibility, and since every edge lies in exactly one block, $|E(G)| = \sum_B |E(B)| \le \sum_B h_m(|V(B)|)$. Within one component the block sizes satisfy $\sum_B (|V(B)|-1) = (\text{order of the component}) - 1$, and summing over components loses one further vertex apiece, so $\sum_B (|V(B)|-1) \le n-1$. That is $\le$.
...
Its optimality follows from the degree
...
At $m=3$ the upper bound covers every multihypergraph, and equality follows in the construction ranges stated above. Both rest on the rank inequality proved next, whose engine is a bound for
...
Two things have to be checked for that maximum to be attained by a multigraph over $G_0$ itself. The first is that the underlying graph of a feasible multigraph is feasible. On an edge, $\kap_{G_0}(u,v) = 1 + \pi(u,v) \le \mu(u,v) + \pi(u,v) = \kap_G(u,v) \le m-1$, since $\mu(u,v) \ge 1$. On a non-adjacent pair, $\mu(u,v) = 0$ and $\kap_{G_0}(u,v) = \pi(u,v) = \kap_G(u,v) \le m-1$. The second is that the chosen multiplicities are all at least one, so the underlying graph really is $G_0$ and no edge silently disappears: $\kap_{G_0}(u,v) \le m-1$ on an edge gives $\pi(u,v) \le m-2$, hence $\mu(u,v) = m-1-\pi(u,v) \ge 1$. Conversely any feasible multigraph is one of the multigraphs considered over its own underlying graph, so nothing is lost by maximising over $G_0$ first.
\end{proof}
...
interior. For the lower bound, take the $\mu(u,v)$ copies of the arc and a maximum
family of $\pi(u,v)$ internally disjoint directed $u$--$v$ paths of length at least two
in $D_0$, lifted into $D$ by choosing one copy of each arc. Two lifted paths cannot share
an arc without sharing an interior vertex, and a path of length at least two cannot use
the arc $uv$, so all these routes are pairwise separated. For the upper bound, a family
of separated routes contains at most $\mu(u,v)$ routes of length one, since these are
distinct copies of the arc. Its longer routes map injectively to directed paths of $D_0$,
because two routes with the same vertex sequence share an interior that is non-empty,
and their images are pairwise internally disjoint, so there are at most $\pi(u,v)$ of
them. Note that a
...
are stored in \code{program/data/machine\_values.json}, which records what
each run returned.
...
and SciPy 1.18.0. The two optional roles use Matplotlib 3.11.0 and
...
\path{program/data/equal_budget_2026-09-06/}, with full witnesses. Nineteen
runs whose calendar deadline disagreed with the measured monotonic duration
were repeated with a monotonic deadline and the same seed, and the repeat
replaces the original in every statistic.

The timing rule is cooperative, so a connectivity calculation already in
progress can overrun its target. The computer was not isolated from
...
against the $57600$-second total allocation. These totals exclude
validation and reporting.

An independent validator rebuilds the
...
These experiments are not a controlled equal-time benchmark.
...
The record saves values and requested budgets, but not each run's
elapsed time or witness. The limits are cooperative stopping targets:
...
across different machines or loads.

\code{program/data/search\_evidence.json} exports the raw search and supplied
construction values in distinct fields for every plotted order.
...
In this section, \emph{branch and bound} means exhaustive generation in
...
The directed multihypergraph arc panel at $m=3$ carries one further standalone value.
At $n=4$ and $r=3$, \code{solve} with \code{exhaustive=True} checks all $3^{12}=531\,441$ multiplicity assignments of the twelve forward hyperedges in about ten minutes, beyond the figure budget, and finds exactly $8$ (\path{program/logs/dir_multihyper_n4_log.txt}).
Every vertex of the returned witness is the tail of one hyperedge taken twice, so the bounded outgoing count of \Cref{const:bounded-outdegree-hyper} is optimal there, while the counting bound of \Cref{prop:dir-hyper-first} reads $12$.
...
In all eleven cases the target is attained and one further hyperedge is
excluded, and the transcript is
...
pairs are reported here, separately from the tables of the main program's
enumerations. The $r=2$ cross-check holds at
...
\begin{table}[ht]
  ...
  completed. Neither column is load-bearing: \Cref{sec:dir-arc-m2} proves the values at
  every $n$ from \Cref{lem:reach-skeleton}, so both columns corroborate rather
  than prove. The node counts coincide because the two feasible
...
\code{program/README.md} records the dependencies, commands and entry points.
