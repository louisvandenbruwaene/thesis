...
unique and is $K_{3,4}$ or $K_{3,5}$ with two further edges inside the side of
size three. It fails at $m = 4$, $5$ and $6$, where from $b = 7$ onwards the
maximum is attained by many non-isomorphic blocks at once, up to $38$ of them,
and none of those is a complete bipartite graph plus edges inside one side. So
the family of \Cref{thm:multi-vertex-bipartite} below is a construction
suggested by the largest $m$ in reach, and it is a lower bound rather than a
description of the extremisers. The script prints each maximiser's
...
$M(n) = \floor{n^2/4}$ for $n \ge 4$, and the difference is linear in $n$ for fixed $m$.
\end{proof}
