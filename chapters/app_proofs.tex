...
At $m = 2$ the value can also be reached by induction on $n$, deleting one vertex of least total degree and bounding what it carried by the average. That route is written out in \code{offcuts.tex} and it very nearly settles the multigraph case at every $m$ as well. Dividing every multiplicity by $m-1$, as in the scaling step of \Cref{ch:certify}, turns a feasible multigraph into a weighting with values in $[0,1]$ whose pairwise maximum flows are all at most one. Deleting a vertex of least weighted degree from that weighting drives the averaging argument through cleanly on the linear branch, and on the quadratic branch as well whenever $n$ is even. What breaks it is a single fractional remainder at odd $n$: a leftover of size $\frac{k}{2k-1}$ that an integer count could round away but a genuinely fractional weight cannot. Finding a specific vertex that is provably light enough to absorb that remainder turns out to resist every direct attempt.
...
\begin{proof}[Proof of \Cref{prop:hyper-edge}\aimedal]
...
\begin{proof}[Proof of \Cref{thm:hyper-vertex-m2}\aimedal]
...
\begin{proof}[Proof of \Cref{thm:hyper-vertex-m3}\aimedal]
...
interior, so the argument of \Cref{lem:multi-vertex-split} applies verbatim: a maximum
family may be assumed to contain all $\mu(u,v)$ copies, and the longest family it can
be extended by is a set of internally disjoint directed routes of length at least two,
which are determined by their vertex sequences and so live in $D_0$. Note that a
...
repository at \url{https://github.com/louisvandenbruwaene/thesis}. Its Git
history records the development of the code, data and thesis sources.
