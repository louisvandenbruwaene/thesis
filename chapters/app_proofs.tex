...
An upper bound needs a deletion that is guaranteed to lower every positive connectivity, paired with a bound on how much it removes. Deleting a vertex preserves feasibility, but need not lower every positive pairwise connectivity. Once multiplicities are free to sit anywhere between $0$ and $m-1$, no single vertex is guaranteed to carry a controllable share of the arcs, which is exactly the fractional remainder above. The fix is to stop insisting on a vertex and delete a whole \emph{subgraph} instead, and the reachability skeleton of \Cref{sec:reach-skeleton} is one: general enough to always exist, sparse enough to bound by \Cref{lem:reach-skeleton}, and, as the next lemma shows, specific enough that removing it lowers every positive pairwise connectivity by at least one.
...
The simple construction uses distinct $(r-1)$-sets on the non-hub vertices. The condition $m-1\le\binom{n-2}{r-2}$ bounds the requested degree cap by the number of such sets containing a fixed vertex. A multihypergraph does not, because it
...
\begin{conjecture}[The multigraph incidence value at $m = 3$, with proof,\aimedal]\label{cor:multi-vertex-m3}
$K_3(n) = 2(n-1)$ for every $n \ge 2$.
\end{conjecture}

\begin{proof}[Proposed proof]
The upper bound is \Cref{thm:hyper-vertex-m3} at $r = 2$, where the displayed bound $q(r-1) \le 2(n-1)$ reads $q \le 2(n-1)$ and $q$ is the total multiplicity. Its proof asks only that hyperedge-nodes have degree $r \ge 2$, which holds at $r = 2$. For the lower bound take a spanning tree and give every edge multiplicity two. A tree edge $uv$ has $\pi(u,v) = 0$, so $\kap(u,v) = 2$ by \Cref{lem:multi-vertex-split}, and a non-adjacent pair is joined by the unique tree path, so $\kap = 1$. Nothing exceeds $2 = m-1$, and the count is $2(n-1)$.
\end{proof}
...
If both the preceding proposed lower construction and the next proposed upper
bound are eventually verified, then first letting $n/m\to\infty$ so that optimised blocks
...
The winning blocks are never complete once $b \ge 5$, and from $m = 4$ on their
degree sequences are lopsided, a few vertices of high degree beside a majority
at degree two or three. At $m = 3$ they are the cycles, every degree two. What
...
not. The answer is therefore a block optimisation problem. In the regime
...
use the absence of orientation. The directed statement is the same one with directed
routes, and it is worth writing down because it is what carries the arc results across
to $K_m^{\mathrm{dir}}(n)$ without any asymptotic argument.
...
\begin{conjecture}[The directed multigraph incidence value, with proof,\aimedal]\label{cor:dir-multi-incidence}
For every $n \ge 2$,
\[
...
\begin{proof}[Proposed proof]
At $m = 2$ the cap is one, so \Cref{lem:multi-vertex-split-dir} forces $\mu(u,v) \le 1$
on every ordered pair and every feasible multidigraph is simple. Hence
$K_2^{\mathrm{dir}}(n) = k_2^{\mathrm{dir}}(n)$, which is $\ell_2^{\mathrm{dir}}(n)$ by
\Cref{thm:dir-vertex-m2-exact} and the displayed value by \Cref{thm:dir-arc-m2-exact}.

For general $m$ the lower bound is the arc extremiser. \Cref{thm:dir-multi-full} supplies
a multidigraph with $(m-1)M(n)$ arcs and $\lec^{\max} \le m-1$, and $\kap \le \lec$, so
that same multidigraph is feasible here. The upper bound is \Cref{thm:dir-hyper-constant}
at $r = 2$, where a forward directed $2$-uniform multihypergraph is a multidigraph and
the displayed bound reads as stated. Both sides have leading term $(m-1)n^2/4$, since
$M(n) = \floor{n^2/4}$ for $n \ge 7$, and the difference is linear in $n$ for fixed $m$.
\end{proof}
...
and NetworkX's implementation of the Edmonds and Karp algorithm. It calls neither the thesis
