...
The proof, in \Cref{app:proofs}, transports the digraph count of \Cref{thm:dir-arc-linear-error} through the incidence graph of the hypergraph, where a hyperedge becomes a node and the two-step route budget survives the translation. Since the bipartite construction already attains that leading term, it is asymptotically optimal.
...
The consequence is a change in what one has to search. Instead of graphs on $n$ vertices with $n$ growing, the question is settled by knowing the best $2$-connected block of each size, and a knapsack then assembles them. That closes every value up to $n \le 8$ and $m \le 8$ by machine, and it refutes the natural guess: from $m = 5$ a single $2$-connected block beats every way of splitting the vertices, so the bouquet of cliques is not the shape of the answer. The best blocks turn out to be dense bipartite graphs rather than complete ones, giving order $\Theta(m^2 n)$ with a per-vertex constant between $3-2\sqrt2$ and $2$ (\Cref{thm:multi-vertex-bipartite,prop:multi-vertex-upper}). Closing that gap is the open problem \Cref{tab:open-problems} records.
