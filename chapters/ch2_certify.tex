...
\def\drawstops{%
  \foreach \id/\lab in {Sa/,Sc/,Sd/,Se/u,Sj/,Sl/,Sm/v,Sn/}
     {\node[metrostop] at (\id) {$\lab$};}}
% --- panel (a): the whole hypergraph, every hyperedge emphasized ---
\begin{scope}
  \stations
  \begin{scope}[on background layer]
    \draw[metro, metroA] (Se) -- (Sa);
...
\end{scope}
% --- panel (b): the routes from v to u, only the ridden rails ---
\begin{scope}[xshift=11cm]
  \stations
  \begin{scope}[on background layer]
...
of \Cref{fig:hyper-gadget-example} read directed. Panel (a) repeats the whole map
with every hyperedge emphasized, one coloured line through its three members, the
parallel \textcolor{metroA}{ead} a doubled track. Panel (b) orients each hyperedge,
one tail and two heads, and traces the routes from $v$ to $u$, drawing a line in
bold only along the rail a route actually rides and giving it an arrow for the
direction of travel. Two edge-disjoint directed Berge routes from $v$ to $u$
survive. The long route $v \to j \to c \to l \to u$ rides \textcolor{metroB}{nmj},
\textcolor{metroG}{mcj}, \textcolor{metroC}{elc} and \textcolor{metroF}{nel} in turn,
and the short route $v \to l \to d \to u$ rides \textcolor{metroH}{elm},
