...
For the directed multigraph arc problem, then, \code{solve} runs this cut-counting optimisation in place of any enumeration. It chooses the minimum cut for every ordered pair, and a zero gap makes the reported $M^{*}(n)$ a proved upper bound. From there $L_m^{\mathrm{dir}}(n) \le (m-1)M^{*}(n)$ delivers a ceiling for every $m$ at once, met from below by the double star whenever the reported optimum is integral.
