...
One convention in that codecard deserves to be said in prose. In vertex mode every adjacency carries capacity one, so a bundle of parallel edges counts as a single direct route. Vertex-disjointness is about intermediate vertices, and parallel copies offer no new intermediate vertices to be disjoint over. Under this reading a multigraph measures exactly as its underlying simple graph, which is why the two multigraph vertex variants reduce to their simple counterparts in \Cref{tab:summary} and why every figure in this thesis reports that reduced problem for them.
