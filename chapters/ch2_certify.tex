...
\Cref{fig:hyper-gadget} shows one hyperedge in isolation. \Cref{fig:hyper-gadget-example} carries the idea to a whole hypergraph, putting the metro map (before) beside the flow network it becomes (after). A pair $u, v$ that has two hyperedge-disjoint Berge routes turns into a network carrying a maximum flow of two, so the checker reads $\lec(u,v) = 2$ straight off the flow value, and that flow value is the whole of what the checker computes.

\begin{figure}[ht]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[>=Stealth,
   route/.style={line width=1.7pt, black, line cap=round,
                 -{Stealth[length=2.6mm]}},
   end/.style={smallvertex, draw=black, line width=1.2pt}]
% Shared vertex layout, reused in both panels. The four hyperedges form two
% rails: e1,e2 share the top stretch a--b and e3,e4 the bottom stretch c--d, so
% along each rail two metro lines run side by side, far more overlap than a
% plain graph would show.
\def\hyperlayout{%
  \coordinate (u) at (0,0);
  \coordinate (a) at (2.4, 1.3);
  \coordinate (b) at (5.0, 1.3);
  \coordinate (c) at (2.4,-1.3);
  \coordinate (d) at (5.0,-1.3);
  \coordinate (v) at (7.6, 0);
}
% ===== panel (a): the hypergraph as a metro map, lines bundled on each rail ====
\begin{scope}
  \hyperlayout
  \begin{scope}[on background layer]
    % top rail: e1 sits a touch above the rail, e2 a touch below, so the two
    % lines run visibly side by side along a--b
    \draw[metro, metroA] (0,0) -- (2.4,1.38) -- (5.05,1.38);          % e1 = {u,a,b}
    \draw[metro, metroB] (2.35,1.22) -- (5.0,1.22) -- (7.6,0);        % e2 = {a,b,v}
    % bottom rail, mirrored
    \draw[metro, metroC] (0,0) -- (2.4,-1.38) -- (5.05,-1.38);        % e3 = {u,c,d}
    \draw[metro, metroD] (2.35,-1.22) -- (5.0,-1.22) -- (7.6,0);      % e4 = {c,d,v}
  \end{scope}
  \foreach \p/\l in {a/a,b/b,c/c,d/d} {\node[smallvertex] at (\p) {$\l$};}
  \node[end] at (u) {$u$};
  \node[end] at (v) {$v$};
  \node[font=\small] at (3.8,-2.35) {(a) the hypergraph};
\end{scope}
% ===== panel (b): the same lines, a turnstile per line, routes as black arcs ===
\begin{scope}[xshift=9.4cm]
  \hyperlayout
  \coordinate (h1) at (2.0, 0.65);
  \coordinate (h2) at (5.6, 0.65);
  \coordinate (h3) at (2.0,-0.65);
  \coordinate (h4) at (5.6,-0.65);
  % the same thick metro lines as panel (a), kept so the two panels read alike
  \begin{scope}[on background layer]
    \draw[metro, metroA] (0,0) -- (2.4,1.38) -- (5.05,1.38);
    \draw[metro, metroB] (2.35,1.22) -- (5.0,1.22) -- (7.6,0);
    \draw[metro, metroC] (0,0) -- (2.4,-1.38) -- (5.05,-1.38);
    \draw[metro, metroD] (2.35,-1.22) -- (5.0,-1.22) -- (7.6,0);
  \end{scope}
  % each Berge route is one black arc from u to v through the turnstiles it rides
  \draw[route] plot[smooth, tension=0.62]
        coordinates {(0,0) (2.0,0.65) (5.0,1.3) (5.6,0.65) (7.6,0)};
  \draw[route] plot[smooth, tension=0.62]
        coordinates {(0,0) (2.0,-0.65) (5.0,-1.3) (5.6,-0.65) (7.6,0)};
  \foreach \p/\l in {a/a,b/b,c/c,d/d} {\node[smallvertex] at (\p) {$\l$};}
  \node[end] at (u) {$u$};
  \node[end] at (v) {$v$};
  \node[hnode, fill=metroA!16] at (h1) {$h_{e_1}$};
  \node[hnode, fill=metroB!16] at (h2) {$h_{e_2}$};
  \node[hnode, fill=metroC!16] at (h3) {$h_{e_3}$};
  \node[hnode, fill=metroD!16] at (h4) {$h_{e_4}$};
  \node[font=\small] at (3.8,-2.35) {(b) the flow network: max flow $=2$};
\end{scope}
\end{tikzpicture}}
\caption[The helper-point network on a whole hypergraph]{From hypergraph to
flow network, before and after. \textbf{(a)} A hypergraph on six vertices, its
four hyperedges drawn as metro lines. Two lines run side by side along each
rail, $e_1 = \{u,a,b\}$ and $e_2 = \{a,b,v\}$ sharing the top stretch $a$--$b$
while $e_3 = \{u,c,d\}$ and $e_4 = \{c,d,v\}$ share the bottom stretch $c$--$d$,
the way a metro map separates two lines along one shared track. The pair $u, v$
has two hyperedge-disjoint Berge routes, $u, e_1, b, e_2, v$ along the top and
$u, e_3, d, e_4, v$ along the bottom. \textbf{(b)} The same vertices and the
same thick lines, now with a helper turnstile inside each hyperedge. Each Berge
route is drawn as one black arc running from $u$ to $v$ through the turnstiles of
the lines it rides, the top arc crossing $h_{e_1}$ and $h_{e_2}$, the bottom arc
crossing $h_{e_3}$ and $h_{e_4}$. Because every turnstile passes only one unit,
the maximum $u$--$v$ flow is exactly two, which the checker reports as
$\lec(u,v) = 2$. A third route would have to reuse a turnstile.}
