...
% A 3-uniform hypergraph on eight stations, the source layout with x and y swapped.
% Panel (a): each hyperedge as one coloured metro line through its three members,
% vertex to vertex (no helper junction). Panel (b): the same map read as the gate
% flow network for the pair u, v, with the helper point of each boarded hyperedge
% drawn as a coloured gate h and the edge-disjoint u--v routes traced through them.
\def\stations{%
...
% --- panel (a): the hypergraph as a metro map (edges only) ---
\begin{scope}
  \stations
  \begin{scope}[on background layer]
    \draw[metro, metroA] (Se)--(Sa)--(Sd);
    \draw[metro, metroB] (Sn)--(Sm)--(Sj);
    \draw[metro, metroC] (Sl)--(Se)--(Sc);
    \draw[metro, metroD] (Se)--(Sd)--(Sm);
    \draw[metro, metroF] (Sn)--(Sl)--(Se);
    \draw[metro, metroH] (Se)--(Sl)--(Sm);
    \draw[metro, metroG] (Sc)--(Sm)--(Sj);
    \draw[metro, metroE] (Sl)--(Sd)--(Sj);
  \end{scope}
  \drawstops
...
\begin{scope}[xshift=11cm]
  \stations
  \coordinate (Helc) at (1.49,3.49); \coordinate (Hmcj) at (4.57,3.43);
  \coordinate (Hedm) at (3.46,3.62); \coordinate (Helm) at (2.54,6.38);
  \coordinate (Hnel) at (0.50,6.00); \coordinate (Hnmj) at (3.27,7.81);
  \begin{scope}[on background layer]
    \draw[metro, metroA, opacity=0.30, line width=2.2pt] (Se)--(Sa)--(Sd);
    \draw[metro, metroB] (Sn)--(Sm)--(Sj);
    \draw[metro, metroC] (Sl)--(Se)--(Sc);
    \draw[metro, metroD] (Se)--(Sd)--(Sm);
    \draw[metro, metroF] (Sn)--(Sl)--(Se);
    \draw[metro, metroH] (Se)--(Sl)--(Sm);
    \draw[metro, metroG] (Sc)--(Sm)--(Sj);
    \draw[metro, metroE, opacity=0.30, line width=2.2pt] (Sl)--(Sd)--(Sj);
  \end{scope}
  % the four edge-disjoint Berge routes u -> v, each through one gate per line boarded
  \draw[flow] (Se)--(Helc); \draw[flow] (Helc)--(Sc);
  \draw[flow] (Sc)--(Hmcj); \draw[flow] (Hmcj)--(Sm);
  \draw[flow] (Se)--(Hedm); \draw[flow] (Hedm)--(Sm);
  \draw[flow] (Se)--(Helm); \draw[flow] (Helm)--(Sm);
  \draw[flow] (Se)--(Hnel); \draw[flow] (Hnel)--(Sn);
  \draw[flow] (Sn)--(Hnmj); \draw[flow] (Hnmj)--(Sm);
...
\Cref{fig:dir-hyper-gadget-example} orients the same hypergraph one-way, again in two panels. In panel (a) each hyperedge keeps its colour but becomes a directed star: the middle of its three stations is the tail, and two arcs run from that tail to the other two, the heads, so a route may enter only at a tail and leave only at a head. Panel (b) reads it as the gate network and follows the same pair $u, v$ of \Cref{fig:hyper-gadget-example}. Orientation costs connectivity, and here it is dramatic: of the four undirected routes between $u$ and $v$ only one survives the arrows, $\lec(u, v) = 1$, since $u$ is the tail of a single hyperedge and every route must leave it that one way. The lone route boards two lines in turn, through the two directed gates $h_{\bullet}$. The same vertex-splitting trick of \Cref{fig:vertex-split} laid on top of the gates then counts internally vertex-disjoint directed routes, so this one construction measures all four hypergraph variants and \code{solve} reaches every one of the twelve problems through a single checker.

\begin{figure}[htbp]
\centering
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[scale=0.92, >=Stealth,
   dmetro/.style={metro, line width=3.0pt, -{Stealth[length=2.6mm]},
                  shorten >=1.6mm, shorten <=1.2mm},
   flowarc/.style={line width=1.2pt, black!88, line cap=round, -{Stealth[length=2.4mm]},
                preaction={draw, -, line width=3.0pt, white}},
   gate/.style={circle, fill=white, draw=black!30, line width=0.6pt,
                inner sep=0.4pt, minimum size=12.5pt, font=\footnotesize}]
% The hypergraph of fig:hyper-gadget-example, oriented one-way (middle station is
% the tail), x and y swapped. Panel (a): each hyperedge as a directed star, two
% arcs tail->head, vertex to vertex. Panel (b): the gate flow network for u, v;
% only one directed route survives, u -> [nel] -> l -> [elm] -> v.
\def\stations{%
  \coordinate (Sa) at (0,0);  \coordinate (Sc) at (2,2);  \coordinate (Sd) at (6,2);
  \coordinate (Se) at (0,4);  \coordinate (Sj) at (8,4);  \coordinate (Sl) at (2,6);
  \coordinate (Sm) at (6,6);  \coordinate (Sn) at (0,8);}
\def\drawstops{%
  \foreach \id/\lab in {Sa/,Sc/,Sd/,Se/u,Sj/,Sl/,Sm/v,Sn/}
     {\node[metrostop] at (\id) {$\lab$};}}
% --- panel (a): the directed hypergraph (middle station is the tail) ---
\begin{scope}
  \stations
  \begin{scope}[on background layer]
    \draw[dmetro, metroA] (Sa)--(Se); \draw[dmetro, metroA] (Sa)--(Sd);
    \draw[dmetro, metroB] (Sm)--(Sn); \draw[dmetro, metroB] (Sm)--(Sj);
    \draw[dmetro, metroC] (Sl)--(Se); \draw[dmetro, metroC] (Sl)--(Sc);
    \draw[dmetro, metroD] (Sd)--(Se); \draw[dmetro, metroD] (Sd)--(Sm);
    \draw[dmetro, metroF] (Se)--(Sn); \draw[dmetro, metroF] (Se)--(Sl);
    \draw[dmetro, metroH] (Sl)--(Se); \draw[dmetro, metroH] (Sl)--(Sm);
    \draw[dmetro, metroG] (Sc)--(Sm); \draw[dmetro, metroG] (Sc)--(Sj);
    \draw[dmetro, metroE] (Sd)--(Sl); \draw[dmetro, metroE] (Sd)--(Sj);
  \end{scope}
  \drawstops
  \node[font=\small] at (4,-1.4) {(a)};
\end{scope}
% --- panel (b): the gate flow network; the lone directed route u -> v ---
\begin{scope}[xshift=11cm]
  \stations
  \coordinate (Hnel) at (0.43,5.57); \coordinate (Helm) at (4.00,6.80);
  \begin{scope}[on background layer]
    \draw[dmetro, metroA, opacity=0.28, line width=2.2pt] (Sa)--(Se); \draw[dmetro, metroA, opacity=0.28, line width=2.2pt] (Sa)--(Sd);
    \draw[dmetro, metroB, opacity=0.28, line width=2.2pt] (Sm)--(Sn); \draw[dmetro, metroB, opacity=0.28, line width=2.2pt] (Sm)--(Sj);
    \draw[dmetro, metroC, opacity=0.28, line width=2.2pt] (Sl)--(Se); \draw[dmetro, metroC, opacity=0.28, line width=2.2pt] (Sl)--(Sc);
    \draw[dmetro, metroD, opacity=0.28, line width=2.2pt] (Sd)--(Se); \draw[dmetro, metroD, opacity=0.28, line width=2.2pt] (Sd)--(Sm);
    \draw[dmetro, metroF] (Se)--(Sn); \draw[dmetro, metroF] (Se)--(Sl);
    \draw[dmetro, metroH] (Sl)--(Se); \draw[dmetro, metroH] (Sl)--(Sm);
    \draw[dmetro, metroG, opacity=0.28, line width=2.2pt] (Sc)--(Sm); \draw[dmetro, metroG, opacity=0.28, line width=2.2pt] (Sc)--(Sj);
    \draw[dmetro, metroE, opacity=0.28, line width=2.2pt] (Sd)--(Sl); \draw[dmetro, metroE, opacity=0.28, line width=2.2pt] (Sd)--(Sj);
  \end{scope}
  \draw[flowarc] (Se)--(Hnel); \draw[flowarc] (Hnel)--(Sl);
  \draw[flowarc] (Sl)--(Helm); \draw[flowarc] (Helm)--(Sm);
  \node[gate] at (Hnel) {$h_{\textcolor{metroF}{\bullet}}$};
  \node[gate] at (Helm) {$h_{\textcolor{metroH}{\bullet}}$};
  \drawstops
  \node[font=\small] at (4,-1.4) {(b)};
...
middle of its three stations is the tail. Panel (a) draws every coloured line as a
one-way star, two arcs running from the tail to the two heads, so a Berge route
enters at a tail and leaves at a head. Panel (b) reads it as the gate network for
the same pair $u, v$: each gate $h_{\bullet}$ is now the directed turnstile of
\Cref{fig:dir-hyper-gadget}, and lines off the route are faded. The arrows forbid
the reverse transfers the undirected map allowed, so connectivity drops sharply:
only one directed route survives, $u$ boarding one line to an intermediate station
and a second on to $v$, and the checker reads $\lec(u, v) = 1$, against the
undirected four of \Cref{fig:hyper-gadget-example}.}
