...
A faster route was suggested by Prof.\ Jan Goedgebeur, an author of the \texttt{nauty} generators \cite{McKayPiperno14}. Rather than search, one generates. The tool \texttt{geng} lists the non-isomorphic undirected graphs on $n$ vertices, and the program then decorates each one edge by edge with a directed multiplicity pair, so that the underlying support graph already carries the isomorphism reduction. The orientation tools \texttt{directg} and \texttt{watercluster2} are not used directly here: each produces a single orientation per edge and so cannot express the bidirected arcs and the repeated multiplicities the problem needs (the extremal graphs are themselves bidirected), which is exactly why the multiplicity decoration is layered in-house on top of \texttt{geng}. The expensive part, listing one representative per isomorphism class, now happens inside \texttt{geng}, where \texttt{nauty} is heavily optimised, rather than inside the search.

The two routes agree to the last graph. The generated pipeline reproduces the in-house enumeration exactly at every settled size, two non-isomorphic extremal families at $n = 4$ and three at $n = 5$, and it is substantially faster, because the only prunings it never has to repeat are the isomorphism reductions \texttt{geng} has already done. One difference matters for what comes next: where the in-house search leans on a conjectured counting bound past $n = 6$, the generated enumeration applies only proved bounds, so it is a sound complete search at every $n$. That makes it the practical route to the single finite case the $m = 3$ argument still waits on, the degree-bounded enumeration at $n = 7$ that is fact (b) of \Cref{rem:odd-step-roadmap}.
