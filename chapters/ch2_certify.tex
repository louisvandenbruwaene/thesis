...
\[
    ...
    \qquad \text{for every } m \ge 2 ,
\]
so that one solve settles infinitely many values. This is exactly what happens at every $n \le 6$, which is what \Cref{thm:dir-multi-small} rests on. It is a verified property of those optima rather than a theorem about all $n$, and where it is not verified the safe reading is the inequality: an integral question such as fact (a) of \Cref{rem:odd-step-roadmap} is then asked of the integral program \code{prove\_integral\_arc\_bound} directly, not deduced from a fractional value.
...
\end{tikzpicture}
\caption{The scaling step for $m = 3$. Dividing each multiplicity by $m - 1 = 2$ maps the integer multigraph (left) to a weight matrix with all values in $[0, 1]$ and max-flow at most one between every pair (right). The double arc $A \to B$ (multiplicity $2$) becomes weight $1$, the single arc $A \to C$ (multiplicity $1$) becomes weight $\tfrac{1}{2}$, and the absent arc $B \to C$ (multiplicity $0$, shown dashed) stays absent. Crucially, $m$ drops out on the right, so a single optimisation over all weight matrices bounds $L_m^{\mathrm{dir}}(n) \le (m-1)\,M^*(n)$ for every $m$ simultaneously. The reverse inequality is not automatic and needs the optimum to be attained by a $\{0,1\}$ matrix, which is a verified property of the optima at $n \le 6$ rather than a theorem about all $n$, as the text above sets out.}
...
Second, every reported connectivity is exact and integral. The checker is a single integer maximum flow, so no extremal value in the thesis is a rounded floating-point quantity. Real numbers enter in exactly two places, and neither carries a reported extremal value on its own. The first is the fractional optimisation of \Cref{thm:dir-multi-small}, where the reported optimum is a $\{0,1\}$ matrix and is checked exactly. The second is the fractional hill-climb behind \Cref{rem:min-degree-obstruction}, which runs a floating-point maximum flow and is reported as numerical evidence for a conjecture, never as a value.

Third, the load-bearing computations are run twice, by implementations that share no code. The base cases of \Cref{thm:dir-arc-m2-exact} and \Cref{thm:dir-vertex-m2-exact} were each confirmed by the program and by separately written checkers, agreeing on every size. The values of \Cref{sec:multi-vertex-standard} were computed by the program's routine and by an independent script, agreeing on every cell. Fact (b) of \Cref{rem:odd-step-roadmap} was produced by the generation enumerator and re-verified graph by graph with the exact checker. Two implementations agreeing is not a proof, but it is the practical guard against the failure mode a certificate would catch, namely a correct method with a wrong program.
...
What this does not amount to is a formally checkable certificate of the kind a SAT solver can emit, such as a resolution proof a small independent verifier could replay. For the one claim here that \emph{would} rest on a solver's infeasibility verdict rather than on an exhaustion, fact (a) of \Cref{rem:odd-step-roadmap}, that would be the right standard to aim at. Fact (a) is not settled either way at present, so nothing in this thesis currently rests on such a verdict, and it is recorded among the open problems rather than claimed.
