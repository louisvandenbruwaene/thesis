...
\Cref{fig:dir-hyper-gadget-example} reads the same hypergraph directed, again in two panels. Panel (a) gives the whole hypergraph oriented, every hyperedge a one-way line whose arrows point away from its tail, so the middle-tailed lines fan out from their centre and the end-tailed ones run one way along their length. Panel (b) traces the routes from $v$ to $u$, the gate rule of \Cref{fig:dir-hyper-gadget}, drawing each hyperedge only along the rail a route actually rides and marking its one-route gate $h_{\bullet}$ in that line's colour, so the map stays a clean metro picture. Orientation costs connectivity. Of the four undirected routes between $u$ and $v$ only two survive the arrows, $\lec(v, u) = 2$, because $v$ can set out through only the two hyperedges it heads, and the two survivors cross at the interchange $l$ so they share no hyperedge. The same vertex-splitting trick of \Cref{fig:vertex-split} laid on top then counts internally vertex-disjoint directed routes, so this one construction measures all four hypergraph variants and \code{solve} reaches every one of the twelve problems through a single checker.
...
\begin{scope}[xshift=11cm]
  ...
  % dotted leg from each gate to the hyperedge's unused third member
  \draw[dotleg, metroB] (7.35,5.35) -- (Sn);
  \draw[dotleg, metroG] (4.84,3.47) -- (Sm);
  \draw[dotleg, metroC] (1.5,4)     -- (Se);
  \draw[dotleg, metroF] (0.65,5.35) -- (Sn);
  \draw[dotleg, metroH] (4,5.5)     -- (Se);
  \draw[dotleg, metroE] (4.35,4.35) -- (Sj);
  \draw[dotleg, metroD] (2.84,2.53) -- (Sm);
  \node[gate] at (7.35,5.35) {$h_{\textcolor{metroB}{\bullet}}$};
  \node[gate] at (4.84,3.47) {$h_{\textcolor{metroG}{\bullet}}$};
  \node[gate] at (1.5,4)     {$h_{\textcolor{metroC}{\bullet}}$};
  \node[gate] at (0.65,5.35) {$h_{\textcolor{metroF}{\bullet}}$};
  \node[gate] at (4,5.5)     {$h_{\textcolor{metroH}{\bullet}}$};
  \node[gate] at (4.35,4.35) {$h_{\textcolor{metroE}{\bullet}}$};
  \node[gate] at (2.84,2.53) {$h_{\textcolor{metroD}{\bullet}}$};
  \drawstops
...
(b) traces the routes from $v$ to $u$, drawing each hyperedge in bold only along the
rail a route rides and setting its one-route gate $h_{\bullet}$ beside that rail, in
the line's colour, the directed turnstile of \Cref{fig:dir-hyper-gadget}. A dotted
leg runs from each gate to the hyperedge's third member, the station the route does
not visit. Two edge-disjoint directed Berge routes from $v$ to $u$ survive. The long route
