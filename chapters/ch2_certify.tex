...
\begin{scope}[xshift=11cm]
  ...
  \begin{scope}[on background layer]
    ...
    \draw[metro, metroB] (Sn) -- (Sm);
    \draw[metro, metroB, opacity=0.28, line width=2.2pt] (Sm) -- (Sj);
    \draw[metro, metroC] ($(Se)+(0.14,-0.14)$) -- ($(Sl)+(0.14,-0.14)$);
    \draw[metro, metroC] (Sl) -- (Sc);
    \draw[metro, metroD] (Se) -- (Sd);
    \draw[metro, metroD] (Sd) -- (Sm);
    \draw[metro, metroF] (Sn) -- (Se);
    \draw[metro, metroF, opacity=0.28, line width=2.2pt] (Se) -- (Sl);
    \draw[metro, metroH] ($(Se)+(-0.14,0.14)$) -- ($(Sl)+(-0.14,0.14)$);
    \draw[metro, metroH] (Sl) -- (Sm);
    \draw[metro, metroG] (Sm) -- (Sc);
    \draw[metro, metroG, opacity=0.28, line width=2.2pt] (Sc) -- (Sj);
...
lines on no $u$--$v$ route are faded. Two routes pass straight through the two hyperedges that
...
\begin{scope}[xshift=11cm]
  ...
  \end{scope}
  % used rails, bold in colour (the metro lines a route rides), no arrows
  \draw[metro, metroB] (Sm) -- (Sj);
  \draw[metro, metroG] (Sj) -- (Sc);
  \draw[metro, metroC] (Sc) -- (Sl);
  \draw[metro, metroF] (Sl) -- (Se);
  \draw[metro, metroH] (Sm) -- (Sl);
  \draw[metro, metroE] (Sl) -- (Sd);
  \draw[metro, metroD] (Sd) -- (Se);
  % helper points, placed in clear gaps off the rails
  \coordinate (Gnmj) at (7.2,5.9);
  \coordinate (Gmcj) at (4.3,2.0);
  \coordinate (Gelc) at (1.0,4.2);
  \coordinate (Gnel) at (0.7,5.3);
  \coordinate (Gelm) at (4.3,5.5);
  \coordinate (Gldj) at (5.2,4.2);
  \coordinate (Gedm) at (2.3,2.85);
...
(b) traces the routes from $v$ to $u$. Each ridden hyperedge keeps its used rail in
bold colour, and a black arrow runs from the route's entry station through the
