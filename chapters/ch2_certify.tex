...
\[
    ...
    \qquad \text{for every } m \ge 2 ,
\]
so that one solve settles infinitely many values. This is exactly what happens at every $n \le 6$, which is what \Cref{thm:dir-multi-small} rests on. It is a verified property of those optima rather than a theorem about all $n$: whether $M^{*}(n)$ itself is always attained by a $\{0,1\}$ matrix stays open past $n = 6$, and \code{prove\_integral\_arc\_bound} exists precisely to answer an integral question, such as $L_3^{\mathrm{dir}}(7) = 24$, directly, without leaning on that unverified property. \Cref{thm:dir-multi-full} (\Cref{app:proofs}) settles the integral question for every $n$ by an entirely different, elementary argument, so this MILP route now serves as an independent check on it rather than the only way to reach it: the same call, run to completion, agrees with the hand proof (\Cref{cor:dir-multi-n7}).
