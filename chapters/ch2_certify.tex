...
\coordinate (a) at (0.0, 0.85);
\coordinate (b) at (-0.1,-0.1);
\coordinate (c) at (0.65,-0.9);
\begin{scope}[on background layer]
...
\node[font=\small] at (0.2, -1.55) {hyperedge $e = \{a,b,c\}$};
% --- arrow ---
\draw[->, very thick, color=KULblauw3b] (1.9, -0.1) -- (3.0, -0.1);
\node[above, font=\small] at (2.45, -0.05) {becomes};
% --- right: the gadget ---
\begin{scope}[xshift=4.6cm]
\node[smallvertex] (a2) at (0, 0.85) {$a$};
\node[smallvertex] (b2) at (0, -0.05) {$b$};
\node[smallvertex] (c2) at (0, -0.95) {$c$};
\node[hnode, fill=metroA!20, minimum size=15pt] (h) at (2.0, -0.05) {$h_e$};
\draw[edge] (a2) -- (h);
\draw[edge] (b2) -- (h);
\draw[edge] (c2) -- (h);
\node[font=\small, align=center] at (2.0, -1.55) {helper point\\(one route only)};
\end{scope}
\end{tikzpicture}
...
\resizebox{\textwidth}{!}{%
\begin{tikzpicture}[>=Stealth,
   mem/.style={line width=0.7pt, black!32},
   tri/.style={line width=0.8pt, black!16},
   fa/.style={line width=2.0pt, vertexorange, -{Stealth[length=2mm]}},
   fb/.style={line width=2.0pt, vertexgreen!70!black, -{Stealth[length=2mm]}},
   end/.style={smallvertex, draw=black, line width=1.2pt}]
% shared vertex layout, reused in both panels. Positions are chosen so the five
% hyperedge triangles do not overlap and each helper sits strictly inside the
% triangle of its three vertices.
\def\hyperlayout{%
  \coordinate (u) at (0,0);
  \coordinate (a) at (1.4, 2.1);
  \coordinate (b) at (3.5, 1.6);
  \coordinate (c) at (6.0, 2.1);
  \coordinate (v) at (7.4, 0);
  \coordinate (d) at (1.4,-2.1);
  \coordinate (e) at (3.5,-1.6);
  \coordinate (f) at (6.0,-2.1);
  \coordinate (w) at (4.9, 0);
}
% ===== panel (a): the hypergraph as a metro map =====
\begin{scope}
  \hyperlayout
  \begin{scope}[on background layer]
    \draw[metro, metroA] (u)--(a)--(b);   % e1 = {u,a,b}
    \draw[metro, metroB] (b)--(c)--(v);   % e2 = {b,c,v}
    \draw[metro, metroC] (u)--(d)--(e);   % e3 = {u,d,e}
    \draw[metro, metroD] (e)--(f)--(v);   % e4 = {e,f,v}
    \draw[metro, metroE] (b)--(w)--(e);   % e5 = {b,w,e}
  \end{scope}
  \foreach \p/\l in {a/a,b/b,c/c,d/d,e/e,f/f,w/w} {\node[smallvertex] at (\p) {$\l$};}
  \node[end] at (u) {$u$};
  \node[end] at (v) {$v$};
  \node[metroA, font=\scriptsize\bfseries] at (0.5, 1.25)  {$e_1$};
  \node[metroB, font=\scriptsize\bfseries] at (4.6, 2.1)   {$e_2$};
  \node[metroC, font=\scriptsize\bfseries] at (0.5,-1.25)  {$e_3$};
  \node[metroD, font=\scriptsize\bfseries] at (4.6,-2.1)   {$e_4$};
  \node[metroE, font=\scriptsize\bfseries] at (4.35,0.55)  {$e_5$};
  \node[font=\small] at (3.7,-2.85) {(a) the hypergraph};
\end{scope}
% ===== panel (b): the flow network (a helper turnstile inside each triangle) =====
\begin{scope}[xshift=9.3cm]
  \hyperlayout
  \coordinate (h1) at (1.63, 1.23);
  \coordinate (h2) at (5.63, 1.23);
  \coordinate (h3) at (1.63,-1.23);
  \coordinate (h4) at (5.63,-1.23);
  \coordinate (h5) at (3.97, 0);
  % faint hyperedge triangles, so each helper sits visibly inside its members
  \draw[tri] (u)--(a)--(b)--cycle;
  \draw[tri] (b)--(c)--(v)--cycle;
  \draw[tri] (u)--(d)--(e)--cycle;
  \draw[tri] (e)--(f)--(v)--cycle;
  \draw[tri] (b)--(w)--(e)--cycle;
  % faint membership edges from each vertex to its helper
  \draw[mem] (u)--(h1) (a)--(h1) (b)--(h1);
  \draw[mem] (b)--(h2) (c)--(h2) (v)--(h2);
  \draw[mem] (u)--(h3) (d)--(h3) (e)--(h3);
  \draw[mem] (e)--(h4) (f)--(h4) (v)--(h4);
  \draw[mem] (b)--(h5) (w)--(h5) (e)--(h5);
  % the two hyperedge-disjoint routes as flow paths through helpers
  \draw[fa] (u)--(h1); \draw[fa] (h1)--(b); \draw[fa] (b)--(h2); \draw[fa] (h2)--(v);
  \draw[fb] (u)--(h3); \draw[fb] (h3)--(e); \draw[fb] (e)--(h4); \draw[fb] (h4)--(v);
  \foreach \p/\l in {a/a,b/b,c/c,d/d,e/e,f/f,w/w} {\node[smallvertex] at (\p) {$\l$};}
  \node[end] at (u) {$u$};
  \node[end] at (v) {$v$};
  \node[hnode, fill=metroA!16] at (h1) {};
  \node[hnode, fill=metroB!16] at (h2) {};
  \node[hnode, fill=metroC!16] at (h3) {};
  \node[hnode, fill=metroD!16] at (h4) {};
  \node[hnode, fill=metroE!16] at (h5) {};
  \node[font=\small] at (3.7,-2.85) {(b) the flow network: max flow $=2$};
\end{scope}
\end{tikzpicture}}
\caption[The helper-point network on a whole hypergraph]{From hypergraph to
flow network, before and after. \textbf{(a)} A hypergraph on nine vertices,
its five hyperedges drawn as metro lines whose triangles do not overlap. The
pair $u, v$ has two hyperedge-disjoint Berge routes, $u, e_1, b, e_2, v$ and
$u, e_3, e, e_4, v$. \textbf{(b)} The same vertices, with one helper square
sitting inside each hyperedge's triangle (a turnstile on the line). Each Berge
route becomes a flow path that enters a
line through one member, crosses its helper, and leaves through another:
$u \to h_{e_1} \to b \to h_{e_2} \to v$ (orange) and
$u \to h_{e_3} \to e \to h_{e_4} \to v$ (green). Because every helper carries
only one unit, the maximum $u$--$v$ flow is exactly two, which the checker
reports as $\lec(u,v) = 2$. A third route would have to reuse a helper.}
