...
\Cref{fig:vertex-split-example} applies the split to a whole digraph. Two arc-disjoint routes are forced through one shared vertex $w$, so on the split network both must cross the single unit-capacity arc inside $w$, which caps the number of internally vertex-disjoint routes below the edge-disjoint count.
...
% ----- panel (a): original digraph, two routes through w -----
\begin{scope}
  \node[smallvertex] (s) at (0,0) {$s$};
  \node[smallvertex] (w) at (3,0) {$w$};
  \node[smallvertex] (t) at (6,0) {$t$};
  \node[smallvertex] (a) at (1.5,1.55) {$a$};
  \node[smallvertex] (b) at (4.5,1.55) {$b$};
  \draw[rxa] (s)--(w); \draw[rxa] (w)--(t);
  \draw[rxb] (s)--(a); \draw[rxb] (a)--(w); \draw[rxb] (w)--(b); \draw[rxb] (b)--(t);
  \node[font=\scriptsize] at (3,-1.05) {$\lec(s,t)=2$, both routes use $w$};
  \node[font=\small] at (3,-1.85) {(a) original digraph};
\end{scope}
% ----- panel (b): split network, w_in -> w_out is the cap-1 bottleneck -----
\begin{scope}[xshift=8.8cm]
  \foreach \name/\x/\y in {s/0/0, w/3/0, t/6/0, a/1.5/1.55, b/4.5/1.55}{
    \node[sp] (\name-i) at (\x-0.34,\y) {};
    \node[sp] (\name-o) at (\x+0.34,\y) {};
    \node[font=\scriptsize, yshift=11pt] at ($(\name-i)!.5!(\name-o)$) {$\name$};
  }
  \foreach \name in {s,a,t,b}{\draw[capf] (\name-i)--(\name-o);}
  \draw[capw] (w-i)--(w-o);
  \node[vertexred, font=\scriptsize] at (3,-0.55) {cap $1$};
  % external route arcs, out -> in
  \draw[rxa] (s-o) to[bend left=6] (w-i);
  \draw[rxa] (w-o) to[bend left=6] (t-i);
  \draw[rxb] (s-o) to[bend left=12] (a-i);
  \draw[rxb] (a-o) to[bend right=6] (w-i);
  \draw[rxb] (w-o) to[bend left=6] (b-i);
  \draw[rxb] (b-o) to[bend left=12] (t-i);
  \node[font=\small] at (3,-1.85) {(b) split network: $\kap(s,t)=1$};
\end{scope}
\end{tikzpicture}
\caption[Vertex splitting on a whole digraph]{The vertex-split checker on a
larger example. \textbf{(a)} A digraph in which the two arc-disjoint $s$--$t$
routes, $s \to w \to t$ (orange) and $s \to a \to w \to b \to t$ (green), both
pass through $w$, so $\lec(s,t)=2$. \textbf{(b)} The split network replaces
every vertex $V$ by an arc $V^{\mathrm{in}} \to V^{\mathrm{out}}$ of capacity
one. Both routes must now traverse the single unit arc inside $w$ (red), which
carries only one unit, so at most one internally vertex-disjoint route gets
through and $\kap(s,t)=1$. Splitting every vertex this way turns the
vertex-disjoint count into an ordinary maximum flow.}
...
\Cref{fig:hyper-gadget} shows one hyperedge in isolation. \Cref{fig:hyper-gadget-example} carries the same idea to a whole small hypergraph, where a Berge route that switches hyperedge partway becomes a single flow path threading two helper points in turn.

\begin{figure}[ht]
\centering
\begin{tikzpicture}[scale=1.0,
   mem/.style={line width=1pt, black!40},
   fA/.style={line width=2.1pt, vertexorange}]
  % vertices in a left column
  \node[smallvertex] (u) at (0, 2.0) {$u$};
  \node[smallvertex] (x) at (0, 1.0) {$x$};
  \node[smallvertex] (y) at (0, 0.0) {$y$};
  \node[smallvertex] (z) at (0,-1.0) {$z$};
  \node[smallvertex] (v) at (0,-2.0) {$v$};
  % two helper squares to the right
  \node[hnode, fill=vertexorange!18, minimum size=15pt] (h1) at (2.7, 1.2) {$h_{e_1}$};
  \node[hnode, fill=KULblauw3b!16, minimum size=15pt] (h2) at (2.7,-1.2) {$h_{e_2}$};
  % membership edges: e1 = {u,x,y}, e2 = {y,z,v}
  \draw[mem] (u)--(h1); \draw[mem] (x)--(h1); \draw[mem] (y)--(h1);
  \draw[mem] (y)--(h2); \draw[mem] (z)--(h2); \draw[mem] (v)--(h2);
  % the Berge route u,e1,y,e2,v as one flow path through both helpers
  \draw[fA] (u)--(h1); \draw[fA] (h1)--(y); \draw[fA] (y)--(h2); \draw[fA] (h2)--(v);
  \node[font=\scriptsize, align=center] at (0,-2.75) {hypergraph vertices};
  \node[font=\scriptsize, align=center] at (2.7,-2.2)
     {helper points\\(one route each)};
  \node[font=\scriptsize, vertexorange!75!black] at (1.75,2.15)
     {route $u, e_1, y, e_2, v$};
\end{tikzpicture}
\caption[The helper-point network on a two-hyperedge route]{The same
construction on a two-hyperedge hypergraph $e_1 = \{u,x,y\}$,
$e_2 = \{y,z,v\}$. The Berge route that enters $e_1$ at $u$, switches
hyperedge at the shared vertex $y$, and leaves $e_2$ at $v$ becomes the single
flow path $u \to h_{e_1} \to y \to h_{e_2} \to v$ (orange), entering each
hyperedge through one member and leaving through another. Because each helper
carries only one unit of flow, a second $u$--$v$ route would have to avoid
both helpers, which is exactly the rule that one hyperedge serves one route.}
