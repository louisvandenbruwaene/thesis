...
\Cref{fig:hyper-gadget} shows one hyperedge in isolation. \Cref{fig:hyper-gadget-example} carries the idea to a whole hypergraph and folds the before and after into one picture. Each of the eight hyperedges is one coloured line that meets its three members at a helper junction, and that junction, boxed and named after the edge, is exactly the gate of \Cref{fig:hyper-gadget}. Read as a metro map the picture is the hypergraph; read as boxes that pass one route each it is the flow network, with no second diagram needed. Several lines share the central spine down to the hub $e$, where they run side by side as parallel coloured rails, the way a metro map separates lines along one stretch of track. Run on this hypergraph the checker reads $\lec(e, l) = 4$: the pair $e, l$ lies on the three common hyperedges $\{e,l,c\}$, $\{e,l,m\}$ and $\{n,e,l\}$ and carries a fourth, longer Berge route besides, while a fifth would have to reuse a gate.

\begin{figure}[htbp]
\centering
\resizebox{0.60\textwidth}{!}{%
\begin{tikzpicture}[scale=1.0,
   gate/.style={hnode, inner sep=1.5pt,
                font=\fontsize{6.4}{7}\selectfont\bfseries}]
% A 3-uniform hypergraph on eight stations a,c,d,e,j,l,m,n. Each of the eight
% hyperedges is one coloured line meeting its three members at a helper junction,
% drawn as a box named after the edge -- the gate of fig:hyper-gadget. Three lines
% share the central spine (x=4) down to the hub e and one runs the upper spine to
% j; on a shared stretch they are nudged into parallel lanes so they read as
% separate rails rather than one smeared line, the metro-map shared-track effect.
\def\stationcoords{%
  \coordinate (Sa) at (0,0);  \coordinate (Sc) at (2,2);  \coordinate (Sd) at (2,6);
  \coordinate (Se) at (4,0);  \coordinate (Sj) at (4,8);  \coordinate (Sl) at (6,2);
  \coordinate (Sm) at (6,6);  \coordinate (Sn) at (8,0);
  \coordinate (Head) at (1,1); \coordinate (Hnmj) at (8,4); \coordinate (Helc) at (4,1);
  \coordinate (Hedm) at (4,7); \coordinate (Hnel) at (6,1); \coordinate (Helm) at (4,3);
  \coordinate (Hmcj) at (4,5); \coordinate (Hldj) at (5.2,5.2);
}
\def\hyperlines{%
  \draw[metro, metroA] (Sa)--(Head); \draw[metro, metroA] (Head)--(Se); \draw[metro, metroA] (Head)--(Sd);
  \draw[metro, metroB] (Sm)--(Hnmj); \draw[metro, metroB] (Hnmj)--(Sn); \draw[metro, metroB] (Hnmj)--(Sj);
  \draw[metro, metroC] (Sl)--(Helc); \draw[metro, metroC] (Helc)--(Sc);
     \draw[metro, metroC] (Se)--(3.62,0.5)--(Helc);                       % e--elc rail
  \draw[metro, metroD] (Sd)--(Hedm); \draw[metro, metroD] (Hedm)--(Sm);
     \draw[metro, metroD] (Se)--(4.42,0.5)--(4.42,6.5)--(Hedm);           % e--edm rail
  \draw[metro, metroF] (Se)--(Hnel); \draw[metro, metroF] (Hnel)--(Sn); \draw[metro, metroF] (Hnel)--(Sl);
  \draw[metro, metroH] (Sl)--(Helm); \draw[metro, metroH] (Helm)--(Sm);
     \draw[metro, metroH] (Se)--(3.40,0.6)--(3.40,2.5)--(Helm);           % e--elm rail
  \draw[metro, metroG] (Sc)--(Hmcj); \draw[metro, metroG] (Hmcj)--(Sm);
     \draw[metro, metroG] (Sj)--(3.55,7.5)--(3.55,5.5)--(Hmcj);           % j--mcj rail
  \draw[metro, metroE] (Sd)--(Hldj); \draw[metro, metroE] (Hldj)--(Sl); \draw[metro, metroE] (Hldj)--(Sj);
}
\stationcoords
\begin{scope}[on background layer] \hyperlines \end{scope}
\foreach \id/\lab in {Sa/a,Sc/c,Sd/d,Se/e,Sj/j,Sl/l,Sm/m,Sn/n}
   {\node[metrostop] at (\id) {$\lab$};}
\node[gate] at (Head) {ead}; \node[gate] at (Hnmj) {nmj};
\node[gate] at (Helc) {elc}; \node[gate] at (Hedm) {edm};
\node[gate] at (Hnel) {nel}; \node[gate] at (Helm) {elm};
\node[gate] at (Hmcj) {mcj}; \node[gate] at (Hldj) {ldj};
\end{tikzpicture}}
\caption[The helper-point network on a whole hypergraph]{A $3$-uniform
hypergraph on eight vertices, drawn as a metro map that is at the same time its
helper-point flow network. The stations are $a, c, d, e, j, l, m, n$, and each of
the eight hyperedges is one coloured line threading its three members through a
helper junction, boxed and named after the edge, so $\{e, a, d\}$ is the line
\textsf{ead} and so on. Each box is the one-route gate of \Cref{fig:hyper-gadget},
which is why the single drawing is both the hypergraph and the network the checker
measures. The lines that share the central spine to the hub $e$ run side by side as
parallel rails, the metro-map rendering of several hyperedges sharing a stretch of
track. The pair $e, l$ lies on the three hyperedges $\{e,l,c\}$, $\{e,l,m\}$ and
$\{n,e,l\}$ and carries one longer route besides, four in all, so the checker reads
$\lec(e, l) = 4$. A fifth would have to reuse a gate.}
...
\Cref{fig:dir-hyper-gadget-example} orients the same hypergraph one-way. Each hyperedge keeps its colour but becomes a directed star: the middle of its three vertices is the tail, and two arcs run from that tail through the gate to the other two, the heads, so a route may enter only at a tail and leave only at a head. The boxes are now the directed turnstiles of \Cref{fig:dir-hyper-gadget}. Orientation costs connectivity, and the checker reads it off: where the undirected pair $e, l$ had four routes, the busiest directed pair $d, e$ has only two, $\lec(d, e) = 2$, because a route must now follow the arrows and most reverse transfers are forbidden. The same vertex-splitting trick of \Cref{fig:vertex-split} laid on top of the gates then counts internally vertex-disjoint directed routes, so this one construction measures all four hypergraph variants and \code{solve} reaches every one of the twelve problems through a single checker.

\begin{figure}[htbp]
\centering
\resizebox{0.60\textwidth}{!}{%
\begin{tikzpicture}[scale=1.0,
   dmetro/.style={metro, -{Stealth[length=2.6mm]}, shorten >=1.7mm, shorten <=1.2mm},
   gate/.style={hnode, inner sep=1.5pt,
                font=\fontsize{6.4}{7}\selectfont\bfseries}]
% The hypergraph of fig:hyper-gadget-example, oriented one-way: the middle of each
% hyperedge's three vertices is the tail, two arcs running from it through the gate
% to the two heads. Same stations and helper boxes as the undirected figure; the
% shared-spine rails now point toward their head (the hub e, or j for mcj).
\def\stationcoords{%
  \coordinate (Sa) at (0,0);  \coordinate (Sc) at (2,2);  \coordinate (Sd) at (2,6);
  \coordinate (Se) at (4,0);  \coordinate (Sj) at (4,8);  \coordinate (Sl) at (6,2);
  \coordinate (Sm) at (6,6);  \coordinate (Sn) at (8,0);
...
\def\dhyperlines{%
  \draw[dmetro, metroA] (Sa)--(Head); \draw[dmetro, metroA] (Head)--(Se); \draw[dmetro, metroA] (Head)--(Sd);
  \draw[dmetro, metroB] (Sm)--(Hnmj); \draw[dmetro, metroB] (Hnmj)--(Sn); \draw[dmetro, metroB] (Hnmj)--(Sj);
  \draw[dmetro, metroC] (Sl)--(Helc); \draw[dmetro, metroC] (Helc)--(Sc);
     \draw[dmetro, metroC] (Helc)--(3.62,0.5)--(Se);
  \draw[dmetro, metroD] (Sd)--(Hedm); \draw[dmetro, metroD] (Hedm)--(Sm);
     \draw[dmetro, metroD] (Hedm)--(4.42,6.5)--(4.42,0.5)--(Se);
  \draw[dmetro, metroF] (Se)--(Hnel); \draw[dmetro, metroF] (Hnel)--(Sn); \draw[dmetro, metroF] (Hnel)--(Sl);
  \draw[dmetro, metroH] (Sl)--(Helm); \draw[dmetro, metroH] (Helm)--(Sm);
     \draw[dmetro, metroH] (Helm)--(3.40,2.5)--(3.40,0.6)--(Se);
  \draw[dmetro, metroG] (Sc)--(Hmcj); \draw[dmetro, metroG] (Hmcj)--(Sm);
     \draw[dmetro, metroG] (Hmcj)--(3.55,5.5)--(3.55,7.5)--(Sj);
  \draw[dmetro, metroE] (Sd)--(Hldj); \draw[dmetro, metroE] (Hldj)--(Sl); \draw[dmetro, metroE] (Hldj)--(Sj);
}
\stationcoords
\begin{scope}[on background layer] \dhyperlines \end{scope}
\foreach \id/\lab in {Sa/a,Sc/c,Sd/d,Se/e,Sj/j,Sl/l,Sm/m,Sn/n}
   {\node[metrostop] at (\id) {$\lab$};}
\node[gate] at (Head) {ead}; \node[gate] at (Hnmj) {nmj};
\node[gate] at (Helc) {elc}; \node[gate] at (Hedm) {edm};
...
\node[gate] at (Hmcj) {mcj}; \node[gate] at (Hldj) {ldj};
\end{tikzpicture}}
\caption[The directed helper-point network on a whole hypergraph]{The hypergraph
of \Cref{fig:hyper-gadget-example} made directed, each hyperedge oriented so the
middle of its three vertices is the tail. Every coloured line becomes a one-way
star, two arcs running from the tail through the gate to the two heads, so a Berge
route enters at a tail and leaves at a head, and the boxes are the directed
turnstiles of \Cref{fig:dir-hyper-gadget}. The arrows forbid most of the reverse
transfers the undirected map allowed, so connectivity drops. The busiest pair is
now $d, e$ with two arc-disjoint directed routes, and the checker reads
$\lec(d, e) = 2$, against the undirected four of \Cref{fig:hyper-gadget-example}.}
