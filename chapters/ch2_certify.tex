...
\code{solve} proves \Cref{thm:dir-multi-small} by returning $M^{*}(3) = 4$, $M^{*}(4) = 6$, $M^{*}(5) = 8$, and $M^{*}(6) = 10$, each optimal with zero gap, so that $M^{*}(n) = 2(n-1)$ over this range. The first three run in well under a second, under a second, and a few seconds. The case $n = 6$ takes about twenty minutes. At $n = 7$ the same encoding does not close, and we do not pretend otherwise: it is a finite obstacle of scale, not a new idea, and it marks the current edge of certified knowledge for this variant. The solver transcript is in \Cref{app:proofs}.
