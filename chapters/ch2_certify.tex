...
Menger's theorem turns the connectivity question into a flow computation. A maximum flow is the standard way to ask how much can be pushed from one point to another through a network in which every link has a limited capacity. Two standard theorems chain together to make this work. Menger's theorem says the maximum number of edge-disjoint $u$--$v$ paths equals the minimum $u$--$v$ edge cut, the fewest links whose removal severs every route between them. The max-flow / min-cut theorem then equates that same minimum cut with the value of a maximum flow, once each multiplicity $\mu(u,v)$ is read as the capacity of the link $u \to v$. Putting the two together, the route count we want is neither more nor less than a maximum-flow value, so computing the flow computes the connectivity. The routine that runs this flow and reports the route count is the connectivity \emph{checker}. It is the single most important routine in the program: it is the only place graph theory meets computation, and every later claim about connectivity passes through it. Because the search of \Cref{ch:discover} calls it by the million on tiny graphs, the program ships an optional helper, \code{\_erdos\_fast.so}, that runs this same flow in compiled C. It is a speed-up only: it returns the identical value the pure-Python checker does, verified against it, so no number in the thesis depends on whether the helper is built.
