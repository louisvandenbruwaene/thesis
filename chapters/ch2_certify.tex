...
% shared vertex layout, reused in both panels
\def\hyperlayout{%
  \coordinate (u) at (0,0);
  \coordinate (a) at (1.5,1.55);
  \coordinate (b) at (3.2,1.55);
  \coordinate (c) at (4.6,0.85);
  \coordinate (v) at (6.1,0);
  \coordinate (d) at (1.5,-1.55);
  \coordinate (e) at (3.2,-1.55);
  \coordinate (f) at (4.6,-0.85);
}
% ===== panel (a): the hypergraph as a metro map =====
\begin{scope}
  \hyperlayout
  \begin{scope}[on background layer]
    \draw[metro, metroA] (u)--(a)--(b);
    \draw[metro, metroB] (b)--(c)--(v);
    \draw[metro, metroC] (u)--(d)--(e);
    \draw[metro, metroD] (e)--(f)--(v);
    \draw[metro, metroE] (a)--(c)--(f);
  \end{scope}
  \foreach \p/\l in {a/a,b/b,c/c,d/d,e/e,f/f} {\node[smallvertex] at (\p) {$\l$};}
  \node[end] at (u) {$u$};
  \node[end] at (v) {$v$};
  \node[metroA, font=\scriptsize\bfseries] at (0.7,1.0)  {$e_1$};
  \node[metroB, font=\scriptsize\bfseries] at (5.4,0.95) {$e_2$};
  \node[metroC, font=\scriptsize\bfseries] at (0.7,-1.0) {$e_3$};
  \node[metroD, font=\scriptsize\bfseries] at (5.4,-0.95){$e_4$};
  \node[font=\small] at (3.0,-2.5) {(a) the hypergraph};
\end{scope}
% ===== panel (b): the flow network (a helper turnstile on each line) =====
\begin{scope}[xshift=8.7cm]
  \hyperlayout
  \coordinate (h1) at (1.55,1.0);
  \coordinate (h2) at (4.55,0.78);
  \coordinate (h3) at (1.55,-1.0);
  \coordinate (h4) at (4.55,-0.78);
  \coordinate (h5) at (3.5,0.5);
  % faint membership edges to helper squares
  \draw[mem] (u)--(h1) (a)--(h1) (b)--(h1);
  \draw[mem] (b)--(h2) (c)--(h2) (v)--(h2);
  \draw[mem] (u)--(h3) (d)--(h3) (e)--(h3);
  \draw[mem] (e)--(h4) (f)--(h4) (v)--(h4);
  \draw[mem] (a)--(h5) (c)--(h5) (f)--(h5);
  ...
  \node[font=\small] at (3.0,-2.5) {(b) the flow network: max flow $=2$};
\end{scope}
\end{tikzpicture}
\caption[The helper-point network on a whole hypergraph]{From hypergraph to
flow network, before and after. \textbf{(a)} A hypergraph on eight vertices,
its five hyperedges drawn as metro lines. The pair $u, v$ has two
hyperedge-disjoint Berge routes, $u, e_1, b, e_2, v$ and $u, e_3, e, e_4, v$.
\textbf{(b)} The same vertices with one helper square per hyperedge (a
turnstile on each line). Each Berge route becomes a flow path that enters a
