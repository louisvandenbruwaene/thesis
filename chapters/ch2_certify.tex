...
% A 3-uniform hypergraph on eight stations a,c,d,e,j,l,m,n, labelled by ascending
% x then ascending y. Each of the eight hyperedges is one coloured line meeting its
% three members at a helper junction, drawn as a box named after the edge -- the
% gate of fig:hyper-gadget. Plain straight spokes; the four helper boxes stack up
% the central spine x=4, and the hub e, on five of the eight hyperedges, is where
% the most lines converge.
\def\stationcoords{%
  ...
  \coordinate (Hmcj) at (4,5); \coordinate (Hldj) at (5,5);
}
\def\hyperlines{%
  \draw[metro, metroA] (Head)--(Se); \draw[metro, metroA] (Head)--(Sa); \draw[metro, metroA] (Head)--(Sd);
  \draw[metro, metroB] (Hnmj)--(Sn); \draw[metro, metroB] (Hnmj)--(Sm); \draw[metro, metroB] (Hnmj)--(Sj);
  \draw[metro, metroC] (Helc)--(Se); \draw[metro, metroC] (Helc)--(Sl); \draw[metro, metroC] (Helc)--(Sc);
  \draw[metro, metroD] (Hedm)--(Se); \draw[metro, metroD] (Hedm)--(Sd); \draw[metro, metroD] (Hedm)--(Sm);
  \draw[metro, metroF] (Hnel)--(Se); \draw[metro, metroF] (Hnel)--(Sn); \draw[metro, metroF] (Hnel)--(Sl);
  \draw[metro, metroH] (Helm)--(Se); \draw[metro, metroH] (Helm)--(Sl); \draw[metro, metroH] (Helm)--(Sm);
  \draw[metro, metroG] (Hmcj)--(Sm); \draw[metro, metroG] (Hmcj)--(Sc); \draw[metro, metroG] (Hmcj)--(Sj);
  \draw[metro, metroE] (Hldj)--(Sl); \draw[metro, metroE] (Hldj)--(Sd); \draw[metro, metroE] (Hldj)--(Sj);
}
\stationcoords
...
measures. The hub $e$, on five of the eight hyperedges, is where the most lines
converge, and the four boxes stacked up the central spine are the gates that share
it. The pair $e, l$ lies on the three hyperedges $\{e,l,c\}$, $\{e,l,m\}$ and
...
% to the two heads. Same stations and helper boxes as the undirected figure, plain
% straight arcs with arrowheads added.
\def\stationcoords{%
  \coordinate (Sa) at (0,0);  \coordinate (Sc) at (2,2);  \coordinate (Sd) at (2,6);
  \coordinate (Se) at (4,0);  \coordinate (Sj) at (4,8);  \coordinate (Sl) at (6,2);
  \coordinate (Sm) at (6,6);  \coordinate (Sn) at (8,0);
...
\def\dhyperlines{%
  % middle of each edge string is the tail: ead->a, nmj->m, elc->l, edm->d,
  % nel->e, elm->l, mcj->c, ldj->d. Arrows run tail -> gate -> each head.
  ...
  \draw[dmetro, metroC] (Sl)--(Helc); \draw[dmetro, metroC] (Helc)--(Se); \draw[dmetro, metroC] (Helc)--(Sc);
  \draw[dmetro, metroD] (Sd)--(Hedm); \draw[dmetro, metroD] (Hedm)--(Se); \draw[dmetro, metroD] (Hedm)--(Sm);
  \draw[dmetro, metroF] (Se)--(Hnel); \draw[dmetro, metroF] (Hnel)--(Sn); \draw[dmetro, metroF] (Hnel)--(Sl);
  \draw[dmetro, metroH] (Sl)--(Helm); \draw[dmetro, metroH] (Helm)--(Se); \draw[dmetro, metroH] (Helm)--(Sm);
  \draw[dmetro, metroG] (Sc)--(Hmcj); \draw[dmetro, metroG] (Hmcj)--(Sm); \draw[dmetro, metroG] (Hmcj)--(Sj);
