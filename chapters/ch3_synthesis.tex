...
multigraph
  ...
  & $n-1$ at $m=2$ and $2(n-1)$ at $m=3$ (proved), while it is open for
    $m \ge 4$, where it is a knapsack over $2$-connected blocks with rate
    between $(3-2\sqrt{2})m^2$ and $2m^2$ per added vertex \\
$r$-hypergraph
...
Two block sweeps use separate scripts in \code{program/scripts/}.
...
Directed multigraph incidence, $m\ge3$ & $K_m^{\mathrm{dir}}(n)=(m-1)n^2/4+O_m(n)$ (\Cref{cor:dir-multi-incidence}) & Determine the linear coefficient, and decide whether the arc extremiser is optimal here too. \\
...
Undirected multigraph incidence, $m\ge4$ & Exact at $m\le3$, and a knapsack over $2$-connected blocks whose per-vertex rate lies between $(3-2\sqrt{2})m^2$ and $2m^2$ (\Cref{thm:multi-vertex-blocks,thm:multi-vertex-bipartite,prop:multi-vertex-upper}) & Determine the rate, which asks for the best $2$-connected block of each size. \\
...
best block of each size, which fixes a per-vertex rate currently known only to
lie between $(3-2\sqrt{2})m^2$ and $2m^2$.
