...
Two constructions give lower bounds for the directed arc value.
...
\begin{conjecture}[Eventual exactness]\label{conj:dir-arc}
For every fixed $m\ge2$, there is an integer $N(m)\ge m$ such that, for all $n\ge N(m)$,
\[
    \ell_m^{\mathrm{dir}}(n) = \floorfrac{(n+m-2)^2}{4}.
\]
\end{conjecture}

The earlier conjecture asserted the maximum of the two lower bounds for every
$n\ge m$. It is false. A construction supplied by Stijn Cambie places a
bidirected clique between sources and sinks and gives $57$ arcs at $m=5$,
$n=12$, with $\lec^{\max}=4$, whereas the proposed value was $56$
(\Cref{const:clique-core}). More generally, it refutes that formula whenever
$m\ge5$ and $m+7\le n\le3m-3$. Thus any threshold in
\Cref{conj:dir-arc} must satisfy $N(m)\ge3m-2$ for $m\ge5$.
No formula for the smaller orders or least threshold is asserted here.
Cambie also supplied an AI-assisted argument for eventual exactness.
It is retained as a research note, and the statement remains a conjecture in
this thesis pending the author's verification of that proof.

At $m=2$, eventual exactness follows from \Cref{thm:dir-arc-m2-exact}, with $N(2)=7$.
...
\addlinespace[2pt]
simple digraph
  & $M(n)$ at $m=2$ (proved), and $n^2/4 + \Theta_m(n)$ for $m \ge 3$, with eventual exactness conjectured
...
Here $M(n)=\max(2(n-1),\floor{n^2/4})$. The hypergraph rows use $r\ge3$. At $r=2$, use the corresponding graph rows, in particular the different leading constant for simple digraphs when $m\ge3$. The multigraph rate constants are asymptotic as $m\to\infty$, after $n$ is large enough to pack the blocks. Simple-graph formulas assume
...
The remaining conjecture concerns sufficiently large orders only.
...
\midrule
Directed simple arc, $m\ge3$ & $\ell_m^{\mathrm{dir}}(n)=n^2/4+\Theta_m(n)$ (\Cref{thm:dir-arc-linear-error}) & Verify eventual exactness (\Cref{conj:dir-arc}), then determine the least threshold and the values below it. \\
...
\Cref{conj:dir-arc} predicts the exact value for sufficiently large orders.
